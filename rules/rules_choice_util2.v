...
Proof.
  ...
  - apply collapse_all_in_ex_bar.
    eapply in_open_bar_pres; eauto; clear k; introv x k; exrepnd; spcast.
  ...
  - eapply all_in_ex_bar_modus_ponens1;try exact k; clear k; introv x k; exrepnd; spcast.
    ...
    + apply ccequivc_ext_sym in k0.
      ...
      exists (@mkc_axiom o) (@mkc_axiom o); dands; auto.
      apply implies_equality_in_unit; spcast; eauto 3 with slow.
...
Proof.
  ...
  eapply lib_extends_preserves_in_open_bar in imp; eauto.
  apply all_in_ex_bar_equality_implies_equality.
  apply equality_in_tnat in ea.
  unfold equality_of_nat_bar in ea.
  eapply in_open_bar_comb; try exact imp; clear imp.
  eapply in_open_bar_pres; try exact ea; clear ea; introv xta ea imp.
  unfold equality_of_nat in *; exrepnd.
  pose proof (imp n) as imp.
  ...
  apply equality_in_bool_ext.
  eapply in_open_bar_pres; try exact imp; clear imp.
  introv xtb imp; exrepnd.
  destruct b;[left|right]; allapply @ccomputes_to_valc_ext_implies_ccequivc_ext; tcsp.
Qed.
...
Lemma is_bool_if_compatible_and_correct {o} :
...
Proof.
  ...
  destruct res, name as [name kd], kd; simpl in *; tcsp; boolvar; subst; repnd; tcsp;
    try (complete (unfold compatible_cs_kind in *; boolvar; try lia));
...
Lemma iscvalue_mkc_boolean {o} : forall b, @iscvalue o (mkc_boolean b).
Proof.
  introv; destruct b; simpl; eauto 3 with slow.
Qed.
...
Lemma extends_bool_choice_sequence_entry {o} :
  forall (lib lib' : @library o) name,
    ...
    -> exists (bs : list bool),
        entry_in_library (lib_cs name (MkChoiceSeqEntry _ (map mkc_boolean bs) csc_bool)) lib'.
Proof.
  introv safe ext i.
  eapply lib_extends_preserves_safe in ext as safe'; auto.
  ...
  inversion i0; subst; tcsp; clear i0; simpl in *; repnd.

  { exists ([] : list bool); tcsp. }
...
Lemma entry_extends_bool_choice_sequence_entry {o} :
  forall (entry : @library_entry o) name,
    ...
    -> exists l, entry = lib_cs name (MkChoiceSeqEntry _ (map mkc_boolean l) csc_bool).
Proof.
  introv safe ck ext.
  inversion ext; subst; clear ext; simpl in *; eauto.
  { exists ([] : list bool); tcsp. }
  repnd.
...
Proof.
  ...
  apply entry_in_library_extends_implies_entry_in_library in i; exrepnd.
  eapply entry_in_library_implies_safe_library_entry in safe';[|eauto].
  inversion i0; subst; clear i0; simpl in *; repnd.

  { exists ([] : list bool); autorewrite with slow; auto. }
...
(*Lemma implies_safe_inf_choice_sequence_entry2inf_def {o} :
  forall name (entry : @ChoiceSeqEntry o) r v,
    ...
    -> safe_inf_choice_sequence_entry name (choice_seq_entry2inf_def entry (Some v)).
Proof.
  ...
  - unfold choice_seq_vals2inf.
    ...
    destruct s; auto; try (complete(compute; tcsp));[].
  ...
  - unfold choice_seq_vals2inf.
    remember (select n vals) as s; symmetry in Heqs.
    unfold on_some; destruct s; auto.
...
(*Lemma implies_safe_inf_library_library2inf_def {o} :
  forall (lib : @library o) d name v r,
    ...
    -> safe_inf_library (library2inf_def lib d name (Some v)).
Proof.
...
(*Lemma inf_lib_extends_library2inf_def {o} :
  forall (lib : @library o) d name v r,
    ...
    -> choice_sequence_satisfies_restriction [v] r
    -> safe_inf_library_entry d
    -> inf_lib_extends (library2inf_def lib d name (Some v)) lib.
Proof.
...
(*Lemma entry_extends_bool_choice_sequence_entry_implies_def {o} :
  forall (lib : @library o) name name0 lib' entry' b,
    ...
    -> safe_library_entry entry'
    -> csn_kind name = cs_kind_nat 1
    -> inf_lib_extends (library2inf_def lib (simple_inf_choice_seq name0) name (Some (mkc_boolean b))) lib'
    -> entry_in_library (bool_choice_sequence_entry name) lib
    -> entry_in_library entry' lib'
    -> entry_extends entry' (bool_choice_sequence_entry name)
    -> exists restr n,
...
Proof.
  ...
  inversion ext; subst; clear ext; simpl in *; repnd; eauto.

  { exists ([] : list bool); autorewrite with slow; tcsp. }
  ...
  assert (forall v, LIn v vals' -> exists (b : bool), v = mkc_boolean b) as vn.
  {
    ...
    autodimp q hyp.
    { erewrite nth_select1; auto; rewrite q0 at 1; eauto. }
    destruct name as [name kind]; unfold correct_restriction in *; simpl in *; subst; simpl in *; repnd.
