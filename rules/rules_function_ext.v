...
Proof.
...

  {
    ...
    - introv ea.
      ext_seq_true in hyp1.
      pose proof (hyp1 (snoc s1 (z,a)) (snoc s1 (z,a'))) as q; exrepnd; clear hyp1.
      repeat (autodimp q hyp).
    ...
    - introv ea.
      ...
      assert (!LIn z (dom_csub s1)) as nz1.
      { allapply @similarity_dom; repnd; allrw; auto. }

      assert (!LIn z (dom_csub s2)) as nz2.
      { allapply @similarity_dom; repnd; allrw; auto. }
      ...
      pose proof (lsubstc_snoc_move b s1 [] z a wfce1) as q.
      autorewrite with list in q.
      pose proof (q pt0) as h; clear q; autodimp h hyp; exrepnd; proof_irr.
      rewrite <- h0; clear h0.

      pose proof (lsubstc_snoc_move b s2 [] z a' wfce1) as q.
      autorewrite with list in q.
      pose proof (q pt3) as h; clear q; autodimp h hyp; exrepnd; proof_irr.
      rewrite <- h0; clear h0.
      auto.
  }
Qed.
...
Lemma rule_lambda_formation_true_ex {p} :
  forall lib i z A B b e x H (bc1 : !LIn z (bound_vars B)),
    @rule_true_if p lib (rule_lambda_formation A B b e x z i H).
Proof.
  intros.
  generalize (rule_lambda_formation_true lib A B b e x z i H bc1); intro rt.
...
Definition rule_function_elimination_wf_concl {o}
  ...
  mk_baresequent
    ...
    (mk_concl C (subst e z (mk_prefl (mk_apply (mk_var f) a) (mk_apply (mk_var f) a)))).
...
Lemma rule_function_elimination_wf_true3 {p} :
  forall lib (A B C a e1 e2 : NTerm)
         (f x z : NVar)
         (H J : @barehypotheses p)
         (i : nat),
    ext_rule_true lib (rule_function_elimination_wf
                         A B C a e1 e2
                         f x z
                         H J
                         i).
Proof.
  unfold rule_function_elimination_wf, ext_rule_true, wf_bseq, closed_type_baresequent, closed_extract_baresequent; simpl.
  intros; repnd.
  ...
  match goal with
  | [ |- context[closed_extract ?a ?b] ] => assert (closed_extract a b) as cec
  end.
  {
    clear hyp1 hyp2.
    unfold closed_extract, wf_sequent, wf_concl; simpl.
          ...
          # (x <> f -> !LIn f (free_vars B))) as vhyps.
  {
    ...
    sp;
      ...
      try (complete (pose proof (cg z) as xx; autodimp xx hyp;
                     rw in_app_iff in xx; rw in_snoc in xx; repndors; tcsp)).
  }
...

  {
    ...
    apply ext_hyps_functionality_snoc2; simpl; auto;[].
    ...
    applydup hf in sim as hf'.
    apply ext_eq_hyps_app in hf'; simpl in hf'; exrepnd; subst; cpx.
    apply app_split in hf'0; repnd; allrw length_snoc;
    ...
    {
      apply ext_sub_eq_hyps_dom in hf'1; repnd.
      rw hf'1; auto.
    }

    rw @ext_eq_hyps_snoc in hf'5; simpl in hf'5; exrepnd; subst; cpx.
    lsubst_tac.

    revert hf'0 hf'6; introv per teq.

    apply per_intensional_function in per; repnd.

    dands.

    - apply implies_tequality_mkc_member_if_equal.

      + applydup per1 in eqa.
...
XXXXXXXXX
  ...
  assert (wf_term (mk_prefl (mk_apply (mk_var f) a) (mk_apply (mk_var f) a))) as wfapp.
  { apply wf_prefl; dands; apply wf_apply; eauto 3 with slow. }

  assert (cover_vars (mk_prefl (mk_apply (mk_var f) a) (mk_apply (mk_var f) a))
                     (snoc s1a0 (f, t1) ++ s1b)) as covapp1.
  { apply cover_vars_prefl; dands;
      apply cover_vars_apply; dands; eauto 3 with slow;
        apply cover_vars_var; rw @dom_csub_app; rw @dom_csub_snoc; simpl;
          rw @in_app_iff; rw in_snoc; tcsp. }

  assert (cover_vars (mk_prefl (mk_apply (mk_var f) a) (mk_apply (mk_var f) a))
                     (snoc s2a0 (f, t2) ++ s2b)) as covapp2.
  { apply cover_vars_prefl; dands;
      apply cover_vars_apply; dands; eauto 3 with slow;
        apply cover_vars_var; rw @dom_csub_app; rw @dom_csub_snoc; simpl;
          rw @in_app_iff; rw in_snoc; tcsp. }
  ...
  { apply similarity_dom in sim6; repnd.
    ...
    rewrite sim8.
    rewrite vars_hyps_substitute_hyps; auto.
    rw in_app_iff; rw in_snoc; intro j; repndors; tcsp.
  }
  exrepnd.
  proof_irr.
  ...
  pose proof (lsubstc_snoc_move
                ...
                []
                z
                (mkc_prefl (mkc_apply t2 (lsubstc a wa (snoc s2a0 (f, t2) ++ s2b) cova2))
                           (mkc_apply t2 (lsubstc a wa (snoc s2a0 (f, t2) ++ s2b) cova2)))
                wfce0) as xx1.
  allrw app_nil_r.
  allrw @dom_csub_app.
...
Lemma rule_function_elimination_wf_true {p} :
  forall lib (A B C a e1 e2 e3 : NTerm)
         (f x z : NVar)
         (H J : @barehypotheses p)
         (i : nat),
    rule_true lib (rule_function_elimination_wf
                     A B C a e1 e2 e3
                     f x z
                     H J
                     i).
Proof.
  ...
  apply rule_function_elimination_wf_true3.
Qed.

Lemma rule_function_elimination_wf_true2 {o} :
  forall lib A B C a e1 e2 e3 f x z H J i,
    @rule_true2 o lib (rule_function_elimination_wf A B C a e1 e2 e3 f x z H J i).
Proof.
  intros.
  apply rule_true_iff_rule_true2.
  apply rule_function_elimination_wf_true.
Qed.
...
Lemma rule_function_elimination_true_ex {p} :
  forall lib (A B C a e1 e2 e3 : NTerm)
         ...
         (H J : @barehypotheses p)
         (i : nat),
    rule_true_if lib (rule_function_elimination_wf
                        A B C a e1 e2 e3
                        f x z
                        H J
                        i).
Proof.
  intros.
  generalize (rule_function_elimination_wf_true lib A B C a e1 e2 e3 f x z H J i); intro rt.
  rw <- @rule_true_eq_ex in rt.
  unfold rule_true_ex in rt; sp.
Qed.
...
Lemma rule_function_elimination_wf_wf {o} :
  forall (A B C a e1 e2 e3 : @NTerm o) f x z H J i,
    wf_term a
    ...
    -> wf_term e1
    -> wf_term e3
    -> wf_rule (rule_function_elimination_wf A B C a e1 e2 e3 f x z H J i).
Proof.
  introv wa cov nizH nizJ dzf we1 we3.
...
Lemma rule_function_elimination_wf_wf2 {o} :
  forall (A B C a e1 e2 e3 : @NTerm o) f x z H J i,
    wf_term a
    -> covered a (snoc (vars_hyps H) f ++ vars_hyps J)
    -> !LIn z (vars_hyps H)
    -> !LIn z (vars_hyps J)
    -> z <> f
    -> wf_rule2 (rule_function_elimination_wf A B C a e1 e2 e3 f x z H J i).
Proof.
...
<<
   H |- x1:a1 -> b1 = x2:a2 -> b2 in Type(i)
     ...
     H |- a1 = a2 in Type(i)
     H y : a1 |- b1[x1\y] = b2[x2\y] in Type(i)
>>
 *)
...
Definition rule_function_equality {p}
  ...
  mk_rule
    (mk_baresequent
       H
       (mk_concl_eq (mk_function a1 x1 b1)
                    (mk_function a2 x2 b2)
                    (mk_uni i)))
    [ mk_baresequent H (mk_concl_eq_ext a1 a2 (mk_uni i) e1),
      mk_baresequent
        (snoc H (mk_hyp y a1))
        (mk_concl_eq_ext (subst b1 x1 (mk_var y))
                         (subst b2 x2 (mk_var y))
                         (mk_uni i)
                         e2)
    ]
    [ sarg_var y , sarg_term (mk_function a1 x1 b1) ].

Lemma rule_function_equality_true3 {o} :
  forall lib (a1 a2 b1 b2 e1 e2 : NTerm)
         (x1 x2 y : NVar)
         (i   : nat)
         (H   : @barehypotheses o),
...
Definition rule_lambda_equality {o}
  ...
  mk_rule
    (mk_baresequent
       H
       (mk_concl_eq (mk_lam x1 t1)
                    (mk_lam x2 t2)
                    (mk_function A x B)))
    [ mk_baresequent
        (snoc H (mk_hyp z A))
        (mk_concl_eq_ext (subst t1 x1 (mk_var z))
...
Lemma rule_lambda_equality_true {o} :
  forall lib (A B t1 t2 e1 e2 : NTerm)
    ...
    rule_true lib (rule_lambda_equality A B t1 t2 e1 e2 x1 x2 x z i H).
Proof.
  intros.
  ...
  match goal with
  | [ |- context[covered ?a ?b] ] => assert (covered a b) as covc
  end.
  {
    clear hyp1 hyp2.
...
Definition rule_function_elimination {p}
           ...
           (H J : @barehypotheses p) :=
  mk_rule
    (rule_function_elimination_concl A B C a e f x z H J)
    [
...
Proof.
  ...
  assert (wf_csequent (rule_function_elimination_concl A B C a e f x z H J)) as wfc.
  {
    clear hyp1 hyp2.
...

  {
    ...
    apply implies_tequality_mkc_member_if_member.

    {
      ...
      vr_seq_true in hyp1.
      pose proof (hyp1 (snoc s1a (f, t0) ++ s1b0) (snoc s2a1 (f, t3) ++ s2b0)) as h1h.
      repeat (autodimp h1h h); exrepnd;[].
      lsubst_tac.

      pose proof (hyp1 (snoc s2a1 (f, t3) ++ s2b0) (snoc s2a1 (f, t3) ++ s2b0)) as h2h.
      repeat (autodimp h2h h); exrepnd; eauto 3 with slow.
      ...
      apply member_if_inhabited in h1h1.
      apply member_if_inhabited in h2h1.

      applydup @tequality_implies_if_member in h1h0; auto;[].
...
Proof.
  ...
  apply tequality_iff_ext_eq; repnd; dands; eauto 3 with slow.
  introv; split; intro h.
