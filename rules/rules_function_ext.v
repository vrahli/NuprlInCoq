...
Definition rule_function_elimination {p}
           ...
           (names1 names2 : list choice_sequence_name) :=
  mk_rule_ext
    (rule_function_elimination_concl A B C e f x z H J)
    (names1 ++ names2)
    [
      (rule_function_elimination_hyp1 A B a ea f x H J,    names1),
      (rule_function_elimination_hyp2 A B C a e f x z H J, names2)
    ]
...
Lemma rule_function_elimination_true3 {p} :
  forall lib (A B C a e : NTerm) ea
         ...
         (H J : @barehypotheses p)
         names1 names2,
    rule_true_ext lib (rule_function_elimination
                         A B C a e
                         ea
                         f x z
                         H J
                         names1 names2).
Proof.
  unfold rule_function_elimination, rule_true_ext, wf_bseq, closed_type_baresequent, closed_extract_baresequent; simpl.
  intros; repnd.
  clear cargs.
  ...
  vr_seq_ext_true in hyp2.

  assert (lib_extends_upto lib' lib names1) as ext1 by eauto 3 with slow.
  assert (lib_extends_upto lib' lib names2) as ext2 by eauto 3 with slow.
...

  {
    ...
    vr_seq_ext_true in hyp1.
    generalize (hyp1 _ (lib_extends_trans xt ext1) (snoc s1a0 (f, t1) ++ s1b) s'); clear hyp1; intros hyp1.
...
Proof.
  ...
  rw @tequality_function in hyp8; repnd.
    ...
    as eq1.
  {
    ...
    apply @cequorsq_equality_trans2 with (t2 := lsubstc t1 w1 s1 c1); sp.
  ...
  {
    apply all_in_ex_bar_equality_implies_equality.
    eapply all_in_ex_bar_modus_ponens2;[|exact hyp0|exact hyp11]; clear hyp0 hyp11; introv y hyp0 hyp11; exrepnd; spcast.
  ...
  {
    generalize (hyp8 _ (lib_extends_refl lib') (lsubstc t1 w1 s1 c1) (lsubstc t2 w2 s2 c3)); intro k1.
    autodimp k1 hyp; sp.
    generalize (hyp8 _ (lib_extends_refl lib') (lsubstc t2 w2 s1 c2) (lsubstc t2 w2 s2 c3)); intro k2.
    autodimp k2 hyp; sp.
  ...
  {
    (* from hyp 4 *)
    generalize (hyp8 _ (lib_extends_refl lib') (lsubstc t1 w1 s1 c1) (lsubstc t1 w1 s2 c0) eq1); intro teq.
    repeat lsubstc_subst_aeq2.
    proof_irr.
    auto.
  }
...
Proof.
  ...
  assert (hyps_functionality_ext lib' s2 H)
    as eqh2
...

  {
...

    {
      ...
      introv ext'' e.
      assert (equality lib'1 a' a' (lsubstc A wA s2 c6))
        as e'
      ...
      exact k2.
      apply @tequality_sym.
      rw @tequality_function in eqfunc; repnd; sp; eauto 3 with slow.
    }

    {
      ...
      split; try (complete (apply @tequality_refl in eqfunc; eauto 3 with slow)).
      introv ext'' e.
      assert (equality lib'1 a a (lsubstc A wA s1 c4)) as e' by (allapply @equality_refl; eauto 3 with slow).
