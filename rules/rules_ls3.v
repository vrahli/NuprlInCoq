...
Fixpoint lib_depth {o} (lib : @library o) : nat :=
  match lib with
  | [] => 0
...
Lemma lib_extends_replace_cs_entry_app {o} :
...
Proof.
  ...
  { Case "lib_ext_cs".
    ...
    { inversion xx0 as [xx]; clear xx0.
      ...
      eapply replace_cs_entry_if_add_choice in addc; eauto; boolvar; tcsp; GC.
...
Lemma find_cs_implies_lib_extends_replace_cs_entry_app_left {o} :
  forall name (vals vals' : @ChoiceSeqVals o) restr lib,
    correct_restriction name restr
    -> choice_sequence_satisfies_restriction (vals ++ vals') restr
...
Lemma lib_extends_replace_cs_entry_between {o} :
...
Proof.
  ...
  inversion q3 as [? ? ? ya yb]; subst; clear q3.
  rewrite <- app_assoc in ya.
  apply app_inv_head in ya; subst; simpl in *.

  dands; try (complete (eapply lib_extends_replace_cs_entry_app; eauto));[].
  eapply lib_extends_preserves_safe in ext; eauto.
  apply find_cs_some_implies_entry_in_library in fcs.
  rewrite app_assoc in fcs; eauto 3 with slow.
  apply ext in fcs; simpl in *; repnd.
  apply find_cs_implies_lib_extends_replace_cs_entry_app_left; eauto 3 with slow.
Qed.
...
Lemma find_cs_replace_cs_entry_implies {o} :
  forall (lib : @library o) name e name' e',
    find_cs (replace_cs_entry lib name e) name' = Some e'
    -> if choice_sequence_name_deq name name'
       then e' = e
...
Lemma cs_entry_between_firstn_cs_entry_replace_cs_entry {o} :
...
Proof.
  ...
  apply find_cs_replace_cs_entry_implies in fcs; boolvar; subst; eauto 3 with slow.
  rewrite fcsa in fcsb; ginv; eauto 3 with slow.
Qed.
Hint Resolve cs_entry_between_firstn_cs_entry_replace_cs_entry : slow.
...
Definition lib_names {o} (lib : @library o) : list EntryName := map entry2name lib.

Fixpoint erase_entries {o} (lib : @library o) : library :=
  match lib with
  | [] => []
  | e :: rest => erase_entry e :: erase_entries rest
  end.

Fixpoint erase_entries_upto {o} (name : EntryName) (lib : @library o) : library :=
  match lib with
  | [] => []
  | e :: rest =>
...
Definition has_first_name {o} name (lib : @library o) :=
  match lib with
  | [] => False
...
Proof.
  ...
  destruct name as [op params sign]; simpl.
...
Definition dec_has_first_name {o} :
  forall (lib : @library o), lib = [] \/ exists name, has_first_name name lib.
Proof.
  introv; destruct lib; simpl in *; tcsp; right; eauto.
  eexists; apply same_entry_name_refl.
Qed.

Lemma inc_cs_lib_same {o} :
  forall name (lib : @library o),
    has_first_name name lib
    -> inc_cs_lib name lib lib = lib.
Proof.
  introv hf; unfold has_first_name, inc_cs_lib in *; destruct lib; simpl; auto; boolvar; tcsp.
Qed.

Lemma inc_cs_lib_nil {o} :
  forall name (lib : @library o), inc_cs_lib name [] lib = lib.
Proof.
  tcsp.
Qed.
Hint Rewrite @inc_cs_lib_nil : slow.
...
Fixpoint extension {o} (lib2 lib1 : @library o) : Prop :=
...
Lemma extension_refl {o} :
  forall (lib : @library o), extension lib lib.
Proof.
  induction lib; introv; simpl; auto.
Qed.
...
Lemma eq_app_cons_implies {o} :
  forall (liba libb libc libd : @library o) e1 e2,
...
Lemma extension_trans {o} :
  forall (liba libb libc : @library o),
    extension liba libb
    -> extension libb libc
    -> extension liba libc.
Proof.
...
Proof.
  ...
  { destruct lib1; simpl in *; tcsp.
    ...
    exists ([] : @library o) ([] : @library o); simpl; auto. }

  destruct lib1; simpl in *.

  { destruct lib2; simpl in *; tcsp; repnd.
    exists ([] : @library o) (a :: lib); simpl; dands; eauto 3 with slow. }
...
Proof.
  introv ext; lib_ext_ind ext Case; introv; tcsp;
    try (complete (try apply add_choice_preserves_lib_names in addc; exists ([] : list EntryName); simpl; auto)).
...
Fixpoint lib_nodup {o} (lib : @library o) :=
  match lib with
  | [] => True
...
Proof.
  ...
  { Case "lib_ext_cs".
    eapply (lib_extends_cs _ name); try exact cond.
...
Lemma extension_implies_lib_extends {o} :
  forall (lib2 lib1 : @library o),
    ...
    -> lib_extends lib2 lib1.
Proof.
  induction lib2; introv safe nodup ext; simpl in *; destruct lib1; simpl in *; tcsp; repnd.
  allrw @strong_safe_library_cons; repnd.
  applydup IHlib2 in ext; auto.
  applydup @extension_implies_same_names in ext.
  eapply lib_extends_trans;[|apply implies_lib_extends_cons;[|eauto] ]; eauto 3 with slow.
  apply implies_lib_cons_lr; auto.
Qed.
...
Proof.
  introv ext; revert e; lib_ext_ind ext Case; introv safe nodup i; tcsp.

  { Case "lib_ext_refl".
    apply entry_in_library_split in i; exrepnd.
    exists lib1 lib2 lib1 lib2 e; dands; tcsp; eauto 3 with slow. }

  { Case "lib_ext_trans".
    assert (strong_safe_library lib2) as safe' by eauto 3 with slow.
    assert (lib_nodup lib2) as nodup' by eauto 3 with slow.
    pose proof (IHext2 _ safe nodup i) as IHext2; exrepnd; subst.
    dup i as j.
    eapply entry_in_library_extends_right in j; eauto; exrepnd.
    pose proof (IHext1 _ safe' nodup' j1) as IHext1; exrepnd; subst.
    ...
    exists liba0 libb0 libc libd e'1; dands; eauto 3 with slow;
      try (complete (eapply extension_trans; eauto)). }

  { Case "lib_ext_new_abs".
    applydup @entry_in_library_split in i; exrepnd; subst.
    exists (lib_abs op vars rhs correct :: lib1) lib2 lib1 lib2 e; dands; eauto 3 with slow.
    ...
    eapply lib_extends_new_abs; intro xx; destruct ni.
    apply in_lib_app; tcsp. }

  { Case "lib_ext_new_cs".
    applydup @entry_in_library_split in i; exrepnd; subst.
    unfold add_cs; simpl.
    exists (lib_cs name (MkChoiceSeqEntry _ [] restr) :: lib1) lib2 lib1 lib2 e; dands; eauto 3 with slow.

    { intro xx; allrw @in_lib_cons; simpl in *; repndors; tcsp.
      destruct ni; allrw @in_lib_app; allrw @in_lib_cons.
      right; left; apply same_entry_name_sym; auto. }

    eapply lib_extends_new_cs; auto; intro xx; destruct ni.
    apply in_lib_app; tcsp. }
...
Definition lib_disjoint {o} (lib1 lib2 : @library o) :=
  forall n, in_lib n lib1 -> ~in_lib n lib2.

Lemma lib_disjoint_cons_right {o} :
  forall lib e (lib' : @library o),
...
Lemma extension_implies_lib_extends_app {o} :
  forall (lib2 lib1 : @library o) lib,
    lib_disjoint lib lib2
    -> strong_safe_library lib2
    -> lib_nodup lib2
    -> extension lib2 lib1
    -> lib_extends (lib ++ lib2) (lib ++ lib1).
Proof.
  induction lib2; introv disj safe nodup ext; simpl in *; destruct lib1; simpl in *; tcsp; repnd.
  allrw @strong_safe_library_cons; repnd.
  apply lib_disjoint_cons_right in disj; repnd.
  applydup (IHlib2 lib1 (snoc lib a)) in ext; auto; try (complete (apply lib_disjoint_snoc_left; tcsp));[].
  rewrite snoc_as_append in ext1; repeat (rewrite <- app_assoc in ext1); simpl in *.
  eapply lib_extends_trans;[exact ext1|]; clear ext1.
  apply lib_extends_middle; eauto 3 with slow.
...
Lemma implies_lib_extends_app {o} :
  forall (liba libb libc libd : @library o),
    ...
    -> lib_extends (liba ++ libb) (libc ++ libd).
Proof.
  introv ext; revert libb libd; lib_ext_ind ext Case; introv ext' safe nodup disj; tcsp.
  ...
  { Case "lib_ext_trans".
    ...
    pose proof (IHext2 _ _ ext' safe nodup disj') as IHext2.
    eapply lib_extends_trans;[|eauto].
  ...
  { Case "lib_ext_cs".
    apply (extension_implies_lib_extends_app _ _ lib) in ext'; eauto 3 with slow;[].
    eapply lib_extends_trans;[|eauto].
    eapply lib_extends_cs;[apply implies_add_choice_app;eauto|]; auto. }
...
Lemma lib_disjoint_nil {o} :
  forall (lib : @library o), lib_disjoint [] lib.
Proof.
...
Lemma in_lib_erase_entries {o} :
  forall n (lib : @library o), in_lib n (erase_entries lib) <-> in_lib n lib.
Proof.
...
Lemma implies_lib_extends_app_left {o} :
  forall (lib k : @library o),
    ...
    -> lib_extends (lib ++ k) k.
Proof.
  induction lib; introv disj nodup safe; simpl in *; repnd; eauto.
  allrw @lib_disjoint_cons_left; repnd.
  allrw @strong_safe_library_cons; repnd.
  eapply lib_extends_trans;[|apply IHlib;auto].
  apply implies_lib_extends_cons_left; auto.
...
Proof.
  introv hf safe nodup ext.
  unfold has_first_name, inc_cs_lib in *.
  ...
  dup ext as ext'.
  apply (in_lib_extends_implies_split _ _ e) in ext'; simpl; auto.
  exrepnd.
  assert (e :: lib1 = [] ++ e :: lib1) as xx by (simpl; auto).
  rewrite xx in ext'2; clear xx.
  apply eq_app_cons_implies in ext'2; simpl; eauto 3 with slow; repnd; subst; simpl in *; GC.
  apply eq_app_cons_implies in ext'1; simpl; eauto 3 with slow; repnd; subst; simpl in *; GC;
    try (complete (allapply @entry_extends_implies_same_entry_name; eauto 3 with slow)).

  applydup @lib_extends_preserves_strong_safe_library in ext as safe'; auto;[].
  allrw @strong_safe_library_app; repnd.
  allrw @strong_safe_library_cons; repnd.
  applydup @lib_extends_preserves_nodup in ext as ext'; simpl; tcsp;[].
  ...
  { apply implies_lib_extends_app;
    ...
    apply extension_implies_lib_extends; eauto 3 with slow. }

  { assert (e :: libd = [] ++ e :: libd) as xx by (simpl; auto); rewrite xx at 2; clear xx.
    apply implies_lib_extends_app;
      allrw @strong_safe_library_cons;
      allrw @lib_disjoint_cons_right;
      autorewrite with slow; simpl; dands; eauto 3 with slow;
    ...
    apply implies_lib_extends_app_left; autorewrite with slow; eauto 3 with slow. }
...
Lemma lib_extends_replace_cs_entry_between1 {o} :
  forall name e e' (lib2 lib1 : @library o),
    ...
    -> lib_extends (replace_cs_entry lib1 name e') lib1.
Proof.
  introv safe find btw ext; eapply lib_extends_replace_cs_entry_between; eauto.
Qed.
...
Lemma entry_in_replace_cs_entry_implies {o} :
  forall entry (lib : @library o) name e,
    entry_in_library entry (replace_cs_entry lib name e)
...
Proof.
  ...
  apply entry_in_replace_cs_entry_implies in i; repndors; subst; simpl in *; tcsp.
Qed.
...
Lemma entry_in_inc_cs_lib_implies {o} :
  forall entry name (lib2 lib1 : @library o),
    entry_in_library entry (inc_cs_lib name lib2 lib1)
...
Lemma lib_depth_inc_cs_lib {o} :
  forall name (lib2 lib1 : @library o),
    lib_depth (inc_cs_lib name lib2 lib1) = lib_depth lib1.
Proof.
  introv; unfold inc_cs_lib.
...
Lemma eq_lib_names_preserves_find_cs {o} :
  forall (lib2 lib1 : @library o) name c,
    lib_names lib2 = lib_names lib1
    -> find_cs lib2 name = Some c
...
Proof.
  introv isna isnb safe nodup ext imp.
  ...
  { eapply lib_extends_trans.
    { eapply lib_extends_replace_cs_entry_between1; eauto; eauto 3 with slow.
      { apply implies_safe_library_replace_cs_entry; simpl; eauto 4 with slow. }
      { pose proof (lib_depth_inc_cs_lib name0 lib2 lib1) as xx; rewrite <- xx.
        eapply cs_entry_between_firstn_cs_entry_replace_cs_entry; try exact Heqfcsa; auto.
...
Proof.
  ...
  clear lib safe nodup ext.
  rename lib' into lib; rename safe' into safe; rename nodup' into nodup.
...
(*  eapply member_monotone in eqz;[|exact xt];[].*)
  ...
  assert (strong_safe_library lib'0) as safe' by eauto 3 with slow.
  assert (lib_nodup lib'0) as nodup' by eauto 3 with slow.
...
Print is_nat_seq_restriction.
  ...
  assert (contains_only name (mkc_apply u (mkc_choice_seq name))) as conta by admit.
  assert (sane_swapping (name, name')) as sane by eauto 3 with slow.

  apply (member_implies_keep_only name) in eqz; auto.
  ...
  rewrite swap_cs_lib_keep_only_nat in eqz; eauto 3 with slow.

  pose proof (lib_extends_keep_only name' lib0) as ext0; autodimp ext0 hyp; eauto 3 with slow.
  eapply member_monotone in eqz; try exact ext0.
  eapply member_monotone in eqz; try exact q2.


(*
  if [member] was [inhabited] we would be done here
 *)
...
Inductive op_kind :=
| opk_abs (a : opabs)
| opk_cs  (c : choice_sequence_name)
| opk_base
| opk_type
| opk_free.

Definition op_kinds := list op_kind.

Definition get_ops_c {p} (c : @CanonicalOp p) : op_kinds :=
  match c with
  | Ncseq c => [opk_cs c]
  | NCSName _ => [opk_free]
  | NUni _ => [opk_type]
...
Definition get_ops_o {p} (o : @Opid p) : op_kinds :=
  match o with
  | Can c => get_ops_c c
  | Abs a => [opk_abs a]
  | _ => []
...
Fixpoint get_ops {p} (t : @NTerm p) : op_kinds :=
  match t with
  | vterm _ => []
  | oterm o bterms => (get_ops_o o) ++ (flat_map get_ops_b bterms)
  end
with get_ops_b {p} (bt : @BTerm p) : op_kinds :=
       match bt with
       | bterm _ t => get_ops t
       end.

(*Fixpoint keep_only_list {o} (names : list choice_sequence_name) (lib : @library o) : @library o :=
  match lib with
  | [] => []
  | lib_cs name e :: entries =>
    if in_dec choice_sequence_name_deq name names
    then [lib_cs name e]
    else keep_only_list names entries
  | entry :: entries => keep_only_list names entries
  end.*)
...
Lemma implies_replace_from_names_extends {o} :
...
Proof.
  ...
  { eapply lib_extends_preserves_safe; eauto; eauto 3 with slow. }
  ...
  remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs;
    [|rewrite replace_cs_entry_if_find_none; eauto].
  apply implies_lib_extends_ext in ext.
  assert (entry_in_library (lib_cs name c0) [lib_cs name c0]) as i by tcsp.
  apply ext in i; simpl in *.
  apply entry_in_library_extends_implies_entry_in_library in i; exrepnd.

  inversion i0; subst; clear i0.

  { apply find_cs_some_implies_entry_in_library in Heqfcs'.
    eapply two_entry_in_library_implies_or in i1; try exact Heqfcs'.
    unfold matching_entries in *; simpl in *; repndors; tcsp; ginv.
    rewrite replace_cs_entry_if_find_cs_in; auto. }
  ...
  apply safe' in Heqfcs'; simpl in *; repnd.
...
Lemma find_cs_lib_extends_implies_cs_entry_extends {o} :
  forall name e2 e1 (lib2 lib1 : @library o),
    lib_extends lib2 lib1
    -> find_cs lib2 name = Some e2
    -> find_cs lib1 name = Some e1
    -> cs_entry_extends e2 e1.
Proof.
  introv ext fcsa fcsb.
  apply find_cs_some_implies_entry_in_library in fcsa.
  apply find_cs_some_implies_entry_in_library in fcsb.
  apply implies_lib_extends_ext in ext.
  apply ext in fcsb.
  apply entry_in_library_extends_implies_entry_in_library in fcsb; exrepnd.
  eapply two_entry_in_library_implies_or in fcsb1; try exact fcsa.
  unfold matching_entries in *; simpl in *.
  repndors; subst; inversion fcsb0; subst; simpl in *; eauto 3 with slow; tcsp.
Qed.

Lemma replace_cs_entry_twice {o} :
  forall (lib : @library o) name c1 c2,
    replace_cs_entry (replace_cs_entry lib name c1) name c2
    = replace_cs_entry lib name c2.
Proof.
  induction lib; introv; simpl in *; tcsp.
  destruct a; repeat (simpl in *; boolvar; subst; tcsp); rewrite IHlib; auto.
Qed.
Hint Rewrite @replace_cs_entry_twice : slow.

Lemma safe_choice_sequence_nat_implies_same_restrictions {o} :
  forall name (e c : @ChoiceSeqEntry o),
    is_nat_cs name
    -> safe_choice_sequence_entry name e
    -> safe_choice_sequence_entry name c
    -> cse_restriction e = cse_restriction c.
Proof.
  introv isn safea safeb.
  unfold safe_choice_sequence_entry, is_nat_cs in *.
  destruct e as [vals1 restr1], c as [vals2 restr2]; simpl in *; repnd.
  destruct name as [n k], k, restr1, restr2; unfold correct_restriction in *;
    simpl in *; boolvar; subst; tcsp; try omega; try (complete (destruct n0; tcsp)).
...
Lemma add_choice_preserves_find_none {o} :
  forall name name' v (lib : @library o) n restr lib',
    add_choice name v lib = Some (n, restr, lib')
    -> find_cs lib name' = None
    -> find_cs lib' name' = None.
Proof.
  ...
  { destruct entry as [vals restr']; simpl in *; boolvar; subst; tcsp; ginv.
    ...
    inversion addc; subst; clear addc; simpl in *; boolvar; tcsp; eauto. }

  { remember (add_choice name v lib) as addc'; symmetry in Heqaddc'; destruct addc'; repnd; ginv.
    inversion addc; subst; clear addc; simpl in *; boolvar; tcsp; eauto. }
Qed.

Lemma implies_lib_extends_replace_cs_entry_find_none {o} :
  forall (lib1 lib2 : @library o) name e,
    is_nat_cs name
    -> find_cs lib1 name = None
    -> safe_library lib1
    -> safe_choice_sequence_entry name e
    -> lib_extends lib2 lib1
    -> lib_extends (replace_cs_entry lib2 name e) lib1.
Proof.
  introv isn fcsa sl safe ext.
  lib_ext_ind ext Case; introv; tcsp;
    try (complete (rewrite replace_cs_entry_if_find_none; auto)).

  { Case "lib_ext_trans".
    repeat (autodimp IHext2 hyp); eauto 3 with slow.
    eapply lib_extends_trans;[|eauto].
    remember (find_cs lib2 name) as fcsb; symmetry in Heqfcsb; destruct fcsb;
      try (complete (rewrite (replace_cs_entry_if_find_none lib2); auto; apply IHext1; eauto 3 with slow)).

    clear IHext1.
    apply implies_lib_extends_replace_cs_entry; eauto 3 with slow.
    eexists; dands; eauto.
    apply safe_library_find_cs_implies in Heqfcsb; eauto 3 with slow.
    eapply safe_choice_sequence_nat_implies_same_restrictions; eauto. }

  { Case "lib_ext_new_cs".
    boolvar; subst; tcsp; eauto.
    { apply implies_lib_extends_cons_left; simpl; eauto 3 with slow. }
    rewrite replace_cs_entry_if_find_none; auto.
    eapply lib_extends_new_cs; auto. }

  { Case "lib_ext_cs".
    dup addc as h.
    eapply add_choice_preserves_find_none in h; eauto.
    rewrite replace_cs_entry_if_find_none; eauto. }
Qed.

Lemma lib_extends_replace_from_trans_if_keep_only {o} :
  forall name (lib3 lib2 lib1 lib : @library o),
    is_nat_cs name
    -> safe_library lib1
    -> lib_extends lib2 (replace_from name lib lib1)
    -> lib_extends lib3 (keep_only name lib1)
    -> lib_extends lib (keep_only name lib1)
    -> lib_extends (replace_from name lib3 lib2) lib1.
Proof.
  introv isn safea exta extb extc.

  assert (safe_library lib3) as safe' by eauto 3 with slow.

  unfold replace_from in *.
  rewrite keep_only_equal in extb.
  rewrite keep_only_equal in extc.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa; destruct fcsa; simpl in *;
    remember (find_cs lib3 name) as fcsb; symmetry in Heqfcsb; destruct fcsb; simpl in *;
      remember (find_cs lib1 name) as fcsc; symmetry in Heqfcsc; destruct fcsc; simpl in *;
        eauto.

  { eapply find_cs_lib_extends_implies_cs_entry_extends in extb; eauto; simpl; boolvar; tcsp.
    eapply find_cs_lib_extends_implies_cs_entry_extends in extc; eauto; simpl; boolvar; tcsp.
    apply (implies_lib_extends_replace_cs_entry _ _ name c0) in exta; eauto 3 with slow;
      try (complete (exists c; rewrite find_cs_replace_cs_entry; boolvar; tcsp; allrw; dands; auto;
                            inversion extb; inversion extc; subst; simpl in *; ginv)).
    autorewrite with slow in *.
    eapply lib_extends_trans; eauto.
    eapply find_cs_implies_lib_extends_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }

  { rewrite replace_cs_entry_if_find_none in exta; auto.
    apply implies_lib_extends_replace_cs_entry_find_none; eauto 3 with slow. }

  { assert (entry_in_library (lib_cs name c0) [lib_cs name c0]) as i; simpl; tcsp.
    apply implies_lib_extends_ext in extb; apply extb in i.
    apply entry_in_library_extends_implies_entry_in_library in i; exrepnd.
    inversion i0; subst; clear i0;
      apply entry_in_library_implies_find_cs_some in i1; try rewrite i1 in *; ginv. }

  { rewrite replace_cs_entry_if_find_none in exta; auto. }

  { assert (entry_in_library (lib_cs name c0) [lib_cs name c0]) as i; simpl; tcsp.
    apply implies_lib_extends_ext in extc; apply extc in i.
    apply entry_in_library_extends_implies_entry_in_library in i; exrepnd.
    inversion i0; subst; clear i0;
      apply entry_in_library_implies_find_cs_some in i1; try rewrite i1 in *; ginv. }
...
Lemma replace_from_of_replace_from {o} :
  forall name (lib lib' : @library o),
    replace_from name lib (replace_from name lib lib')
    = replace_from name lib lib'.
Proof.
  introv; unfold replace_from.
  remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs; auto.
  autorewrite with slow; auto.
Qed.
Hint Rewrite @replace_from_of_replace_from : slow.

Lemma keep_only_replace_from {o} :
  forall name (lib1 lib2 : @library o),
    has_name name lib1
    -> has_name name lib2
    -> keep_only name (replace_from name lib2 lib1)
       = keep_only name lib2.
Proof.
  introv ha hb.
  repeat rewrite keep_only_equal.
  unfold replace_from, has_name in *; exrepnd; allrw.
  rewrite find_cs_replace_cs_entry; boolvar; tcsp; GC; allrw; auto.
Qed.
...
Lemma in_open_bar_ext_keep_only_pres {o} :
  forall name (lib : @library o)
    ...
    in_ext_ext lib (fun lib1 (y : lib_extends lib1 lib) =>
                      forall lib2
                             (x : lib_extends (replace_from name lib2 lib1) lib)
                             (z : lib_extends lib2 (keep_only name lib)),
                        F _ x -> G _ z)
    -> in_open_bar_ext lib F
    -> in_open_bar_ext (keep_only name lib) G.
Proof.
  introv isn safe hasn cond h.
  introv ext.
  ...
  { eapply lib_extends_replace_from_trans_if_keep_only; eauto. }

  assert (lib_extends (replace_from name lib'0 lib'') lib'') as xtc.
  { eapply lib_extends_replace_from_left; eauto. }

  pose proof (h1 _ xtc xtb) as h1; simpl in h1.

  pose proof (cond _ xtb lib'0) as cond; simpl in cond.
  autorewrite with slow in *.
  pose proof (cond xtb z) as cond; auto.
Qed.

Lemma if_entry_in_library_extends_lib_cs {o} :
  forall (lib : @library o) name entry,
    entry_in_library_extends (lib_cs name entry) lib
    -> exists e,
      find_cs lib name = Some e
      /\ cs_entry_extends e entry.
Proof.
  introv h.
  apply lib_cs_in_library_extends_implies in h; exrepnd.
  eexists; dands; eauto; simpl.
  destruct entry; inversion h0; simpl in *; subst; auto.
Qed.

Lemma cs_entry_extends_implies_has_compatible_restriction {o} :
  forall (lib : @library o) name c c',
    find_cs lib name = Some c'
    -> cs_entry_extends c c'
    -> has_compatible_restriction lib name c.
Proof.
  introv fcs ext.
  unfold has_compatible_restriction; allrw.
  eexists; dands; eauto.
  inversion ext; subst; simpl; auto.
Qed.
Hint Resolve cs_entry_extends_implies_has_compatible_restriction : slow.

Lemma implies_replace_from_name_ext {o} :
  forall {name} {lib lib' lib'' : @library o},
    is_nat_cs name
    -> safe_library lib
    -> lib_extends lib' (keep_only name lib)
    ...
    -> lib_extends (replace_from name lib' lib'') lib.
Proof.
  introv isn safe exta extb.
  assert (safe_library lib') as safe' by eauto 3 with slow.
  unfold replace_from.
  remember (find_cs lib' name) as fcsa; symmetry in Heqfcsa; destruct fcsa; auto.
  rewrite keep_only_equal in exta.
  remember (find_cs lib name) as fcsb; symmetry in Heqfcsb; destruct fcsb; auto.

  { assert (entry_in_library (lib_cs name c0) [lib_cs name c0]) as i; simpl; tcsp.
    apply implies_lib_extends_ext in exta; apply exta in i.
    apply if_entry_in_library_extends_lib_cs in i; exrepnd.
    rewrite i1 in *; ginv.
    eapply lib_extends_trans;[|eapply find_cs_implies_lib_extends_replace_cs_entry_ext_left];
      try exact i0; eauto; eauto 3 with slow.
    apply implies_lib_extends_replace_cs_entry; eauto 3 with slow. }

  { apply implies_lib_extends_replace_cs_entry_find_none; eauto 3 with slow. }
Qed.

Lemma implies_in_open_bar_ext_and {o} :
  forall (lib : @library o) (F G : forall x : library, lib_extends x lib -> [U]),
    in_open_bar_ext lib F
    -> in_open_bar_ext lib G
    -> in_open_bar_ext lib (fun lib' x => F _ x /\ G _ x).
Proof.
  introv h q.
  eapply in_open_bar_ext_comb;[|exact h]; clear h.
  eapply in_open_bar_ext_pres; try exact q; clear q.
  introv h q; tcsp.
Qed.

Definition keep_only_lib_per {o} {lib} name (safe : safe_library lib) (eqa : lib-per(lib,o)) : lib-per(keep_only name lib,o).
Proof.
...
Lemma lib_extends_replace_from_left2 {o} :
  forall {name} {lib0 lib lib' : @library o},
    has_name name lib'
    -> safe_library lib
    -> safe_library lib'
    -> lib_extends lib0 (replace_from name lib lib')
    -> lib_extends (replace_from name lib0 lib) lib.
Proof.
  introv hn safe safe' ext.
  eapply lib_extends_replace_from_left; eauto; eauto 3 with slow.
Qed.

Definition replace_from_lib_per
           {o} {lib} {name} {lib'}
           (hn    : has_name name lib')
           (safe  : safe_library lib)
           (safe' : safe_library lib')
           (eqa   : lib-per(lib,o)) : lib-per(replace_from name lib lib',o).
Proof.
  exists (fun lib0 (x : lib_extends lib0 (replace_from name lib lib')) =>
            eqa (replace_from name lib0 lib) (lib_extends_replace_from_left2 hn safe safe' x)).
  introv; apply lib_per_cond.
Defined.


Lemma has_name_replace_cs_entry {o} :
  forall name (lib : @library o) c,
    has_name name lib
    -> has_name name (replace_cs_entry lib name c).
Proof.
  introv hn.
  unfold has_name in *; exrepnd.
  rewrite find_cs_replace_cs_entry; boolvar; tcsp; GC.
  allrw; eauto.
Qed.
Hint Resolve has_name_replace_cs_entry : slow.

Lemma has_name_replace_form {o} :
  forall name (lib lib' : @library o),
    has_name name lib'
    -> has_name name (replace_from name lib lib').
Proof.
  introv hn; unfold replace_from.
  remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs; eauto 3 with slow.
Qed.
Hint Resolve has_name_replace_form : slow.

Lemma extends_replace_cs_entry_implies_cs_entry_extends {o} :
  forall name c c' (lib lib' : @library o),
    has_name name lib'
    -> lib_extends lib (replace_cs_entry lib' name c')
    -> find_cs lib name = Some c
    -> cs_entry_extends c c'.
Proof.
  introv hn ext fcs.
  eapply lib_extends_find_some_left_implies in fcs; eauto; repndors.

  { rewrite find_cs_replace_cs_entry in fcs; boolvar; subst; tcsp; GC.
    unfold has_name in *; exrepnd.
    rewrite hn0 in *; ginv. }

  exrepnd; subst; simpl in *.
  rewrite find_cs_replace_cs_entry in fcs0; boolvar; tcsp.
  unfold has_name in *; exrepnd.
  rewrite hn0 in *; GC; ginv.
  destruct e' as [vals' restr']; simpl in *; eauto.
Qed.
Hint Resolve extends_replace_cs_entry_implies_cs_entry_extends : slow.

Lemma has_compatible_restriction_replace_cs_entry {o} :
  forall name (lib : @library o) c c',
    has_name name lib
    -> is_nat_cs name
    -> safe_choice_sequence_entry name c
    -> safe_choice_sequence_entry name c'
    -> has_compatible_restriction (replace_cs_entry lib name c) name c'.
Proof.
  introv hn isn safea safeb.
  unfold has_compatible_restriction.
  rewrite find_cs_replace_cs_entry; boolvar; tcsp; GC.
  unfold has_name in *; exrepnd; allrw.
  eexists; dands; eauto.
  apply (safe_choice_sequence_nat_implies_same_restrictions name); auto.
Qed.
Hint Resolve has_compatible_restriction_replace_cs_entry : slow.

Lemma find_cs_some_implies_has_name {o} :
  forall name c (lib : @library o),
    find_cs lib name = Some c
    -> has_name name lib.
Proof.
  introv fcs; eexists; eauto.
Qed.
Hint Resolve find_cs_some_implies_has_name : slow.


Lemma implies_lib_extends_replace_from_trans1 {o} :
  forall name (lib lib' lib'' lib'0 lib'1 : @library o),
    is_nat_cs name
    -> has_name name lib'
    -> safe_library lib'1
    -> safe_library lib'0
    -> safe_library lib
    -> lib_extends lib'' (replace_from name lib'0 lib)
    -> lib_extends lib'1 (replace_from name lib lib')
    -> lib_extends (replace_from name lib'1 lib'') lib.
Proof.
  introv isn hn safea safeb safec exta extb.

  unfold replace_from in *.
  remember (find_cs lib'0 name) as fcsa; symmetry in Heqfcsa; destruct fcsa; simpl in *;
    remember (find_cs lib name) as fcsb; symmetry in Heqfcsb; destruct fcsb; simpl in *;
      remember (find_cs lib'1 name) as fcsc; symmetry in Heqfcsc; destruct fcsc; simpl in *;
        eauto.

  { apply (implies_lib_extends_replace_cs_entry _ _ name c1) in exta; eauto 3 with slow.
    { eapply lib_extends_trans;[eauto|]; autorewrite with slow.
      eapply find_cs_implies_lib_extends_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }
    apply has_compatible_restriction_replace_cs_entry; eauto 3 with slow. }

  { eapply lib_extends_find_none_left_implies in extb; eauto.
    rewrite find_cs_replace_cs_entry in extb; boolvar; tcsp; GC.
    unfold has_name in hn; exrepnd.
    rewrite hn0 in *; ginv. }
...
Lemma replace_from_replace_from {o} :
  forall name (lib lib' : @library o),
    has_name name lib
    -> has_name name lib'
    -> replace_from name (replace_from name lib lib') lib
       = lib.
Proof.
  introv hna hnb; unfold has_name in *; exrepnd.
  unfold replace_from.
...
Lemma replace_from_replace_from2 {o} :
  forall name (lib1 lib2 lib' : @library o),
    has_name name lib1
    -> has_name name lib'
    -> replace_from name (replace_from name lib1 lib') lib2
       = replace_from name lib1 lib2.
Proof.
  introv hna hnb; unfold has_name in *; exrepnd.
  unfold replace_from.
...
Lemma in_open_bar_ext_replace_from_pres {o} :
  forall name (lib lib' : @library o)
         (F : forall lib' (x : lib_extends lib' lib), Prop)
         (G : forall lib0 (x : lib_extends lib0 (replace_from name lib lib')), Prop)
         (isn   : is_nat_cs name)
         (safe  : safe_library lib)
         (safe' : safe_library lib')
         (hasn  : has_name name lib)
    ...
    in_ext_ext lib (fun lib1 (y : lib_extends lib1 lib) =>
                      forall lib2
                             (x : lib_extends (replace_from name lib2 lib1) lib)
                             (z : lib_extends lib2 (replace_from name lib lib')),
                        F _ x -> G _ z)
    -> in_open_bar_ext lib F
    -> in_open_bar_ext (replace_from name lib lib') G.
Proof.
  introv isn safe safe' hasn hasn' cond h.
  introv ext.
  dup ext as xta.
  apply lib_extends_replace_from_left in xta; eauto 3 with slow.
  pose proof (h _ xta) as h; simpl in h; exrepnd.
  assert (safe_library lib'') as safea by eauto 3 with slow.

  exists (replace_from name lib'' lib'0).
  exists (lib_extends_replace_from_left hasn safea y).
  introv xtb; introv.

  assert (lib_extends (replace_from name lib'1 lib'') lib'') as xtc.
  { eapply lib_extends_replace_from_left; try exact xtb; eauto 3 with slow. }

  assert (lib_extends (replace_from name lib'1 lib'') lib) as xtd by eauto 3 with slow.

  pose proof (h1 (replace_from name lib'1 lib'') xtc xtd) as h1; simpl in h1.
  eapply cond; try exact h1.
  eauto 3 with slow.
Qed.

Lemma lib_extends_replace_from_left_trans2 {o} :
  forall {name} {lib lib' lib0 lib1 : @library o},
    is_nat_cs name
    -> has_name name lib'
    -> safe_library lib
    -> safe_library lib'
    ...
    -> lib_extends (replace_from name lib0 lib1) lib.
Proof.
  introv isn hn safea safeb xta xtb.
  assert (safe_library lib0) as safec by eauto 3 with slow.
  unfold replace_from in *.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib0 name) as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; eauto.

  { apply (implies_lib_extends_replace_cs_entry _ _ name c0) in xtb; eauto 3 with slow.
    eapply lib_extends_trans;[eauto|].
    eapply find_cs_implies_lib_extends_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }
...
Definition replace_from_lib_per2
           {o} {lib} {name} {lib'}
           (isn   : is_nat_cs name)
           (hn    : has_name name lib')
           (safe  : safe_library lib)
           ...
           (eqa   : lib-per(lib,o)) : lib-per(replace_from name lib lib',o).
Proof.
  exists (fun lib0 (x : lib_extends lib0 (replace_from name lib lib')) a b =>
            exists (lib1 : library) (y : lib_extends lib1 lib),
              eqa (replace_from name lib0 lib1) (lib_extends_replace_from_left_trans2 isn hn safe safe' x y) a b).
  repeat introv; split; intro h; exrepnd; exists lib1 y0; eapply lib_per_cond; eauto.
Defined.

Lemma in_open_bar_ext_const_imp_implies {o} :
  forall (lib : @library o) (P : Type) (F : forall lib' (x : lib_extends lib' lib), Prop),
    P
    -> in_open_bar_ext lib (fun lib' x => P -> F lib' x)
    -> in_open_bar_ext lib (fun lib' x => F lib' x).
Proof.
  introv p ext.
  eapply in_open_bar_ext_pres; eauto; clear ext; introv h; tcsp.
Qed.

Lemma implies_lib_extends_replace_form {o} :
  forall name (lib lib1 lib2 : @library o),
    (forall c, find_cs lib name = Some c -> has_compatible_restriction lib2 name c)
    -> lib_extends lib1 lib2
    -> lib_extends (replace_from name lib lib1) (replace_from name lib lib2).
Proof.
  introv cond ext.
  unfold replace_from.
  remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs; auto.
...
Lemma implies_lib_extends_replace_from_flib {o} :
  forall {name} {lib0 lib : @library o} (Flib : FunLibExt lib) x,
    has_name name lib
    -> is_nat_cs name
    -> safe_library lib0
    -> lib_extends (replace_from name lib0 (Flib (replace_from name lib0 lib) x)) lib.
Proof.
  introv hn isn safe.
  remember (Flib (replace_from name lib0 lib) x) as lib''; clear Heqlib''.
  destruct lib'' as [lib'' ext']; simpl in *.
  apply (implies_lib_extends_replace_form name lib0) in ext'.
  { autorewrite with slow in *; eauto 3 with slow. }
  introv fcs.
  unfold replace_from; allrw.
  apply has_compatible_restriction_replace_cs_entry; eauto 3 with slow.
Qed.
...
Proof.
  introv isn mon conta contb imp safe hasn cl.
  close_cases (induction cl using @close_ind') Case; introv; subst.
  ...
  { Case "CL_bar".
    ...
    apply CL_bar.
    assert (safe_library lib) as safe' by eauto 3 with slow.
    repeat (apply in_open_bar_ext_const_imp_implies in reca; auto; simpl in * ).

    exists (keep_only_lib_per name safe' eqa); simpl.
    dands.

    { eapply in_open_bar_ext_keep_only_pres; try exact reca; auto;[]; simpl.
      introv ext h.
      repeat (autodimp h hyp); eauto 3 with slow;[].
      rewrite keep_only_replace_from in h; eauto 3 with slow;[].

  }
Qed.*)

Lemma lib_extends_replace_from_trans1 {o} :
  forall {name} {lib0 libf lib lib1 : @library o},
    has_name name lib1
    -> safe_library lib0
    -> lib_extends libf lib
    -> lib_extends lib0 (replace_from name libf lib1)
    -> lib_extends (replace_from name lib0 libf) lib.
Proof.
  introv hn safe xta xtb.
  eapply lib_extends_trans;[|eauto].
  eapply lib_extends_replace_from_left; eauto.
Qed.
...
Proof.
  exists (fun lib0 (x : lib_extends lib0 (replace_from name lib lib')) a b =>
            exists (lib1 : library)
                   (ext1 : lib_extends lib1 (replace_from name lib lib'))
                   (xta  : lib_extends (replace_from name lib1 lib) lib)
                   (ext2 : lib_extends lib0 (replace_from name (Flib (replace_from name lib1 lib) xta) lib1)),
              eqa (replace_from name lib0 (Flib (replace_from name lib1 lib) xta))
                  (lib_extends_replace_from_trans1
                     (lib_extends_preserves_has_name _ _ _ ext1 (has_name_replace_form _ _ _ hn'))
                     (lib_extends_preserves_safe _ _ x (implies_safe_library_replace_from _ _ _ safe safe'))
                     (lib_extends_trans (lib_extends_Flib _ _ Flib xta) xta)
                     ext2)
                  a b).
  repeat introv.
  ...
  forall name lib (u : cts(o)) (t1 t2 : @CTerm o) e,
    ...
    -> (forall lib t1 t2 e,
           ...
           -> forall lib',
               has_name name lib'
               -> safe_library lib'
               -> u (replace_from name lib lib') t1 t2 e)
    -> strong_safe_library lib
    -> has_name name lib
    -> close u lib t1 t2 e
    -> forall lib',
...
Proof.
  ...
  { Case "CL_bar".
    ...
    { introv xta.
      ...
      pose proof (reca0 _ xta0) as recb; simpl in recb; exrepnd.

      remember (Flib (replace_from name lib'0 lib) xta0) as lib''.
      assert (safe_library lib'') as safea by (subst; eauto 3 with slow).
      assert (lib_extends lib'' (replace_from name lib'0 lib)) as y by (subst; eauto 3 with slow).
      exists (replace_from name lib'' lib'0).
      exists (lib_extends_replace_from_left hasn safea y).
      introv xtb; introv.
      ...
      pose proof (recb (replace_from name lib'1 lib'') xtc xtd) as recb; simpl in recb.
      repeat (autodimp recb hyp); eauto 3 with slow;[].

      pose proof (recb lib'1) as q; repeat (autodimp q hyp); eauto 3 with slow.
      rewrite replace_from_replace_from in q; eauto 3 with slow;[].

      assert (lib_extends lib'' lib) as xte by eauto 3 with slow.

      eapply close_type_extensionality; try exact q; auto;[].

      introv; simpl; split; intro w; exrepnd.
      { subst; exists lib'0 xta xta0 xtb; eapply lib_per_cond; eauto. }
      remember (lib_extends_replace_from_trans1
                  (lib_extends_preserves_has_name name lib1 (replace_from name lib lib') ext1
                  ...
                  (lib_extends_trans (lib_extends_Flib lib (replace_from name lib1 lib) Flib xta1) xta1)
                  ext2) as xt; clear Heqxt.
      pose proof (reca0 _ xta1 (replace_from name lib'1 (Flib (replace_from name lib1 lib) xta1))) as w; simpl in w.
      autodimp w hyp.
      { eapply lib_extends_replace_from_trans1; try exact ext2; eauto 3 with slow. }
