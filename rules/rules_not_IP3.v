...
Lemma substc3_lsubstc_vars {o} :
  forall (n B A : NVar)
    (a : @CTerm o)
    (w : wf_term (mk_var A))
    (s : CSubstitution)
    (c : cover_vars_upto (mk_var A) (csub_filter (csub_filter (csub_filter s [A]) [B]) [n]) [n, B, A]),
    substc3 n B a A (lsubstc_vars (mk_var A) w (csub_filter (csub_filter (csub_filter s [A]) [B]) [n]) [n, B, A] c)
    = mk_cv [n, B] a.
Proof.
  introv.
  simpl.
  ...
  unfold subst; unfold csubst; simpl.
  erewrite lsubst_nest_progs_swap_single; eauto 3 with slow;
    [|allrw @dom_csub_eq;
      allrw @dom_csub_csub_filter;
      apply disjoint_remove_nvars2;
      apply disjoint_remove_nvars2;
      eauto 3 with slow];[].

  repeat (rw singleton_as_snoc).
  erewrite cl_lsubst_var_snoc_in; simpl; eauto 3 with slow;
    try (complete (intro x; destruct x)).
  eapply (lsubst_cterm (mk_ct a p)).
Qed.
...
Definition IP {o} (A B n : NVar) (i : nat) : @NTerm o :=
  mk_all
    ...
    (mk_all
       ...
       B
       (mk_fun
          (mk_fun (mk_squash (mk_var A)) (mk_squash (mk_exists mk_tnat n (mk_apply (mk_var B) (mk_var n)))))
          (mk_squash (mk_exists mk_tnat n (mk_fun (mk_squash (mk_var A)) (mk_apply (mk_var B) (mk_var n))))))).
...
Check lsubstc_vars.
...

  {
    ...
    apply tequality_fun; dands; eauto 3 with slow;[|];
      try (complete (apply tequality_mkc_squash; eauto 3 with slow));[].
...

  {
    ...
    repeat lsubstc_vars_as_mkcv.
    autorewrite with slow.
    apply implies_cequivc_mkc_squash.
  ...
  pose proof (q2 _ blib2 _ (lib_extends_refl lib')) as q2.
  cbv beta in q2.
  ...
  unfold inhabited_type in q2; exrepnd.

  eapply alphaeqc_preserving_equality in q0;
    [|apply substc_alphaeqcv;
      apply substc2_product;tcsp].

  rewrite mkcv_product_substc in q0; tcsp;[].
  apply inhabited_type_if_equality in q0.
  apply inhabited_product in q0; repnd.
  clear q1 q2.
  exrepnd.
  rename q1 into q.
  autorewrite with slow in q.
                  ...
                  lib') as blib.
  {
    ...
    pose proof (entry_extends_cs_zeros_implies lib name name0 m0 restr0 lib' entry') as q.
    repeat (autodimp q hyp).
    exrepnd; subst.
    exists n0 restr1 lib'; dands; auto.
  }

  clear m0 blib1.
  exrepnd.
  assert (safe_library lib') as safe' by eauto 3 with slow.
  pose proof (q3 _ blib2 _ (lib_extends_refl lib')) as q3.
  cbv beta in q3.
  exrepnd; spcast.
  ...
  pose proof (extend_bool_choice_sequence_zero lib name m0 restr1 (S k)) as q.
  repeat (autodimp q hyp);[].
  exrepnd.

  assert (k < m0 + S k) as ltk by lia.
  remember (m0 + S k) as m'; clear Heqm'.
  assert (safe_library lib') as safe' by eauto 3 with slow.
  eapply lib_extends_preserves_computes_to_valc in q2;[|eauto];[].

  pose proof (entry_cs_zeros_implies_exists_extension_with_one lib' name m' restr1) as q.
  repeat (autodimp q hyp);[].
  exrepnd.
  assert (safe_library lib'0) as safe'' by eauto 3 with slow.
  eapply lib_extends_preserves_computes_to_valc in q2;[|eauto];[].
...
(*  pose proof (q0 (mkc_pair (mkc_nat m) mkc_axiom) (mkc_pair (mkc_nat m) mkc_axiom)) as q0.*)
  pose proof (q0 mkc_axiom mkc_axiom) as q0.
  autodimp q0 hyp;[|].

  {
    ...
    exists (@mkc_pair o (mkc_nat m) mkc_axiom).
