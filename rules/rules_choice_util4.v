...
Lemma remove_nvars_trivial2 :
  forall x y z w u v b,
    remove_nvars [x, y, z, w, u, b, v] [b] = [].
Proof.
  introv.
  unfold remove_nvars; simpl; repeat (boolvar; simpl; tcsp).
Qed.
Hint Rewrite remove_nvars_trivial2 : slow.
...
Proof.
  ...
  destruct nfo as [n1 n2]; simpl; boolvar; subst; auto;
    try apply swap_cs_inj in e; subst; auto; tcsp.
Qed.
    ...
    { dopid_noncan ncan SSCase; simpl in *;
      ...
      { SSCase "NSwapCs1".
        dterms w; ginv; csunf; simpl in *; repndors; repnd; subst; simpl in *;
          unfold apply_bterm; autorewrite with slow; simpl; eauto.
        { apply approx_star_swap.compute_step_swap_cs1_aux_success_implies in comp; exrepnd; subst; simpl in *; auto. }
...
Lemma swap_computes_to_valc {o} :
  forall {sw} {lib : @plibrary o} {a b : @CTerm o},
    computes_to_valc lib a b
    -> computes_to_valc (swap_cs_plib sw lib) (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv comp; destruct_cterms; unfold computes_to_valc in *; simpl in *.
  apply swap_computes_to_value; auto.
Qed.
...
Proof.
  introv comp; introv.
  spcast; apply swap_computes_to_valc; auto.
Qed.

Lemma implies_ciscvalue_swap_cs_cterm {o} :
  forall sw (t : @CTerm o),
    ciscvalue t
    -> ciscvalue (swap_cs_cterm sw t).
Proof.
  introv i; destruct_cterms; unfold ciscvalue in *; simpl in *; spcast.
  unfold iscvalue in *; simpl in *; eauto 3 with slow.
Qed.
Hint Resolve implies_ciscvalue_swap_cs_cterm : slow.

Lemma nt_wf_swap_cs_term_implies {o} :
  forall sw (b : @NTerm o),
    nt_wf (swap_cs_term sw b) -> nt_wf b.
Proof.
  nterm_ind b as [v|op bs ind] Case; introv wf; auto.
  simpl in *.
  allrw @nt_wf_oterm_iff; repnd.
  rewrite map_map in wf0; unfold compose in *; autorewrite with slow in *.
  dands.
  { rewrite <- wf0; apply eq_maps; introv i; destruct x; simpl in *; tcsp. }
  introv i.
  pose proof (wf (swap_cs_bterm sw b)) as wf; destruct b; simpl in *.
  autodimp wf hyp.
  { apply in_map_iff; eexists; dands; eauto. }
  inversion wf; subst; constructor; eauto.
Qed.

Lemma isprogram_swap_cs_term_implies {o} :
  forall sw (b : @NTerm o),
    isprogram (swap_cs_term sw b) -> isprogram b.
Proof.
  introv isp.
  destruct isp as [c wf]; constructor; eauto 3 with slow.
  { unfold closed in *; autorewrite with slow in *; auto. }
  eapply nt_wf_swap_cs_term_implies; eauto.
Qed.

Lemma respects_alpha_sum {o} :
  forall (a b : @NTerm o -> @NTerm o -> Type),
    respects_alpha a
    -> respects_alpha b
    -> respects_alpha (a \2/ b).
Proof.
  introv respa respb.
  unfold respects_alpha in *; repnd; dands; introv aeq h; repndors; tcsp;
    try (complete (left; eauto));
    try (complete (right; eauto)).
Qed.
Hint Resolve respects_alpha_sum : slow.

Lemma respects_alpha_bot2 {o} : respects_alpha (@bot2 o).
Proof.
  introv; split; introv aeq h; tcsp.
Qed.
Hint Resolve respects_alpha_bot2 : slow.

Lemma implies_wf_swap_cs_sub {o} :
  forall sw (sub : @Sub o),
    wf_sub sub
...
Qed.
Hint Resolve implies_wf_swap_cs_sub : slow.

Lemma swap_cs_sub_twice {o} :
  forall sw (sub : @Sub o),
    swap_cs_sub sw (swap_cs_sub sw sub) = sub.
Proof.
  induction sub; introv; repnd; simpl; auto; autorewrite with slow; allrw; auto.
Qed.
Hint Rewrite @swap_cs_sub_twice : slow.

Lemma implies_isprogram_lsubst_swap_cs_sub {o} :
  forall sw (t : @NTerm o) sub,
    isprogram (lsubst (swap_cs_term sw t) sub)
    -> isprogram (lsubst t (swap_cs_sub sw sub)).
Proof.
  introv isp.
  pose proof (swap_cs_sub_twice sw sub) as ea.
  rewrite <- ea in isp.
  rewrite <- swap_cs_term_lsubst in isp.
  apply isprogram_swap_cs_term_implies in isp; auto.
Qed.
Hint Resolve implies_isprogram_lsubst_swap_cs_sub : slow.

Lemma implies_alpha_eq_bterm_swap_cs_bterm {o} :
  forall sw (b1 b2 : @BTerm o),
    alpha_eq_bterm b1 b2
    -> alpha_eq_bterm (swap_cs_bterm sw b1) (swap_cs_bterm sw b2).
Proof.
  introv aeq.
  destruct b1, b2; simpl in *.
  inversion aeq as [? ? ? ? ? disj lena lenb norep aeq']; subst.
  eapply al_bterm; autorewrite with slow; eauto.
  repeat rewrite lsubst_swap_cs_term_var_ren; eauto 3 with slow.
Qed.
...
Lemma swap_cs_bterm_idem {o} :
  forall (r : cs_swap)
         (b : @BTerm o),
    swap_cs_bterm r (swap_cs_bterm r b) = b.
Proof.
...
Proof.
  ...
  constructor.
  unfold close_comput; dands; auto; eauto 2 with slow.
  ...
  { introv compa.
    ...
    eexists; dands; eauto.
    unfold lblift in *; repnd; dands; try rewrite map_length in *; auto.
    introv len.
    applydup compa0 in len; clear compa0.
    unfold blift in *; exrepnd.
    exists lv (swap_cs_term sw nt1) (swap_cs_term sw nt2); dands; auto.
    { unfold olift in *; repnd; dands; auto; eauto 2 with slow.
      introv wf ispa ispb.
      pose proof (len0 (swap_cs_sub sw sub)) as len0.
      repeat (autodimp len0 hyp); eauto 3 with slow.
      repndors; tcsp; left.
      pose proof (swap_cs_sub_twice sw sub) as ea.
      rewrite <- ea; repeat rewrite <- swap_cs_term_lsubst.
      apply ind; auto. }
    { rewrite selectbt_map in len2; auto.
      apply (implies_alpha_eq_bterm_swap_cs_bterm sw) in len2.
      rewrite swap_cs_bterm_idem in len2.
      simpl in *; auto. }
    { rewrite selectbt_map; auto; try rewrite <- compa2; auto.
      apply (implies_alpha_eq_bterm_swap_cs_bterm sw) in len1.
      simpl in *; auto. } }

  { introv compa.
    apply (@swap_reduces_to o sw) in compa; autorewrite with slow in compa.
    simpl in compa; fold_terms.
    apply ce in compa; exrepnd; repndors; tcsp;
      try (complete (inversion compa2));
      try (complete (inversion compa1)).
    apply (@swap_reduces_to o sw) in compa0; simpl in *; fold_terms.
    eexists; eexists; dands; try exact compa0; left.
    { rewrite <- (swap_cs_term_idem sw a0); apply ind; auto. }
    { rewrite <- (swap_cs_term_idem sw e); apply ind; auto. } }
Qed.

Lemma swap_approxc {o} :
  forall sw lib (a b : @CTerm o),
    approxc lib a b
    -> approxc (swap_cs_plib sw lib) (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv apx; destruct_cterms; unfold approxc in *; simpl in *.
  apply swap_approx; auto.
Qed.

Lemma swap_cequivc {o} :
  forall sw lib (a b : @CTerm o),
    cequivc lib a b
    -> cequivc (swap_cs_plib sw lib) (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv h; destruct h as [h q]; split; apply swap_approxc; auto.
Qed.
...
Proof.
  introv sane comp ext.
  apply (swap_lib_extends sane) in ext; autorewrite with slow in ext.
  apply comp in ext; exrepnd.
  apply (swap_ccomputes_to_valc sw) in ext1.
  simpl in ext1; autorewrite with slow in ext1.
  eexists; dands; eauto; eauto 3 with slow.
  apply (swap_ccequivc sw) in ext0; simpl in ext0; autorewrite with slow in ext0; auto.
Qed.
...
Lemma swap_are_same_qnats {o} :
  forall sw lib c (a b : @CTerm o),
    sane_swapping sw
    -> are_same_qnats lib c a b
    -> are_same_qnats (swap_cs_lib sw lib) c (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv sane ceq ext.
...
Qed.
Hint Resolve swap_are_same_qnats : slow.

Lemma swap_sat_qnat_cond {o} :
  forall sw lib c (a : @CTerm o),
    sane_swapping sw
    -> sat_qnat_cond lib c a
    -> sat_qnat_cond (swap_cs_lib sw lib) c (swap_cs_cterm sw a).
Proof.
  introv sane sat h exta extb compa compb.
  applydup @lib_extends_swap_right_to_left in exta; auto.
  applydup (@swap_lib_extends o sw) in extb; auto.
  apply (@swap_computes_to_valc o sw) in compa; auto.
  apply (@swap_computes_to_valc o sw) in compb; auto.
  autorewrite with slow in *.
  eapply sat; eauto.
Qed.
Hint Resolve swap_sat_qnat_cond : slow.

Lemma swap_equality_of_qnat {o} :
  forall sw lib c (a b : @CTerm o),
    sane_swapping sw
    -> equality_of_qnat lib c a b
    -> equality_of_qnat (swap_cs_lib sw lib) c (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv sane ceq.
...
Lemma swap_cs_cterm_mkc_qnat {o} : forall sw c, @swap_cs_cterm o sw (mkc_qnat c) = mkc_qnat c.
Proof.
  introv; apply cterm_eq; simpl; auto.
Qed.
...
Lemma swap_cs_cterm_mkc_qlt {o} :
  forall sw a b, @swap_cs_cterm o sw (mkc_qlt a b) = mkc_qlt (swap_cs_cterm sw a) (swap_cs_cterm sw b).
Proof.
  introv; destruct_cterms; apply cterm_eq; simpl; auto.
Qed.
Hint Rewrite @swap_cs_cterm_mkc_qlt : slow.
...
Proof.
  ...
  { apply h in q; clear h.
    ...
    apply (swap_ccomputes_to_valc_ext sw) in q1; auto.
    apply (swap_ccomputes_to_valc_ext sw) in q0; auto.
    autorewrite with slow in *.
    eexists; dands; eauto. }
  { apply h; clear h.
...
Lemma swap_equality_of_qnat_bar {o} :
  forall sw lib (eq : per(o)) c,
    ...
    -> (swap_cs_per sw eq) <=2=> (equality_of_qnat_bar (swap_cs_lib sw lib) c).
Proof.
  introv sane h; introv; unfold swap_cs_per; split; intro q.
  { apply h in q; clear h.
    eapply in_open_bar_swap_cs_lib_pres; try exact q; clear q; auto.
    introv ext q.
    apply (swap_equality_of_qnat sw) in q; auto; autorewrite with slow in *; auto. }
  { apply h; clear h.
    eapply in_open_bar_swap_cs_lib_pres2; try exact q; clear q; auto.
    introv ext q.
    apply (swap_equality_of_qnat sw) in q; auto; autorewrite with slow in *; auto. }
...
Proof.
  ...
  { apply h in q; clear h.
    ...
    apply (swap_ccomputes_to_valc_ext sw) in q0; auto.
    apply (swap_ccomputes_to_valc_ext sw) in q2; auto.
    autorewrite with slow in *.
...
Proof.
  ...
  { apply h in q; clear h.
    ...
    apply (swap_ccequivc sw) in q0; auto.
    apply (swap_ccequivc sw) in q2; auto.
...
Lemma swap_per_qlt_eq_bar {o} :
  forall sw (sane : sane_swapping sw) lib (eq : per(o)) (a b : @CTerm o),
    (eq <=2=> (equality_of_qlt_bar lib a b))
    -> (swap_cs_per sw eq) <=2=> (equality_of_qlt_bar (swap_cs_lib sw lib) (swap_cs_cterm sw a) (swap_cs_cterm sw b)).
Proof.
  introv sane h; introv; unfold swap_cs_per; split; intro q.
  { apply h in q; clear h.
    eapply in_open_bar_swap_cs_lib_pres; try exact q; clear q; auto.
    introv ext q; simpl in *.
    unfold equality_of_qlt in *; repnd.
    introv xt.
    remember (lib_extends_swap_cs_lib_twice_implies (swap_lib_extends sane xt)) as xe; clear Heqxe.
    apply q in xe; clear q; exrepnd.
    apply (swap_ccomputes_to_valc sw) in xe0; auto.
    apply (swap_ccomputes_to_valc sw) in xe2; auto.
    autorewrite with slow in *; eauto. }
  { apply h; clear h.
    eapply in_open_bar_swap_cs_lib_pres2; try exact q; clear q; auto.
    introv ext q; simpl in *.
    unfold equality_of_qlt in *; repnd.
    introv xt.
    apply (swap_lib_extends sane) in xt.
    apply q in xt; exrepnd.
    apply (swap_ccomputes_to_valc sw) in xt0; auto.
    apply (swap_ccomputes_to_valc sw) in xt2; auto.
    autorewrite with slow in *; eauto. }
Qed.
...
Proof.
  ...
  apply (swap_ccomputes_to_valc_ext sw) in u0; auto.
  apply (swap_ccomputes_to_valc_ext sw) in u1; auto.
  autorewrite with slow in *; dands; auto.
...
Proof.
  ...
  split; intro h; allrw app_eq_nil_iff; repnd; dands;
    try (complete (destruct op; simpl in *; tcsp;
                   try destruct c; try destruct n; try destruct s; simpl in *; tcsp)).
...
Ltac one_move2ext ext :=
  ...
  | lib_extends ?lib1 ?lib2 =>
    ...
    | [ H : lib_cond_sat_def ?lib3 |- _ ] =>
      let x := fresh "x" in
      assert (lib3 = lib2) as x by reflexivity; clear x;
      let h := fresh H in
      assert (lib_cond_sat_def lib1) as h by eauto 3 with slow;
      clear H; rename h into H
    end
  end.
...
Lemma in_lib_swap_two_names_implies {o} :
  forall n name c name' c' (lib : @plibrary o),
    in_lib n (swap_two_names name c name' c' lib)
    -> exists e,
      List.In e lib
      /\ same_entry_name n (entry2name (swap_two_names_entry name c name' c' e)).
Proof.
  induction lib; introv h; simpl in *.
  { unfold in_lib in *; simpl in *; exrepnd; tcsp. }
  apply in_lib_cons in h; simpl in *; repndors.
  { destruct a; simpl in *; boolvar; subst; simpl in *; tcsp;
      try (complete (eexists; dands; eauto;
                     simpl; boolvar; subst; simpl in *; tcsp;
                     destruct n; simpl in *; tcsp; subst; auto)). }
  { apply IHlib in h; exrepnd.
    eexists; dands; eauto. }
Qed.

Lemma in_swap_two_names_implies {o} :
  forall e name c name' c' (lib : @plibrary o),
    List.In e (swap_two_names name c name' c' lib)
    -> exists (e' : library_entry),
      List.In e' lib
      /\ e = swap_two_names_entry name c name' c' e'.
Proof.
  induction lib; introv h; simpl in *; tcsp.
  repndors; subst.
  { eexists; dands; eauto. }
...
Lemma lin_swap_two_names_implies {o} :
  forall e name c name' c' (lib : @plibrary o),
    LIn e (swap_two_names name c name' c' lib)
    -> {e' : library_entry &
      List.In e' lib
      /\ e = swap_two_names_entry name c name' c' e'}.
Proof.
...
Lemma implies_in_swap_two_names {o} :
  forall e name c name' c' (lib : @plibrary o),
    LIn e lib
    -> LIn (swap_two_names_entry name c name' c' e) (swap_two_names name c name' c' lib).
Proof.
  induction lib; introv h; simpl in *; tcsp.
  repndors; subst; tcsp.
Qed.

Lemma in_lib_implies_in_lib_names {o} :
  forall name (lib : @plibrary o),
    in_lib (entry_name_cs name) lib
    -> LIn (entry_name_cs name) (lib_names lib).
Proof.
  introv i; unfold in_lib in *; exrepnd.
  apply LIn_iff_In; eauto 3 with slow; exrepnd.
  apply List.in_map_iff; eexists; dands; eauto.
  destruct e; simpl in *; subst; tcsp.
Qed.
Hint Resolve in_lib_implies_in_lib_names : slow.

Lemma implies_in_lib_names_swap_two_names {o} :
  forall name name' c c' (lib : @plibrary o),
    LIn (entry_name_cs name) (lib_names lib)
    -> LIn (entry_name_cs name') (lib_names (swap_two_names name c name' c' lib)).
Proof.
  induction lib; introv h; simpl in *; tcsp.
  repndors; tcsp.
  destruct a; simpl in *; ginv; boolvar; subst; GC; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names : slow.

Lemma implies_in_lib_names_swap_two_names2 {o} :
  forall name name' c c' (lib : @plibrary o),
    LIn (entry_name_cs name') (lib_names lib)
    -> LIn (entry_name_cs name) (lib_names (swap_two_names name c name' c' lib)).
Proof.
  ...
  destruct a; simpl in *; ginv; boolvar; subst; GC; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names2 : slow.

Lemma implies_in_lib_names_swap_two_names3 {o} :
  forall name name' c c' (lib : @plibrary o),
    LIn (entry_name_cs name') (lib_names (swap_two_names name c name' c' lib))
    -> LIn (entry_name_cs name) (lib_names lib).
Proof.
  ...
  simpl in *; ginv; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names3 : slow.

Lemma implies_in_lib_names_swap_two_names4 {o} :
  forall name name' c c' (lib : @plibrary o),
    LIn (entry_name_cs name) (lib_names (swap_two_names name c name' c' lib))
    -> LIn (entry_name_cs name') (lib_names lib).
Proof.
  ...
  simpl in *; ginv; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names4 : slow.

Lemma implies_in_lib_names_swap_two_names5 {o} :
  forall name0 name name' c c' (lib : @plibrary o),
    name0 <> name
    -> name0 <> name'
    -> LIn (entry_name_cs name0) (lib_names lib)
    -> LIn (entry_name_cs name0) (lib_names (swap_two_names name c name' c' lib)).
Proof.
  induction lib; introv da db h; simpl in *; tcsp.
  repndors; tcsp.
  destruct a; simpl in *; ginv; boolvar; subst; GC; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names5 : slow.

Lemma implies_in_lib_names_swap_two_names6 {o} :
  forall name0 name name' c c' (lib : @plibrary o),
    name0 <> name
    -> name0 <> name'
    -> LIn (entry_name_cs name0) (lib_names (swap_two_names name c name' c' lib))
...
Proof.
  induction lib; introv da db h; simpl in *; tcsp.
  repndors; tcsp.
  destruct a; simpl in *; ginv; boolvar; subst; GC; tcsp; simpl in *; ginv; tcsp.
Qed.
Hint Resolve implies_in_lib_names_swap_two_names6 : slow.

Lemma implies_eqset_lib_names_swap_two_names {o} :
  forall name c name' c' (lib : @plibrary o),
    in_lib (entry_name_cs name) lib
    -> in_lib (entry_name_cs name') lib
    -> eqset (lib_names (swap_two_names name c name' c' lib)) (lib_names lib).
Proof.
  introv ia ib; introv; split; intro h.
  { apply in_map_iff in h; exrepnd; subst; simpl in *.
    apply lin_swap_two_names_implies in h1; exrepnd; subst.
    destruct e'; simpl in *; boolvar; subst; simpl in *; tcsp; eauto 3 with slow.
    { apply LIn_iff_In; eauto 3 with slow; exrepnd.
      apply List.in_map_iff; eexists; dands; eauto; tcsp. }
    { apply LIn_iff_In; eauto 3 with slow; exrepnd.
      apply List.in_map_iff; eexists; dands; eauto; tcsp. } }
  { apply in_map_iff in h; exrepnd; subst.
    apply (implies_in_swap_two_names _ name c name' c') in h1.
    destruct a; simpl in *; boolvar; subst; tcsp; eauto 3 with slow;
      apply in_map_iff; eexists; dands; eauto. }
Qed.

Lemma in_lib_names_implies_in_lib {o} :
  forall name (lib : @plibrary o),
    LIn (entry_name_cs name) (lib_names lib)
    -> in_lib (entry_name_cs name) lib.
Proof.
  ...
  apply LIn_implies_In in h1.
  eexists; dands; eauto.
  rewrite <- h0; simpl; auto.
Qed.
Hint Resolve in_lib_names_implies_in_lib : slow.

Lemma in_lib_abs_swap_two_names {o} :
  forall op name c name' c' (lib : @plibrary o),
    in_lib (entry_name_abs op) (swap_two_names name c name' c' lib)
    <-> in_lib (entry_name_abs op) lib.
Proof.
  induction lib; introv; split; simpl in *; introv h; tcsp.
  { allrw @in_lib_cons; repndors; tcsp.
    { destruct a; simpl in *; tcsp; boolvar; subst; simpl in *; tcsp. }
    { apply IHlib in h; tcsp. } }
  { allrw @in_lib_cons; repndors; tcsp.
    { destruct a; simpl in *; tcsp; boolvar; subst; simpl in *; tcsp. }
    { apply IHlib in h; tcsp. } }
Qed.
Hint Rewrite @in_lib_abs_swap_two_names : slow.

Lemma implies_lib_nodup_swap_two_names {o} :
  forall name c name' c' (lib : @plibrary o),
    lib_nodup lib
    -> lib_nodup (swap_two_names name c name' c' lib).
Proof.
  induction lib; introv nodup; simpl in *; tcsp; repnd.
  dands; tcsp.
  destruct a; simpl in *; simpl; boolvar; subst; simpl in *; tcsp.
  { intro xx; apply in_lib_implies_in_lib_names in xx.
    apply implies_in_lib_names_swap_two_names3 in xx; destruct nodup0; eauto 3 with slow. }
  { intro xx; apply in_lib_implies_in_lib_names in xx.
    apply implies_in_lib_names_swap_two_names4 in xx; destruct nodup0; eauto 3 with slow. }
  { intro xx; apply in_lib_implies_in_lib_names in xx.
    apply implies_in_lib_names_swap_two_names6 in xx; destruct nodup0; eauto 3 with slow. }
  { intro xx; autorewrite with slow in xx; tcsp. }
Qed.
Hint Resolve implies_lib_nodup_swap_two_names : slow.
...
Proof.
  destruct lib as [lib cond]; simpl; introv h.
  unfold pre_swap_names.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; simpl in *; tcsp; eauto 3 with slow.
Qed.
...
Lemma implies_sat_lib_cond_swap_names {o} :
  forall name name' (lib : @library o),
    lib_nodup lib
    -> sat_lib_cond lib
    -> sat_lib_cond (swap_names name name' lib).
Proof.
  destruct lib as [lib cond]; simpl; introv nodup h.
  unfold sat_lib_cond, swap_names in *; simpl in *.
  introv i.
  unfold pre_swap_names in *.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; simpl in *; tcsp; eauto 3 with slow.
  apply entry_in_library_implies_in in i.
  apply in_swap_two_names_implies in i; exrepnd; subst.
  apply in_implies_entry_in_library_if_nodup in i1; auto.
  apply h in i1.
  apply find_cs_some_implies_entry_in_library in Heqfcsa.
  apply find_cs_some_implies_entry_in_library in Heqfcsb.
  apply h in Heqfcsa; apply h in Heqfcsb.
  clear h.
  unfold lib_cond_sat_entry in *.
  destruct e'; simpl in *; tcsp.
  boolvar; subst; tcsp.
Qed.
Hint Resolve implies_sat_lib_cond_swap_names : slow.

Lemma implies_strong_safe_library_swap_names {o} :
  forall name name' (lib : @library o),
    strong_safe_library lib
    -> strong_safe_library (swap_names name name' lib).
Proof.
  destruct lib as [lib cond]; simpl; introv safe i.
  unfold pre_swap_names in *.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; simpl in *; tcsp; eauto 3 with slow.
  ...
  apply safe in i1.
  apply find_cs_some_implies_list_in in Heqfcsa; apply safe in Heqfcsa.
  apply find_cs_some_implies_list_in in Heqfcsb; apply safe in Heqfcsb.
  destruct e'; simpl in *; tcsp.
  boolvar; subst; tcsp.
Qed.
Hint Resolve implies_strong_safe_library_swap_names : slow.

Lemma swap_sames_same_not_in {o} :
  forall name name' (lib : @library o),
    (~in_lib (entry_name_cs name) lib \/ ~in_lib (entry_name_cs name') lib)
    -> swap_names name name' lib = lib.
Proof.
  introv h.
  destruct lib as [lib cond]; simpl in *.
  unfold swap_names; simpl; f_equal.
  unfold pre_swap_names.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; simpl in *; auto.
  apply find_cs_some_implies_in_lib in Heqfcsa.
  apply find_cs_some_implies_in_lib in Heqfcsb; tcsp.
Qed.

Lemma swap_names_same {o} :
  forall name (lib : @library o),
    lib_nodup lib
    -> swap_names name name lib = lib.
Proof.
  ...
  unfold pre_swap_names.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  destruct fcsa; simpl in *; auto.
  rewrite swap_two_names_same; auto.
Qed.

Hint Rewrite length_app : slow.

Lemma swapped_css_swap_two_names {o} :
  forall name name' c c' (lib : @plibrary o),
    name <> name'
    -> find_cs lib name = Some c
    -> find_cs lib name' = Some c'
    -> lib_nodup lib
    -> swapped_css (name, name') (swap_two_names name c name' c' lib) lib.
Proof.
  introv dn ia ib nodup.
  applydup @find_cs_some_implies_entry_in_library in ia.
  applydup @find_cs_some_implies_entry_in_library in ib.
  apply entry_in_library_split in ia0.
  apply entry_in_library_split in ib0.
  exrepnd; subst; simpl in *.
  apply app_eq_app_implies in ib1.
  boolvar; exrepnd; subst; simpl in *.

  { destruct l0; simpl in *; ginv; repeat (autorewrite with slow in *; simpl in *; repnd;tcsp).
    rewrite in_lib_app in nodup5; apply not_or in nodup5; tcsp;repnd.
    rewrite in_lib_cons in nodup5; apply not_or in nodup5; repnd.
    rewrite swap_two_names_split_fst_eq; tcsp.
    rewrite swap_two_names_split_snd_eq; tcsp.
    rewrite swap_two_names_split_eq_not_in; tcsp. }

  { destruct l; simpl in *; ginv; repeat (autorewrite with slow in *; simpl in *; repnd;tcsp).
    rewrite in_lib_app in nodup; apply not_or in nodup; tcsp;repnd.
    rewrite in_lib_cons in nodup; apply not_or in nodup; repnd.
    rewrite <- app_assoc; simpl.
    rewrite swap_two_names_split_snd_eq; tcsp.
    rewrite swap_two_names_split_fst_eq; tcsp.
    rewrite swap_two_names_split_eq_not_in; tcsp. }
Qed.
Hint Resolve swapped_css_swap_two_names : slow.

Lemma swapped_css_libs_swap_names {o} :
  forall name name' (lib : @library o),
    name <> name'
    -> in_lib (entry_name_cs name) lib
    -> in_lib (entry_name_cs name') lib
    -> lib_nodup lib
    -> swapped_css_libs name name' (swap_names name name' lib) lib.
Proof.
  introv dn ia ib nodup.
  destruct lib as [lib cond]; simpl in *.
  unfold swapped_css_libs, swap_names, same_conds; simpl; dands; auto.
  unfold pre_swap_names.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb;
    try apply find_cs_non_implies_not_in_lib2 in Heqfcsa;
    try apply find_cs_non_implies_not_in_lib2 in Heqfcsb;
    tcsp; eauto 3 with slow.
Qed.
Hint Resolve swapped_css_libs_swap_names : slow.

Lemma implies_lib_extends_swap_names_left {o} :
  forall name name' (lib1 lib0 : @library o),
    lib_nodup lib0
    -> sat_lib_cond lib0
    -> strong_safe_library lib0
    -> name <> name'
    -> in_lib (entry_name_cs name) lib0
    -> in_lib (entry_name_cs name') lib0
    -> lib_extends lib1 (swap_names name name' lib0)
    -> lib_extends (swap_names name name' lib1) lib0.
Proof.
  introv nodup sat safe dn ia ib ext.
  eapply lib_extends_preserves_perm_libs; try exact ext; eauto 3 with slow.
Qed.

Lemma implies_in_lib_swap_names1 {o} :
  forall name name' (lib : @library o),
    in_lib (entry_name_cs name) lib
    -> in_lib (entry_name_cs name') lib
    -> in_lib (entry_name_cs name) (swap_names name name' lib).
Proof.
  introv i j.
  destruct lib as [lib cond]; simpl in *.
  unfold pre_swap_names.
  ...
  apply in_lib_names_implies_in_lib.
  apply implies_eqset_lib_names_swap_two_names; auto; eauto 3 with slow.
Qed.
Hint Resolve implies_in_lib_swap_names1 : slow.

Lemma implies_in_lib_swap_names2 {o} :
  forall name name' (lib : @library o),
    in_lib (entry_name_cs name) lib
    -> in_lib (entry_name_cs name') lib
    -> in_lib (entry_name_cs name') (swap_names name name' lib).
Proof.
  ...
  apply implies_eqset_lib_names_swap_two_names; auto; eauto 3 with slow.
Qed.
Hint Resolve implies_in_lib_swap_names2 : slow.

Lemma lib_extends_preserves_in_lib {o} :
  forall (lib1 lib2 : @library o) (name : EntryName),
    lib_extends lib1 lib2
    -> in_lib name lib2
    -> in_lib name lib1.
Proof.
  ...
  apply ext in i1; exrepnd.
  eexists; dands; eauto.
  apply entry_extends_implies_same_entry_name in i2; eauto 3 with slow.
Qed.
Hint Resolve lib_extends_preserves_in_lib : slow.

Lemma implies_lib_extends_swap_names_lr {o} :
  forall (name name' : choice_sequence_name) (lib1 lib0 : @library o),
    lib_nodup lib0
    -> sat_lib_cond lib0
    -> strong_safe_library lib0
    -> name <> name'
    ...
    -> lib_extends (swap_names name name' lib1) (swap_names name name' lib0).
Proof.
  introv nodup sat safe diff ia ib ext.
  rewrite <- (swap_names_twice name name' lib0) in ext; auto.
  eapply implies_lib_extends_swap_names_left in ext; eauto 3 with slow.
Qed.

Lemma swapped_css_swap_names {o} :
  forall name name' (lib : @library o),
    name <> name'
    -> lib_nodup lib
    -> in_lib (entry_name_cs name) lib
    -> in_lib (entry_name_cs name') lib
    -> swapped_css (name, name') lib (swap_names name name' lib).
Proof.
  introv d nodup ia ib.
  unfold swap_names; simpl.
  unfold pre_swap_names.
  remember (find_cs lib name) as fcsa; symmetry in Heqfcsa.
  remember (find_cs lib name') as fcsb; symmetry in Heqfcsb.
  destruct fcsa, fcsb; simpl in *; tcsp; eauto 3 with slow;
    try (complete (apply find_cs_non_implies_not_in_lib in Heqfcsa; tcsp));
    try (complete (apply find_cs_non_implies_not_in_lib in Heqfcsb; tcsp)).
  apply swapped_css_sym.
  apply swapped_css_swap_two_names; eauto 3 with slow.
Qed.

Lemma swapped_css_libs_equality_of_qnat {o} :
  forall {name name'} {lib lib' : @library o} {a b : @CTerm o} {c},
    lib_nodup lib
    -> sat_lib_cond lib
    -> strong_safe_library lib
    -> swapped_css_libs name name' lib lib'
    ...
    -> equality_of_qnat lib' c a b.
Proof.
  introv nodup sat safe swap ceq.
  unfold equality_of_qnat in *; repnd; dands.

  { introv ext.
    applydup @swapped_css_libs_sym in swap as swap'.
    eapply lib_extends_preserves_perm_libs in ext; eauto; eauto 3 with slow; repnd.
    pose proof (ceq0 _ ext0) as ceq0.
    cbv beta in ceq0; exrepnd.
  ...
  { introv q exta extb compa compb; subst.
    assert (lib_nodup lib1) as nodup1 by eauto 3 with slow.
    applydup @swapped_css_libs_sym in swap as swap'.
    dup exta as exta'.
    eapply lib_extends_preserves_perm_libs in exta; eauto; eauto 3 with slow; repnd.

    destruct (in_lib_dec (entry_name_cs name) lib1) as [d1|d1];[|rewrite swap_sames_same_not_in in exta0; tcsp; eapply ceq; eauto];[].
    destruct (in_lib_dec (entry_name_cs name') lib1) as [d2|d2];[|rewrite swap_sames_same_not_in in exta0; tcsp; eapply ceq; eauto];[].
    destruct (choice_sequence_name_deq name' name) as [d3|d3]; subst;[rewrite swap_names_same in exta0; eauto 4 with slow|];[].

    applydup (@implies_lib_extends_swap_names_lr o name name') in extb; eauto 3 with slow;[].

    pose proof (@swapped_css_ccomputes_to_valc o (name,name') lib1 (swap_names name name' lib1) a (mkc_nat n1)) as compa'.
    repeat (autodimp compa' hyp); try (complete (spcast; auto)); eauto 3 with slow.
    { apply swapped_css_swap_names; eauto 4 with slow. }

    pose proof (@swapped_css_ccomputes_to_valc o (name,name') lib2 (swap_names name name' lib2) a (mkc_nat n2)) as compb'.
    repeat (autodimp compb' hyp); try (complete (spcast; auto)); eauto 3 with slow.
    { apply swapped_css_swap_names; eauto 3 with slow. }

    exrepnd; spcast.
    allapply @alphaeqc_mkc_nat_implies; subst.
    eapply ceq; eauto. }
Qed.

Lemma swapped_css_libs_equality_of_qnat_bar {o} :
  forall name name' (lib lib' : @library o) c,
    lib_nodup lib
    -> sat_lib_cond lib
    -> strong_safe_library lib
    -> swapped_css_libs name name' lib lib'
    -> (equality_of_qnat_bar lib c) <=2=> (equality_of_qnat_bar lib' c).
Proof.
  introv nodup sat safe swap; introv; split; intro h.

  { eapply in_open_bar_swapped_css_libs_pres; try exact h; eauto.
    clear h; introv ext h.
    eapply (@swapped_css_libs_equality_of_qnat o name name'); try exact h; eauto 3 with slow.
    eapply lib_extends_preserves_perm_libs; eauto. }
...
Lemma swapped_css_libs_equality_of_qlt_bar {o} :
  forall name name' (lib lib' : @library o) a b
         (nodup : lib_nodup lib)
         (sat   : sat_lib_cond lib)
         (safe  : strong_safe_library lib)
         (swap  : swapped_css_libs name name' lib lib'),
    (equality_of_qlt_bar lib a b) <=2=> (equality_of_qlt_bar lib' a b).
Proof.
  introv; introv; split; intro h.
  { eapply in_open_bar_swapped_css_libs_pres; try exact h; eauto.
    clear h; introv ext h xt.
    destruct (in_lib_dec (entry_name_cs name) lib'0) as [d1|d1];[|rewrite swap_sames_same_not_in in xt; tcsp];[].
    destruct (in_lib_dec (entry_name_cs name') lib'0) as [d2|d2];[|rewrite swap_sames_same_not_in in xt; tcsp];[].
    destruct (choice_sequence_name_deq name' name) as [d3|d3]; subst;[rewrite swap_names_same in xt; eauto 3 with slow|];[].
    pose proof (h (swap_names name name' lib'1)) as h.
    autodimp h hyp.
    { apply implies_lib_extends_swap_names_left; eauto 3 with slow. }
    cbv beta in h; exrepnd.
    eapply swapped_css_libs_ccomputes_to_valc in h0; eauto;
      try eapply lib_extends_preserves_perm_libs; eauto; eauto 3 with slow; exrepnd;
        try (apply implies_lib_nodup_swap_names; eauto 4 with slow);
        try (eapply lib_extends_trans;[|eauto];
             apply implies_lib_extends_swap_names_left; eauto 3 with slow).
    ...
    apply alphaeqc_mkc_nat_implies in h4; subst.
    rewrite swap_names_twice in h0; eauto 4 with slow.
    rewrite swap_names_twice in h2; eauto 4 with slow.
  ...
  { dup swap as swap'; apply swapped_css_libs_sym in swap'.
    ...
    cbv beta in h; exrepnd.
    eapply swapped_css_libs_ccomputes_to_valc in h0; eauto;
      try (apply swapped_css_libs_swap_names; eauto 5 with slow);
      try (apply implies_lib_nodup_swap_names; eauto 5 with slow).
    eapply swapped_css_libs_ccomputes_to_valc in h2; eauto;
      try (apply swapped_css_libs_swap_names; eauto 5 with slow);
      try (apply implies_lib_nodup_swap_names; eauto 5 with slow).
    exrepnd; spcast.
...
Qed.
Hint Resolve tequality_mkc_csprop : slow.

Lemma substc5_mkcv_squash {o} :
  forall (t1 t2 t3 : @CTerm o) x v w z a,
    substc5 x t1 v t2 w t3 z (mkcv_squash _ a)
    = mkcv_squash
        _
        (substc5 x t1 v t2 w t3 z a).
Proof.
...
Proof.
  ...
  unfold equality_of_csname; exists name; dands; eauto 3 with slow.
Qed.
Hint Resolve implies_equality_mkc_choice_seq_mkc_csname_0 : slow.

Lemma ccequivc_app_app_ax {o} :
  forall lib b x (u v : @CTerm o),
    ccequivc_ext
      lib
      (mkc_apply (mkc_apply (mkc_lam b (mkcv_lam [b] x (mkcv_ax [x, b]))) u) v)
      mkc_axiom.
Proof.
  introv ext; spcast.
  destruct_cterms; unfold cequivc; simpl.
  apply reduces_to_implies_cequiv; eauto 3 with slow.
  { repeat apply isprogram_apply; eauto 3 with slow.
    apply isprogram_lam; apply isprog_vars_lam_iff; simpl; eauto 3 with slow. }
