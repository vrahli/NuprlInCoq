...
Lemma equality_in_sper_function {o} :
  forall lib (f g : @CTerm o) A B w s c,
    equality lib f g (lsubstc (mk_sper_function A B) w s c)
    -> {wa : wf_term A
        , {wb : wf_term B
        , {ca : cover_vars A s
        , {cb : cover_vars B s
...
Proof.
  ...
  assert (wf_term A)
    as wA by (dup w as w'; apply wf_term_mk_sper_function in w'; sp).
  assert (wf_term B)
    as wB by (dup w as w'; apply wf_term_mk_sper_function in w'; sp).

  assert (cover_vars A s)
    as cA by (dup c as c'; apply cover_vars_sper_function in c'; sp).
  assert (cover_vars B s)
    as cB by (dup c as c'; apply cover_vars_sper_function in c'; sp).

  exists wA wB cA cB; cbv zeta.

  unfold mk_sper_function in eq.
  lsubst_tac.
  rwpers.
  ...
  revert eq0 eq1 eq2 eq3 eq.
  revert dependent w1.
  revert dependent c1.
  rw e1; introv inh ty teq1 teq2 isper.
  lsubst_tac.
...
Lemma tequality_mk_sper_function_implies {o} :
  forall lib (A B : @NTerm o) w s1 s2 c1 c2,
    tequality lib (lsubstc (mk_sper_function A B) w s1 c1)
              (lsubstc (mk_sper_function A B) w s2 c2)
    -> {wa : wf_term A
        , {wb : wf_term B
        , {ca1 : cover_vars A s1
        , {ca2 : cover_vars A s2
...
Proof.
  ...
  assert (cover_vars A s1)
    as cA1 by (dup c1 as c'; apply cover_vars_sper_function in c'; sp).
  assert (cover_vars B s1)
    as cB1 by (dup c1 as c'; apply cover_vars_sper_function in c'; sp).
  assert (cover_vars A s2)
    as cA2 by (dup c2 as c'; apply cover_vars_sper_function in c'; sp).
  assert (cover_vars B s2)
    as cB2 by (dup c2 as c'; apply cover_vars_sper_function in c'; sp).

  exists wA wB cA1 cA2 cB1 cB2; cbv zeta.

  unfold mk_sper_function in teq.
  lsubst_tac.
  rwpers.
  generalize (unfold_mk_iper_function_rel2 A B); intro e; exrepnd.
  apply newvars5_prop2 in e0; simpl in e0;
  ...
  revert teq0 teq1 teq2 teq.
  revert dependent w1.
  revert dependent c0.
  revert dependent c3.
  rw e1; introv teq teq1 teq2 isper.
  lsubst_tac.
...
(* [29] ============ SPER-FUNCTION ELIMINATION ============ *)

  (*
   H, f : sper-function(A,B), J |- C ext e[z\axiom]

     By sperFunctionElimination s y z

     H, f : sper-function(A,B), J |- a in A
     H, f : sper-function(A,B), J, z : (f a) in (B a) |- C ext e
 *)
Definition rule_sper_function_elimination {o}
           (A B C a e : @NTerm o)
           (f z : NVar)
           (H J : barehypotheses) :=
  mk_rule
    (mk_bseq
       (snoc H (mk_hyp f (mk_sper_function A B)) ++ J)
       (mk_concl C (subst e z mk_axiom)))
    [ mk_bseq
        (snoc H (mk_hyp f (mk_sper_function A B)) ++ J)
        (mk_conclax (mk_member a A)),
      mk_bseq
        (snoc (snoc H (mk_hyp f (mk_sper_function A B)) ++ J)
              (mk_hyp z (mk_member (mk_apply (mk_var f) a) (mk_apply B a))))
        (mk_concl C e)
    ]
    [sarg_term a, sarg_var z].

Lemma rule_sper_function_elimination_true {o} :
  forall lib,
  forall A B C a e : @NTerm o,
  forall f z : NVar,
  forall H J : barehypotheses,
    rule_true lib (rule_sper_function_elimination
                 A B C a e
                 f z
                 H J).
Proof.
  introv.
  unfold rule_sper_function_elimination, rule_true, closed_type_baresequent, closed_extract_baresequent; simpl.
  intros.
  ...
  assert (covered
            (subst e z mk_axiom)
            (nh_vars_hyps (snoc H (mk_hyp f (mk_sper_function A B)) ++ J))) as cv.
  ...
  clear hyp1 hyp2.
  remember (mk_sper_function A B) as pf.
  remember (mk_iper_function_rel A B) as pfr.
  dwfseq.
  subst.
  allrw @free_vars_mk_sper_function.
  ...
  sp;
    ...
    try (complete (pose proof (wfh6 f); allrw in_app_iff; sp));
    try (complete (introv i; pose proof (wfh6 t); allrw in_app_iff; sp; unfold disjoint in wfh11; discover; sp)).
...
Require Export rules_spertype.

unfold mk_bseq.

Ltac gen_seq_true :=
  match goal with
    | [ |- sequent_true _ (mk_wcseq _ ?x) ] => remember x; clears_last
  end.

gen_seq_true.

pose proof (rule_spertype_elimination4_true_ex
              lib
              z
              (mk_iper_function_rel A B)
              C
              (subst e z mk_axiom)
              f H J) as r.
unfold rule_true_if in r.
unfold rule_spertype_elimination4 in r; simpl in r.
apply r;
  try (complete sp);
  try (clear r);
  try (complete (unfold args_constraints, arg_constraints; simpl; sp; subst; sp));
  try (complete (inversion w; auto)).

(*

introv hyps; sp; subst.

remember (wfh0, (wfct, wfce0), (ct, ce)).

unfold wf_csequent, wf_sequent; simpl.
*)

Focus 2.

introv hyps; introv.
dorn hyps; try (complete sp); subst.
