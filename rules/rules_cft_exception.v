...
(* [18] ============ ISPAIR (and other canonical form test) EXCEPTION CASES ============ *)
...
(*   H |- a = b in T
     ...
     H |- t in Base
 *)
Definition rule_can_test_exception_cases {o}
           (H : barehypotheses)
           test
           (a b T t u v: @NTerm o)
           (x: NVar) :=
  mk_rule
    ...
    [ mk_baresequent H (mk_conclax (mk_isexc (mk_can_test test t u v))),
...
Lemma rule_can_test_exception_cases_true {o} :
  forall lib test (a b T t u v : NTerm),
  forall x : NVar,
  forall H : @barehypotheses o,
    rule_true lib (rule_can_test_exception_cases H test a b T t u v x).
Proof.
  unfold rule_can_test_exception_cases, rule_true, closed_type_baresequent,
  ...
  apply if_raises_exceptionc_can_test in hyp1.
  repndors; exrepnd.
