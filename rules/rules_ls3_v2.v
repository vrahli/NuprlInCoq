...
Definition swap_cs_lib_cond {o} (r : cs_swap) (c : @LibCond o) :=
  fun v => c (swap_cs_term r v).
...
Proof.
  ...
  unfold swap_cs_lib_cond; simpl; autorewrite with slow; auto.
Qed.
...
Proof.
  introv ni sat i; apply sat; simpl; right; dands; tcsp.
...
Proof.
  ...
  { destruct entry as [vals restr]; simpl in *.
    introv i; unfold sat_cond_cterm.
...
Proof.
  introv ext sat.
  inversion ext; subst; tcsp; simpl in *; clear ext.
  introv i; apply sat; apply in_app_iff; tcsp.
Qed.
...
Proof.
  ...
  { autorewrite with slow; apply sat; simpl; tcsp. }
...
Proof.
  ...
  destruct entry as [vals restr]; simpl in *; eauto 3 with slow.
Qed.
...
Proof.
  introv sat.
  destruct e as [vals restr]; simpl in *; tcsp.
...
Proof.
  ...
  clear lib safe nodup sat ext.
  rename lib' into lib; rename safe' into safe; rename nodup' into nodup; rename sat' into sat.
...
(*  eapply member_monotone in eqz;[|exact xt];[].*)
  ...
  assert (sat_lib_cond lib'0) as sat' by eauto 3 with slow.
...
Print is_nat_seq_restriction.
   ...
   Lemma contains_atmost_implies_computes_to_valc_keep_only {o} :
     forall name (lib : plibrary) (t v : @CTerm o),
       contains_atmost name t
       -> computes_to_valc lib t v
       -> computes_to_valc (keep_only_pre name lib) t v.
   Proof.
   Admitted.
...
Definition replace_from {o} name (lib2 lib1 : @library o) :=
  ...
  | Some e => pre_replace_cs_entry lib1 name e
  | None => lib1
  end.
...
Proof.
  ...
  remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs;
    [|rewrite pre_replace_cs_entry_if_find_none; eauto].
  apply implies_lib_extends_ext in ext.
  ...
  apply find_cs_implies_lib_extends_pre_replace_cs_entry_app_left; auto.
Qed.
...
Lemma find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left {o} :
  forall name (e e' : @ChoiceSeqEntry o) lib,
    safe_choice_sequence_entry name e'
    -> find_cs lib name = Some e
    -> cs_entry_extends e' e
    -> lib_extends (pre_replace_cs_entry lib name e') lib.
Proof.
  introv safe fcs ext.
...
Proof.
  ...
  remember (find_cs lib' name) as fcs; symmetry in Heqfcs; destruct fcs;
    remember (find_cs lib'' name) as fcs'; symmetry in Heqfcs'; destruct fcs'; eauto;
      try (complete (rewrite pre_replace_cs_entry_if_find_none; eauto)).

  eapply find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left; eauto; eauto 3 with slow.
  apply implies_lib_extends_ext in ext.
  assert (entry_in_library (lib_cs name c) (pre_replace_cs_entry lib name c)) as i.
  { apply find_cs_some_implies_entry_in_library.
    rewrite find_cs_pre_replace_cs_entry; boolvar; tcsp; GC.
...
Lemma pre_replace_cs_entry_twice {o} :
  forall (lib : @library o) name c1 c2,
    pre_replace_cs_entry (pre_replace_cs_entry lib name c1) name c2
    = pre_replace_cs_entry lib name c2.
Proof.
  induction lib; introv; simpl in *; tcsp.
  destruct a; repeat (simpl in *; boolvar; subst; tcsp); rewrite IHlib; auto.
Qed.
Hint Rewrite @pre_replace_cs_entry_twice : slow.
...
Lemma implies_lib_extends_pre_replace_cs_entry_find_none {o} :
  forall (lib1 lib2 : @library o) name e,
    is_nat_cs name
    -> find_cs lib1 name = None
    -> safe_library lib1
    -> safe_choice_sequence_entry name e
    -> lib_extends lib2 lib1
    -> lib_extends (pre_replace_cs_entry lib2 name e) lib1.
Proof.
  introv isn fcsa sl safe ext.
  lib_ext_ind ext Case; introv; tcsp;
    try (complete (rewrite pre_replace_cs_entry_if_find_none; auto)).

  { Case "lib_ext_trans".
    repeat (autodimp IHext2 hyp); eauto 3 with slow.
    eapply lib_extends_trans;[|eauto].
    remember (find_cs lib2 name) as fcsb; symmetry in Heqfcsb; destruct fcsb;
      try (complete (rewrite (pre_replace_cs_entry_if_find_none lib2); auto; apply IHext1; eauto 3 with slow)).

    clear IHext1.
    apply implies_lib_extends_pre_replace_cs_entry; eauto 3 with slow.
    eexists; dands; eauto.
  ...
  { Case "lib_ext_new_cs".
    ...
    rewrite pre_replace_cs_entry_if_find_none; auto.
...
Proof.
  ...
  { eapply find_cs_lib_extends_implies_cs_entry_extends in extb; eauto; simpl; boolvar; tcsp.
    ...
    apply (implies_lib_extends_pre_replace_cs_entry _ _ name c0) in exta; eauto 3 with slow;
      try (complete (exists c; rewrite find_cs_pre_replace_cs_entry; boolvar; tcsp; allrw; dands; auto;
                            inversion extb; inversion extc; subst; simpl in *; ginv)).
    autorewrite with slow in *.
    eapply lib_extends_trans; eauto.
    eapply find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }

  { rewrite pre_replace_cs_entry_if_find_none in exta; auto.
    apply implies_lib_extends_pre_replace_cs_entry_find_none; eauto 3 with slow. }
...
Proof.
  ...
  rewrite find_cs_pre_replace_cs_entry; boolvar; tcsp; GC; allrw; auto.
Qed.
...
Proof.
  ...
  { assert (entry_in_library (lib_cs name c0) [lib_cs name c0]) as i; simpl; tcsp.
    ...
    rewrite i1 in *; ginv.
    eapply lib_extends_trans;[|eapply find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left];
...
Lemma has_name_pre_replace_cs_entry {o} :
  forall name (lib : @library o) c,
    has_name name lib
    -> has_name name (pre_replace_cs_entry lib name c).
Proof.
  introv hn.
  ...
  allrw; eauto.
Qed.
Hint Resolve has_name_pre_replace_cs_entry : slow.
...
Lemma extends_pre_replace_cs_entry_implies_cs_entry_extends {o} :
  forall name c c' (lib lib' : @library o),
    has_name name lib'
    -> lib_extends lib (pre_replace_cs_entry lib' name c')
    -> find_cs lib name = Some c
    -> cs_entry_extends c c'.
Proof.
  introv hn ext fcs.
  ...
  { rewrite find_cs_pre_replace_cs_entry in fcs; boolvar; subst; tcsp; GC.
    unfold has_name in *; exrepnd.
    rewrite hn0 in *; ginv. }

  exrepnd; subst; simpl in *.
  rewrite find_cs_pre_replace_cs_entry in fcs0; boolvar; tcsp.
  unfold has_name in *; exrepnd.
  rewrite hn0 in *; GC; ginv.
  destruct e' as [vals' restr']; simpl in *; eauto.
Qed.
Hint Resolve extends_pre_replace_cs_entry_implies_cs_entry_extends : slow.

Lemma has_compatible_restriction_pre_replace_cs_entry {o} :
  forall name (lib : @library o) c c',
    has_name name lib
    -> is_nat_cs name
    -> safe_choice_sequence_entry name c
    -> safe_choice_sequence_entry name c'
    -> has_compatible_restriction (pre_replace_cs_entry lib name c) name c'.
Proof.
...
Qed.
Hint Resolve has_compatible_restriction_pre_replace_cs_entry : slow.
...
Proof.
  ...
  { apply (implies_lib_extends_pre_replace_cs_entry _ _ name c1) in exta; eauto 3 with slow.
    { eapply lib_extends_trans;[eauto|]; autorewrite with slow.
      eapply find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }
    apply has_compatible_restriction_pre_replace_cs_entry; eauto 3 with slow. }

  { eapply lib_extends_find_none_left_implies in extb; eauto.
    rewrite find_cs_pre_replace_cs_entry in extb; boolvar; tcsp; GC.
...
Proof.
  ...
  { apply (implies_lib_extends_pre_replace_cs_entry _ _ name c0) in xtb; eauto 3 with slow.
    eapply lib_extends_trans;[eauto|].
    eapply find_cs_implies_lib_extends_pre_replace_cs_entry_ext_left; eauto; eauto 3 with slow. }
...
Proof.
  ...
  { dup ext as xt; eapply find_cs_lib_extends_implies_cs_entry_extends in ext; eauto.
    remember (find_cs lib name) as fcs; symmetry in Heqfcs; destruct fcs;
      [|repeat (rewrite pre_replace_cs_entry_if_find_none; auto)].
    inversion ext; subst; simpl in *.
    eapply lib_extends_trans;[|eapply find_cs_implies_lib_extends_pre_replace_cs_entry_app_left];
      try (rewrite pre_replace_cs_entry_twice; apply lib_extends_refl).
    ...
    { rewrite find_cs_pre_replace_cs_entry; boolvar; tcsp; GC; allrw; tcsp. } }
...
Definition ceq_per {o} lib (e1 e2 : per(o)) :=
  forall a b,
    e1 a b ->
    exists a' b',
...
Definition ceq_pers {o} lib (e1 e2 : per(o)) :=
  ceq_per lib e1 e2 /\ ceq_per lib e2 e1.

Lemma in_open_bar_ext_strong_safe_implies {o} :
  forall (lib : @library o) (F : forall lib' (x : lib_extends lib' lib), Prop),
    strong_safe_library lib
    -> in_open_bar_ext lib (fun lib' x => strong_safe_library lib' -> F lib' x)
    -> in_open_bar_ext lib (fun lib' x => F lib' x).
Proof.
  introv safe ext.
  eapply in_open_bar_ext_pres; eauto; clear ext; introv h; tcsp.
...
Lemma in_open_bar_ext_has_name_implies {o} :
  forall name (lib : @library o) (F : forall lib' (x : lib_extends lib' lib), Prop),
    has_name name lib
    -> in_open_bar_ext lib (fun lib' x => has_name name lib' -> F lib' x)
    -> in_open_bar_ext lib (fun lib' x => F lib' x).
Proof.
  introv hn ext.
  eapply in_open_bar_ext_pres; eauto; clear ext; introv h; tcsp.
...
Definition FunPer {o} {lib} (F : @FunLibExt o lib) :=
  forall lib1 (ext1 : lib_extends lib1 lib)
         lib2 (ext2 : lib_extends lib2 (F lib1 ext1))
         (z : lib_extends lib2 lib),
    per(o).

Lemma in_open_bar_per_choice {o} :
  forall (lib : @library o)
         (F : FunLibExt lib)
         (G : forall lib1 (ext1 : lib_extends lib1 lib) lib2 (ext2 : lib_extends lib2 (F lib1 ext1)) (z : lib_extends lib2 lib) (p : per(o)), Prop),
    in_ext_ext
      lib
      (fun lib1 (ext1 : lib_extends lib1 lib) =>
         in_ext_ext
           ...
           (fun lib2 (ext2 : lib_extends lib2 (F lib1 ext1)) =>
              ...
              exists (p : per(o)),
                G lib1 ext1 lib2 ext2 z p))
    -> exists (Fper : FunPer F),
      in_ext_ext
        lib
        (fun lib1 (ext1 : lib_extends lib1 lib) =>
           in_ext_ext
             ...
             (fun lib2 (ext2 : lib_extends lib2 (F lib1 ext1)) =>
                forall (z : lib_extends lib2 lib),
                  G lib1 ext1 lib2 ext2 z (Fper lib1 ext1 lib2 ext2 z))).
Proof.
  introv h.
  pose proof (FunctionalChoice_on
                ...
                per(o)
                (fun d p =>
                   G (lib_ext_lib1 d) (lib_ext_ext1 d) (lib_ext_lib2 d) (lib_ext_ext2 d) (lib_ext_extz d) p)) as C.
  simpl in C.
  repeat (autodimp C hyp).
  { introv; destruct a as [lib1 ext1 lib2 ext2 extz]; simpl in *.
...
Definition per2ceq {o} lib (p : per(o)) : per(o) :=
  fun a b =>
    exists a' b',
      (forall c, ccequivc_ext lib a c <-> ccequivc_ext lib a' c)
      /\ (forall c, ccequivc_ext lib b c <-> ccequivc_ext lib b' c)
      /\ p a' b'.
  ...
  forall name lib (u : cts(o)) (t1 t2 : @CTerm o) e,
    ...
    -> (forall lib t1 t2 e,
           ...
           -> forall lib',
               has_name name lib'
               -> safe_library lib'
               -> u (replace_from name lib lib') t1 t2 (per2ceq lib e))
    -> strong_safe_library lib
    -> has_name name lib
    -> close u lib t1 t2 e
    -> forall lib',
        has_name name lib'
        -> safe_library lib'
        -> close u (replace_from name lib lib') t1 t2 (per2ceq lib e).
Proof.
...
2:{ Case "CL_int".
    ...
    introv; unfold per2ceq, equality_of_int_bar; simpl; split; intro h; exrepnd.

2:{

    { apply per in h1; clear per; unfold equality_of_int_bar in h1.
      introv xta.
      applydup @lib_extends_replace_from_left in xta; eauto 4 with slow.
      pose proof (h1 _ xta0) as h1; simpl in h1; exrepnd.
      assert (safe_library lib'') as safea by eauto 4 with slow.
      assert (safe_library lib'0) as safeb by eauto 4 with slow.
      assert (lib_extends (replace_from name lib'' lib'0) lib'0) as xtb.
      { eapply lib_extends_replace_from_left; try exact xt; eauto 3 with slow. }
      exists (replace_from name lib'' lib'0) xtb.
      introv xtc.
      assert (lib_extends (replace_from name lib'1 lib'') lib'') as xtd.
      { eapply lib_extends_replace_from_left; try exact xtc; eauto 3 with slow. }
      pose proof (h1 (replace_from name lib'1 lib'') xtd) as h1; simpl in h1.

      assert (lib_extends lib'1 (replace_from name lib lib')) as xte by eauto 3 with slow.
      assert (lib_extends (replace_from name lib'1 lib'') lib) as xtf by eauto 3 with slow.
      unfold equality_of_int in *; exrepnd.
      exists k; dands.

      { eapply lib_extends_preserves_ccequivc_ext in h3; try exact xte.
        eapply lib_extends_preserves_ccequivc_ext in h4; try exact xte.
        eapply lib_extends_preserves_ccequivc_ext in h0; try exact xtf.
        eapply lib_extends_preserves_ccequivc_ext in h2; try exact xtf.

        eapply ccomputes_to_valc_ext_integer_implies_computes_to_valc_in_ext in h1.
SearchAbout ccomputes_to_valc lib_extends.
 }
    }
...
*)
  ...
  { Case "CL_bar".
    ...
    assert (safe_library lib) as safe' by eauto 3 with slow.
    repeat (apply in_open_bar_ext_const_imp_implies in reca; auto; simpl in *).
    apply in_open_bar_ext_strong_safe_implies in reca; auto.
    apply in_open_bar_ext_has_name_implies in reca; auto.
