...
Proof.
  ...
  + SCase "uniquely_valued".
    ...
    * SSCase "CL_atom".
      assert (uniquely_valued (per_atom_bar inh (close inh ts))) as uv
  ...
  + SCase "type_symmetric"; repdors; subst; dclose_lr;
    ...
    assert (type_symmetric (per_atom_bar inh (close inh ts))) as tys
      by (apply per_atom_bar_type_symmetric);
    assert (type_extensionality (per_atom_bar inh (close inh ts))) as tye
  ...
  + SCase "type_value_respecting"; sp; subst;
      apply per_atom_bar_implies_close;
      apply per_atom_implies_per_atom_bar in per;
      assert (type_value_respecting inh (per_atom_bar inh (close inh ts)))
        as tvr
  ...
  + SCase "term_symmetric".
    assert (term_symmetric (per_atom_bar inh (close inh ts))) as tes
      by (apply per_atom_bar_term_symmetric).
    eapply tes; eauto 3 with slow.

  + SCase "term_transitive".
    assert (term_transitive (per_atom_bar inh (close inh ts))) as tet
      by (apply per_atom_bar_term_transitive).
    eapply tet; eauto 3 with slow.

  + SCase "term_value_respecting".
    assert (term_value_respecting inh (per_atom_bar inh (close inh ts))) as tvr
