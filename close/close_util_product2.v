...
Proof.
  ...
  - unfold per_product_eq_bar, per_product_eq.
    ...
    assert (eqa lib'0 z a a') as e'' by (eapply (lib_per_cond _ _ eqa); eauto).
  ...
  - eapply in_open_bar_ext_comb; try exact per1; clear per1.
    ...
    apply (sub_per_per_product_bar_eq _ _ _ e) in h.
...
Lemma type_value_respecting_trans_per_bar_per_product_bar1 {o} :
  forall inh lib (ts : cts(o)) T T1 T2 A v B A' v' B' C w D (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) eq,
    ...
    -> per_bar inh (per_product_bar inh ts) lib T1 T2 eq
    -> per_bar inh (per_product_bar inh ts) lib T T2 eq.
Proof.
...
Lemma type_value_respecting_trans_per_bar_per_product_bar2 {o} :
  forall inh lib (ts : cts(o)) T T1 T2 A v B A' v' B' C w D (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) eq,
    ...
    -> ccequivc_ext inh lib A A'
    -> bcequivc_ext inh lib [v] B [v'] B'
    -> per_bar inh (per_product_bar inh ts) lib T2 T1 eq
    -> per_bar inh (per_product_bar inh ts) lib T T2 eq.
Proof.
  introv tsa tsb comp1 comp2 ceqa ceqb per.
  unfold per_bar in *; exrepnd.
...
Lemma per_product_eq_bar_resp {o} :
  forall inh ts lib (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) A v B A' v' B' t t',
    in_ext_ext inh lib (fun lib' x => type_sys_props4 inh ts lib' A A' (eqa lib' x))
    -> in_ext_ext inh lib (fun lib' x => forall a a' (e : eqa lib' x a a'), type_sys_props4 inh ts lib' (substc a v B) (substc a' v' B') (eqb lib' x a a' e))
    -> per_product_eq_bar inh lib eqa eqb t t
...
Proof.
  ...
  dup per1 as c.
  eapply (ccequivc_ext_pair _ _ t t') in c; eauto 3 with slow; exrepnd; spcast.
...
Lemma per_bar_per_product_bar_trans1 {o} :
  forall inh (ts : cts(o)) lib T T1 T2 A v B A' v' B' (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) eq1 eq2,
    ...
    -> in_ext_ext inh lib (fun lib' x => forall a a' (e : eqa lib' x a a'), type_sys_props4 inh ts lib' (substc a v B) (substc a' v' B') (eqb lib' x a a' e))
    -> ccomputes_to_valc_ext inh lib T (mkc_product A v B)
    -> per_bar inh (per_product_bar inh ts) lib T1 T eq1
    -> per_bar inh (per_product_bar inh ts) lib T T2 eq2
    -> per_bar inh (per_product_bar inh ts) lib T1 T2 eq1.
Proof.
...
Lemma per_bar_per_product_bar_trans2 {o} :
  forall inh (ts : cts(o)) lib T T1 T2 A v B A' v' B' (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) eq1 eq2,
    ...
    -> per_bar inh (per_product_bar inh ts) lib T1 T eq1
    -> per_bar inh (per_product_bar inh ts) lib T T2 eq2
    -> per_bar inh (per_product_bar inh ts) lib T1 T2 eq2.
Proof.
  introv tsa tsb comp pera perb.
  unfold per_bar in *; exrepnd.
  exists eqa0; dands; auto.
...
Lemma type_value_respecting_trans_per_bar_per_product_bar3 {o} :
  forall inh lib (ts : cts(o)) T T1 T2 A v B C w D (eqa : lib-per(inh,lib,o)) (eqb : lib-per-fam(inh,lib,eqa,o)) eq,
    ...
    -> per_bar inh (per_product_bar inh ts) lib T T1 eq
    -> per_bar inh (per_product_bar inh ts) lib T T2 eq.
Proof.
