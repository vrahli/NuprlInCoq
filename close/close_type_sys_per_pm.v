...
Proof.
  ...
  + SCase "uniquely_valued".
    ...
    generalize (type_pfamily_eq_term_equals
                  ...
                  ap' A' eqa
                  bp' ba' B' eqb).
    introv k; repeat (autodimp k hyp); try (apply eq_type_pfamilies_mkcn_pm).
    repnd; repeat subst.
  ...
  + SCase "type_value_respecting".
    ...
    (* 1 *)
    generalize (cequivcn_mkcn_pm lib T T3 P ap A bp ba B cp ca cb C p c1); intro k.
    autodimp k hyp; exrepnd.

    generalize (type_pfamily_cequivc
                  lib mkcn_pm (close lib ts) T T3 eqp eqa eqb
                  P ap A bp ba B cp ca cb C p
    ...
    (* 2 *)
    generalize (cequivcn_mkcn_pm lib T' T3 P' ap' A' bp' ba' B' cp' ca' cb' C' p' c2); intro k.
    autodimp k hyp; exrepnd.
  ...
  + SCase "term_symmetric".
    ...
    generalize (type_pfamily_sym
                    lib mkcn_pm (close lib ts) T T' eqp0 eqa0 eqb0
                    cp ca cb C
                    cp' ca' cb' C'
                    p p'
                    P ap A bp ba B cp ca cb C p
    ...
    repeat (autodimp k hyp);
      try (complete (apply eq_type_pfamilies_mkcn_pm)).
  ...
  + SCase "type_mtransitive".
    ...
    generalize (type_pfamily_eq_term_equals
                  lib mkcn_pm (close lib ts) T T3 T4
    ...
    generalize (type_pfamily_trans2
                  lib (close lib ts) T3 T' T4 mkcn_pm
                  eqp0 eqa0 eqb0
                  eqp1 eqa1 eqb1
                  eqp eqa eqb
                  P' ap' A' bp' ba' B' cp' ca' cb' C' p'
