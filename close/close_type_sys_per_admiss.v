...
Lemma close_type_system_admiss {p} :
  forall lib (ts : cts(p))
    ...
    -> computes_to_valcn lib T (mkcn_admiss A1)
    -> computes_to_valcn lib T' (mkcn_admiss A2)
...
Proof.
  ...
  - SCase "type_value_respecting"; repdors; subst;
    ...
    (* 1 *)
    generalize (cequivcn_mkcn_admiss lib T T3 A1); introv k; repeat (autodimp k hyp); exrepnd.
    exists A1 a' eqa; sp; spcast; sp.
    onedtsp uv tys tyt tyst tyvr tes tet tevr tygs tygt tymt.
    generalize (tyvr A1 a'); sp.

    (* 2 *)
    generalize (cequivcn_mkcn_admiss lib T' T3 A2); introv k; repeat (autodimp k hyp); exrepnd.
