...
Proof.
  ...
  duplicate per as pi.
  unfold per_nat in pi; exrepnd; spcast.
  ...
  + SCase "uniquely_valued".
    ...
    * SSCase "CL_int".
      assert (uniquely_valued (per_nat_bar inh (close inh ts))) as uv
  ...
  + SCase "type_symmetric"; repdors; subst; dclose_lr;
      ...
      apply per_nat_implies_per_nat_bar in per.

    assert (type_symmetric (per_nat_bar inh (close inh ts))) as tys
        by (apply per_nat_bar_type_symmetric).
    assert (type_extensionality (per_nat_bar inh (close inh ts))) as tye
  ...
  + SCase "type_value_respecting"; sp; subst;
      apply per_nat_bar_implies_close;
      apply per_nat_implies_per_nat_bar in per;
      assert (type_value_respecting inh (per_nat_bar inh (close inh ts)))
        as tvr
  ...
  + SCase "type_mtransitive"; repdors; subst; dclose_lr;
      dands; apply per_nat_bar_implies_close; allunfold @per_nat_bar; sp;
