...
Lemma type_value_respecting_trans_per_bar_per_set1 {o} :
  forall uk lib (ts : cts(o)) T T1 T2 A v B A' v' B' C w D (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)) eq,
    ...
    -> per_bar (per_set ts) uk lib T1 T2 eq
    -> per_bar (per_set ts) uk lib T T2 eq.
Proof.
...
Lemma type_value_respecting_trans_per_bar_per_set2 {o} :
  forall uk lib (ts : cts(o)) T T1 T2 A v B A' v' B' C w D (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)) eq,
    ...
    -> ccequivc_ext lib A A'
    -> bcequivc_ext lib [v] B [v'] B'
    -> per_bar (per_set ts) uk lib T2 T1 eq
    -> per_bar (per_set ts) uk lib T T2 eq.
Proof.
  introv tsa tsb comp1 comp2 ceqa ceqb per.
  unfold per_bar in *; exrepnd.
...
Lemma type_sys_props4_implies_term_equality_respecting {o} :
  forall uk (lib : library) (ts : cts(o)) (A B : CTerm) (eqa : per(o)),
  ...
  forall ts uk lib (T1 T2 : @CTerm o) A A' v v' B B' equ eqa eqb,
    ...
    -> in_ext_ext lib
                  (fun lib' x =>
                     forall a a' (e : eqa lib' x a a'),
                       type_sys_props4 ts uk lib' (B)[[v\\a]] (B')[[v'\\a']] (eqb lib' x a a' e))
    -> per_set ts uk lib T1 T2 equ
    -> per_set ts uk lib T2 T1 equ.
Proof.
  ...
  forall ts uk lib (T T1 T2 : @CTerm o) eq1 eq2 eqa eqb A v B A' v' B',
    ...
    -> in_ext_ext
         ...
         (fun lib' x =>
            forall a a' (e : eqa lib' x a a'),
              type_sys_props4 ts uk lib' (B)[[v\\a]] (B')[[v'\\a']] (eqb lib' x a a' e))
    -> per_set ts uk lib T T2 eq2
    -> per_set ts uk lib T1 T eq1
    -> per_set ts uk lib T1 T2 eq1.
Proof.
...
Lemma per_bar_per_set_trans1 {o} :
  forall (ts : cts(o)) uk lib T T1 T2 A v B A' v' B' (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)) eq1 eq2,
    ...
    -> in_ext_ext lib (fun lib' x => forall a a' (e : eqa lib' x a a'), type_sys_props4 ts uk lib' (substc a v B) (substc a' v' B') (eqb lib' x a a' e))
    -> ccomputes_to_valc_ext lib T (mkc_set A v B)
    -> per_bar (per_set ts) uk lib T1 T eq1
    -> per_bar (per_set ts) uk lib T T2 eq2
    -> per_bar (per_set ts) uk lib T1 T2 eq1.
Proof.
...
Lemma per_bar_per_set_trans2 {o} :
  forall (ts : cts(o)) uk lib T T1 T2 A v B A' v' B' (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)) eq1 eq2,
    ...
    -> per_bar (per_set ts) uk lib T1 T eq1
    -> per_bar (per_set ts) uk lib T T2 eq2
    -> per_bar (per_set ts) uk lib T1 T2 eq2.
Proof.
  introv tsa tsb comp pera perb.
  unfold per_bar in *; exrepnd.
  exists eqa0; dands; auto.
...
Proof.
  introv comp tspa tspb pera perb.
  apply (per_set_trans1 ts uk lib T T1 T2 eq1 eq2 eqa eqb A' v' B' A v B);
...
Proof.
  introv comp tspa tspb pera perb.
  apply (per_set_trans2 ts uk lib T T1 T2 eq1 eq2 eqa eqb A' v' B' A v B);
    try exact pera; try exact perb; eauto.
...
Lemma type_value_respecting_trans_per_bar_per_set3 {o} :
  forall uk lib (ts : cts(o)) T T1 T2 A v B C w D (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)) eq,
    ...
    -> per_bar (per_set ts) uk lib T T1 eq
    -> per_bar (per_set ts) uk lib T T2 eq.
Proof.
