...
Lemma close_type_system_bar {o} :
  forall (ts : cts(o)) uk (lib : @library o) T T' eq (eqa : lib-per(lib,o)),
    ts_implies_per_bar ts
    -> type_system ts
    -> defines_only_universes ts
    -> type_monotone ts
    -> in_open_bar_ext lib (fun lib' x => close ts uk lib' T T' (eqa lib' x))
    -> in_open_bar_ext lib (fun lib' x => type_sys_props4 (close ts) uk lib' T T' (eqa lib' x))
...
Proof.
  ...
  + SCase "uniquely_valued".
...

    {
      ...
      apply (close_monotone ts mon uk _ lib') in cl; auto; exrepnd; eauto 3 with slow.
      apply cl0 in h.
    ...
    {
      assert (close ts uk lib T T' eq) as cl'.
      { apply CL_bar; auto. }
  ...
  + SCase "type_mtransitive".
    ...
    dands; apply CL_bar;
      pose proof (all_in_bar_ext_type_sys_props4_implies_type_transitive_per_bar1
                    (close ts) uk lib T T' eqa eq alltsp per T0 T3 T4 eq1 eq2) as q;
