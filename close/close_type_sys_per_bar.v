...
Lemma close_type_system_bar {o} :
  forall inh (ts : cts(o)) (lib : @library o) T T' eq (eqa : lib-per(inh,lib,o)),
    ...
    -> in_open_bar_ext inh lib (fun lib' x => type_sys_props4 inh (close inh ts) lib' T T' (eqa lib' x))
...
Proof.
  ...
  + SCase "uniquely_valued".
...

    {
      ...
      apply (close_monotone _ _ mon _ lib') in cl; auto; exrepnd; eauto 3 with slow.
      apply cl0 in h.
    ...
    {
      assert (close inh ts lib T T' eq) as cl'.
      { apply CL_bar; auto. }

      eapply (close_implies_per_bar_above _ _ lib) in cl;[|eauto]; eauto 3 with slow.
      ...
      eapply type_sys_props4_implies_eq_term_equals in cl1;[|eauto].
      eapply (lib_per_cond _ _ eqa0); apply cl1.
  ...
  + SCase "type_gsymmetric".
    ...
    * apply close_implies_per_bar in cl; auto.
      apply CL_bar.
      eapply (all_in_bar_ext_type_sys_props4_implies_type_symmetric_per_bar1 _ _ _ T T'); eauto.
  ...
  + SCase "type_mtransitive".
    ...
    dands; apply CL_bar;
      pose proof (all_in_bar_ext_type_sys_props4_implies_type_transitive_per_bar1
                    inh (close inh ts) lib T T' eqa eq alltsp per T0 T3 T4 eq1 eq2) as q;
