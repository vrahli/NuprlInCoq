...
Proof.
  ...
  + SCase "uniquely_valued".
    ...
    * SSCase "CL_csname".
      assert (uniquely_valued (per_csname_bar (close ts))) as uv
        by (apply per_csname_bar_uniquely_valued).
      eapply uv; eauto.
      eapply uniquely_valued_trans5; eauto.
      { apply per_csname_bar_type_extensionality. }
      { apply per_csname_bar_type_symmetric. }
      { apply per_csname_bar_type_transitive. }

  + SCase "type_symmetric"; repdors; subst; dclose_lr;
    apply CL_csname; auto;
    assert (type_symmetric (per_csname_bar (close ts))) as tys
      by (apply per_csname_bar_type_symmetric);
    assert (type_extensionality (per_csname_bar (close ts))) as tye
      by (apply per_csname_bar_type_extensionality);
    apply tye with (eq := eq); auto.

  + SCase "type_value_respecting"; sp; subst; apply CL_csname;
    assert (type_value_respecting (per_csname_bar (close ts)))
           as tvr
           by (apply per_csname_bar_type_value_respecting).
  ...
  + SCase "term_transitive".
    assert (term_transitive (per_csname_bar (close ts))) as tet
      by (apply per_csname_bar_term_transitive).
    eapply tet; eauto.

  + SCase "term_value_respecting".
    assert (term_value_respecting (per_csname_bar (close ts))) as tvr
      by (apply per_csname_bar_term_value_respecting).
  ...
  + SCase "type_gsymmetric"; repdors; subst; split; sp; dclose_lr.
    ...
    { apply CL_csname; allunfold @per_csname_bar; sp.
      eexists; dands; eauto. }
