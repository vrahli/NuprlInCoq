...
Definition inf_choice_sequence_satisfies_restriction {o}
  ...
  | csc_res P => forall n, on_some (vals n) (fun v => exists l, inh l (mkc_apply2 P (mkc_nat n) v)) True
  end.
...
Definition safe_inf_choice_sequence_entry {o} inh (name : choice_sequence_name) (e : @InfChoiceSeqEntry o (*M*)) :=
  match e with
  | MkInfChoiceSeqEntry _ vals restriction =>
    correct_restriction name restriction
    /\ inf_choice_sequence_satisfies_restriction inh vals restriction
  end.
...
Definition safe_inf_library_entry {o} inh (e : @inf_library_entry o (*M*)) :=
  match e with
  | inf_lib_cs name cse => safe_inf_choice_sequence_entry inh name ((*upd_restr_inf_entry name*) cse)
  | _ => True
  end.
...
Definition inf_entry_in_inf_library_default {o}
           (inh    : InhAt)
           (entry  : @inf_library_entry o)
           (inflib : inf_library) :=
  (forall n, ~ matching_inf_entries (inflib n) entry)
  /\ safe_inf_library_entry inh entry
...
Definition entry_in_inf_library {o}
  ...
  inf_entry_in_inf_library_default inh entry inflib.

Definition safe_inf_library {o} inh (inflib : @inf_library o (*M*)) :=
  forall entry, entry_in_inf_library inh entry inflib -> safe_inf_library_entry inh entry.
...
(*Definition entry_in_inf_library_default {o}
           (entry  : @library_entry o)
           (inflib : inf_library) :=
...
Definition BarLibExt {o}
           ...
           (bar : @bar_lib o)
           (lib : @library o) :=
  forall (lib' : library), bar lib' -> lib_extends inh lib' lib.
...
Fixpoint library2choice_sequence_names {o} (lib : @pre_library o) : list choice_sequence_name :=
  match lib with
  | [] => []
...
Lemma not_in_library2choice_sequence_names_iff_find_cs_none {o} :
  forall (lib : @pre_library o) (name : choice_sequence_name),
...
Lemma safe_inf_choice_sequence_entry_simple_inf_choice_seq_entry {o} :
  forall inh name, @safe_inf_choice_sequence_entry o inh name (simple_inf_choice_seq_entry name).
Proof.
...
Lemma safe_inf_library_entry_simple_inf_choice_seq {o} :
  forall inh name, @safe_inf_library_entry o inh (simple_inf_choice_seq name).
Proof.
...
Lemma implies_entry_in_library_snoc {o} :
  forall entry (lib : @pre_library o) e,
    entry_in_library entry lib
...
Lemma implies_entry_in_library_extends_snoc {o} :
  forall entry e (lib : @pre_library o),
...
Lemma safe_library_snoc_implies {o} :
  forall (lib : @pre_library o) (e : library_entry),
    safe_library (snoc lib e)
    ->
    (
      safe_library lib
...
Lemma in_implies_entry_in_library {o} :
  forall (x e : @library_entry o) (lib : pre_library),
    matching_entries e x
    -> List.In e lib
...
Lemma lib_extends_preserves_non_shadowed_entry {o} :
  forall inh (e : @library_entry o) lib1 lib2,
    lib_extends inh lib2 lib1
    -> non_shadowed_entry e lib2
    -> non_shadowed_entry e lib1.
Proof.
  introv ext nsh.
  apply implies_lib_extends_ext in ext; eauto 3 with slow.
Qed.
...
Lemma add_choice_implies3 {o} :
  forall name v (lib : @pre_library o) n lib',
    add_choice name v lib = (n, lib')
    -> (lib = lib' /\ !in_lib (entry_name_cs name) lib)
       \/
       exists vals,
         length vals = n
         /\ entry_in_library (lib_cs name vals) lib
         /\ entry_in_library (lib_cs name (snoc vals v)) lib'
         /\ forall lib0, add_choice name v (lib ++ lib0) = (n, lib' ++ lib0).
Proof.
  induction lib; introv add; simpl in *; tcsp.

  { inversion add; subst; simpl in *; tcsp.
    left; dands; tcsp.
    unfold in_lib; simpl; introv xx; exrepnd; tcsp. }

  destruct a; boolvar; subst; simpl in *; repndors; repnd; tcsp; subst; simpl in *; tcsp.

  { inversion add; subst; clear add; simpl in *.
    right.
    exists vals; dands; tcsp. }

  { remember (add_choice name v lib) as w; symmetry in Heqw; repnd; inversion add; subst; clear add.
    pose proof (IHlib n w) as IHlib; repeat (autodimp IHlib hyp).
    repndors; repnd; subst; tcsp.

    { left; dands; tcsp.
      introv xx; destruct IHlib; unfold in_lib in *; exrepnd; simpl in *; repndors; subst; tcsp.
      exists e; dands; auto. }

    { exrepnd.
      right; exists vals0; dands; tcsp.
      introv.
      pose proof (IHlib0 lib0) as IHlib0.
      allrw; simpl; tcsp. } }
  ...
  { remember (add_choice name v lib) as w; symmetry in Heqw; repnd; inversion add; subst; clear add.
    ...
    { left; dands; tcsp.
      introv xx; destruct IHlib; unfold in_lib in *; exrepnd; simpl in *; repndors; subst; tcsp; simpl in *; tcsp.
      exists e; dands; auto. }

    { exrepnd.
      right; exists vals; dands; tcsp.
      introv.
      pose proof (IHlib0 lib0) as IHlib0.
      allrw; simpl; tcsp. } }
Qed.
...
Proof.
  ...
  { autodimp IHext1 hyp; autodimp IHext2 hyp; eauto 3 with slow. }

  { apply lib_extends_new_cs; auto.
    allrw @non_shadowed_entry_cons; repnd; simpl in *.
    introv xx; destruct ni.
    unfold in_lib in *; exrepnd.
    allrw @list_in_snoc; repndors; subst; tcsp.
    { exists e0; dands; auto. }
    unfold matching_entries in *; simpl in *; destruct allt0.
    remember (entry2name e) as w; destruct w; subst; tcsp. }

  { applydup @add_choice_implies3 in adc.
    repndors; repnd; subst; tcsp; eauto 3 with slow.
    exrepnd.
    pose proof (adc1 [e]) as adc1.
    allrw <- snoc_as_append.
    eapply (lib_extends_cs _ name v n M d Md _); auto. }
  ...
  { applydup @add_choice_implies3 in adc.
    ...
    pose proof (adc1 [e]) as adc1.
    allrw <- snoc_as_append.
    eapply (lib_extends_law _ name (f n) n f _); auto. }

  { assert (lib_extends lib' lib) as xt by eauto.
    applydup @add_choice_implies3 in adc.
    repndors; repnd; subst; tcsp; eauto 3 with slow.
    exrepnd.
    pose proof (adc1 [e]) as adc1.
    allrw <- snoc_as_append.
    eapply (lib_extends_res _ name v n M _ Q e0 p); auto.
    autodimp IHext hyp; eauto 3 with slow;[].
    eapply lib_extends_trans;[eauto|].

    eapply lib_extends_preserves_non_shadowed_entry in allt;[|exact xt].
    eapply lib_extends_preserves_non_shadowed_entry in allt;[|exact ext].
  }
...
Lemma non_shadowed_and_in_library_implies_non_matching {o} :
  forall e entry (lib : @pre_library o),
    non_shadowed_entry e lib
    -> entry_in_library entry lib
...
Proof.
  ...
  apply nsh in i1.
  unfold diff_entry_names in i1; boolvar; tcsp.
Qed.
...
Lemma choice_sequence_satisfies_restriction_snoc_implies {o} :
  forall inh (vals : @ChoiceSeqVals o) v restr,
    choice_sequence_satisfies_restriction inh (snoc vals v) restr
    -> choice_sequence_satisfies_restriction inh vals restr.
Proof.
  introv h.
  unfold choice_sequence_satisfies_restriction in *.
  destruct restr; simpl in *; introv q; eauto.
  { apply h.
...
(* This definition could more fine-grained by allowing the values
   to be part of the extended library ([lib] here)... *)
Definition safe_choice_seq_restriction_upto {o}
           (inh   : InhAt)
           (vals  : @ChoiceSeqVals o)
  ...
  | csc_res Q =>
    forall n v,
      ...
      -> inh lib (mkc_apply2 Q (mkc_nat n) v)
  | _ => True
  end.

Definition safe_choice_sequence_entry_upto {o}
           inh
           (name  : choice_sequence_name)
           (vals  : @ChoiceSeqVals o)
           (restr : @ChoiceSeqRestriction o)
  ...
  correct_restriction name restr
  /\ choice_sequence_satisfies_restriction inh vals restr
  /\ safe_choice_seq_restriction_upto inh vals restr lib.

Definition safe_library_entry_upto {o} inh (e : @library_entry o) (lib : @library o) :=
  match e with
  | lib_cs name vals =>
    match find_restriction (lib_res lib) name with
    | Some restr => safe_choice_sequence_entry_upto inh name vals restr lib
    | None => False
    end
  | _ => True
  end.

Lemma safe_choice_seq_restriction_upto_snoc_implies {o} :
  forall inh (vals : @ChoiceSeqVals o) v restr lib,
    safe_choice_seq_restriction_upto inh (snoc vals v) restr lib
    -> safe_choice_seq_restriction_upto inh vals restr lib.
Proof.
  introv h.
  unfold safe_choice_seq_restriction_upto in *.
  destruct restr; simpl in *; auto.
  introv sel.
  pose proof (h n v0) as h.
  rewrite select_snoc_eq in h; boolvar; tcsp; subst; try omega.
  { rewrite implies_select_none in sel; auto; ginv. }
  apply select_lt in sel; try omega.
Qed.
Hint Resolve safe_choice_seq_restriction_upto_snoc_implies : slow.

(* add_cs in computation_lib_extends is an instance of this: *)
Definition add_cs_entry {o}
           (name : choice_sequence_name)
           (vals : @ChoiceSeqVals o)
           (r    : ChoiceSeqRestriction)
           (lib  : @library o) : library :=
  MkLibrary
    (lib_cs name vals :: lib_lib lib)
    (MkRes name r :: lib_res lib).

Lemma safe_choice_sequence_entry_upto_implies_correct_restriction {o} :
  forall inh name (vals : @ChoiceSeqVals o) r lib,
    safe_choice_sequence_entry_upto inh name vals r lib
    -> correct_restriction name r.
Proof.
  introv h.
  unfold safe_choice_sequence_entry_upto in h; tcsp.
Qed.
Hint Resolve safe_choice_sequence_entry_upto_implies_correct_restriction : slow.

Lemma safe_choice_sequence_entry_upto_snoc_implies {o} :
  forall inh name (vals : @ChoiceSeqVals o) a r lib,
    safe_choice_sequence_entry_upto inh name (snoc vals a) r lib
    -> safe_choice_sequence_entry_upto inh name vals r lib.
Proof.
  introv safe.
  unfold safe_choice_sequence_entry_upto in *; repnd; dands; eauto 3 with slow.
Qed.
Hint Resolve safe_choice_sequence_entry_upto_snoc_implies : slow.

Lemma safe_library_add_abs_implies {o} :
...
Proof.
  introv ni safe i; simpl in *.
  apply safe; simpl.
  unfold matching_entries; simpl.
  destruct entry in *; simpl in *; tcsp.
  destruct (same_opabs_dec opabs op) as [d|d]; tcsp.
  destruct ni.
  eapply entry_in_library_implies_in_lib_abs in i; simpl in *; eauto.
  unfold in_lib in *; exrepnd.
  eexists; dands; eauto.
  destruct e; simpl in *; tcsp.
  apply same_opabs_sym in d; eapply same_opabs_trans; eauto.
Qed.
Hint Resolve safe_library_add_abs_implies : slow.

Lemma implies_entry_in_library_add_cs {o} :
  forall name restr entry (lib : @library o),
    !in_lib (entry_name_cs name) lib
    -> entry_in_library entry lib
    -> entry_in_library entry (add_cs name restr lib).
Proof.
  introv ni i.
  unfold add_cs; simpl; right; dands; tcsp.
  introv h.
  destruct entry; simpl in *; tcsp.
  unfold matching_entries in h; simpl in *; subst; tcsp.
  destruct ni.
  apply entry_in_library_implies_in in i.
  eexists; dands; eauto; tcsp.
Qed.
Hint Resolve implies_entry_in_library_add_cs : slow.

Lemma safe_library_add_cs_implies {o} :
...
Proof.
  ...
  eexists; dands; eauto; tcsp.
Qed.
Hint Resolve safe_library_add_cs_implies : slow.

Lemma safe_choice_sequence_entry_snoc_implies {o} :
  forall inh  name (vals : @ChoiceSeqVals o) v restr,
    safe_choice_sequence_entry inh name (snoc vals v) restr
    -> safe_choice_sequence_entry inh name vals restr.
Proof.
  introv safe.
  unfold safe_choice_sequence_entry in *; repnd; dands; auto.
  unfold choice_sequence_satisfies_restriction in *.
  destruct restr; simpl in *; tcsp; introv sel.

  { applydup @select_lt in sel.
    apply safe; rewrite select_snoc_eq; boolvar; subst; tcsp; try omega. }

  { pose proof (safe i) as safe; rewrite length_snoc in safe; autodimp safe hyp.
    rewrite select_snoc_eq in safe; boolvar; tcsp. }

  { applydup @select_lt in sel.
    apply safe; rewrite select_snoc_eq; boolvar; subst; tcsp; try omega. }
Qed.
Hint Resolve safe_choice_sequence_entry_snoc_implies : slow.

Lemma add_choice_csc_coq_law_preserves_safe_rev {o} :
...
Proof.
  ...
  eapply add_choice_implies in add; eauto.
  repndors; exrepnd; subst; tcsp; simpl in *; tcsp; ginv.
  allrw.
  apply safe in add0; simpl in *; exrepnd.
  rewrite findr in *; eauto 3 with slow.
Qed.
Hint Resolve add_choice_csc_coq_law_preserves_safe_rev : slow.

Lemma add_choice_csc_type_preserves_safe_rev {o} :
...
Lemma add_choice_csc_res_preserves_safe_rev {o} :
  forall inh name v (lib : @pre_library o) n Q lib' R,
    add_choice name v lib = (n, lib')
    -> find_restriction R name = Some (csc_res Q)
    -> (exists l, inh l (mkc_apply2 Q (mkc_nat n) v))
    -> safe_library inh (MkLibrary lib' R)
    -> safe_library inh (MkLibrary lib R).
Proof.
  ...
  rewrite findr in *; eauto 3 with slow.
Qed.
Hint Resolve add_choice_csc_res_preserves_safe_rev : slow.

Lemma lib_extends_preserves_safe_rev {o} :
...
Proof.
  introv ext.
  lib_ext_ind ext Case; introv safe; auto; eauto 3 with slow;
    try (complete (remember (M n v) as q; exrepnd; eauto 3 with slow)).
Qed.
Hint Resolve lib_extends_preserves_safe : slow.

Lemma add_choice_preserves_in_lib {o} :
  forall (lib : @pre_library o) name v n lib' name',
    add_choice name v lib = (n, lib')
    -> in_lib name' lib
    -> in_lib name' lib'.
Proof.
  introv addc i.
  apply add_choice_preserves_subset_library in addc.
  unfold in_lib in *; exrepnd.
  apply addc in i1; exrepnd.
  eexists; dands; eauto.
  destruct entry2, e; simpl in *; repnd; subst; ginv; inversion i2; subst; auto.
Qed.
Hint Resolve add_choice_preserves_in_lib : slow.

Lemma lib_extends_preserves_in_lib {o} :
  forall inh (lib1 lib2 : @library o) name,
    lib_extends inh lib2 lib1
    -> in_lib name lib1
    -> in_lib name lib2.
Proof.
  introv ext; lib_ext_ind ext Case.
Qed.
Hint Resolve lib_extends_preserves_in_lib : slow.
...
Lemma lib_extends_preserves_not_in_lib {o} :
  forall inh (lib1 lib2 : @library o) name,
    ...
    -> !in_lib name lib1.
Proof.
  introv ext ni i; destruct ni; eauto 3 with slow.
Qed.
Hint Resolve lib_extends_preserves_not_in_lib : slow.

Lemma found_entry_sign_implies_in_lib {o} :
  forall (lib : @pre_library o) name,
    found_entry_sign lib name = true
    -> in_lib name lib.
Proof.
  induction lib; introv h; simpl in *; ginv; boolvar; tcsp; ginv.
  { exists a; simpl; dands; tcsp. }
  apply IHlib in h; eauto 3 with slow.
Qed.

Definition InhAt_mono {o} inh :=
  forall (lib' lib : @library o) P,
    lib_extends inh lib' lib -> inh lib P -> inh lib' P.

Lemma lib_extends_cons_new {o} :
  forall inh (lib : @library o) name vals r,
    InhAt_mono inh
    -> safe_choice_sequence_entry_upto inh name vals r lib
    -> !in_lib (entry_name_cs name) lib
    -> lib_extends inh (add_cs_entry name vals r lib) lib.
Proof.
  introv mon safe ni.
  unfold add_cs_entry.

  induction vals using rev_list_indT; simpl in *; eauto.
  { apply lib_extends_new_cs; eauto 3 with slow. }

  repeat (autodimp IHvals hyp); eauto 3 with slow;[].

  eapply lib_extends_trans;[|exact IHvals].
  unfold safe_choice_sequence_entry_upto in *; repnd.
  destruct r; simpl in *; eauto; GC.

  { apply (lib_extends_cs _ _ name a (length vals) typ d typd); simpl; boolvar; tcsp.
    apply safe1.
    rewrite select_snoc_eq; boolvar; tcsp; try omega. }

  { apply (lib_extends_law _ _ name a (length vals) f); simpl; boolvar; tcsp.
    pose proof (safe1 (length vals)) as safe1; allrw length_snoc.
    autodimp safe1 hyp.
    rewrite select_snoc_eq in safe1; boolvar; tcsp; try omega.
    inversion safe1; auto. }

  { pose proof (safe (length vals) a) as safe'.
    rewrite select_snoc_eq in safe'; boolvar; subst; try omega;[].
    autodimp safe' hyp; exrepnd.
    ...
    pose proof (lib_extends_res
                 inh
                  ...
                  (MkRes name (csc_res P) :: lib_res lib)
               ) as w.
...
(*Lemma implies_lib_extends_cons_left {o} :
  forall e (lib lib2 : @library o),
    lib_extends lib lib2
    -> safe_library_entry_upto e lib
    -> non_shadowed_entry e lib
    -> lib_extends (e :: lib) lib2.
Proof.
  introv ext safee allt.
  eapply lib_extends_trans;[apply lib_extends_cons_new|]; auto.
Qed.
...
Lemma entry_in_inf_library_shift_implies {o} :
  forall inh (inflib : @inf_library o) e,
    ~ matching_inf_entries (inflib 0) e
    -> entry_in_inf_library inh e (shift_inf_lib inflib)
    -> entry_in_inf_library inh e inflib.
Proof.
  introv ni h.
...
Lemma inf_library_extends_implies {o} :
  forall inh (e : @library_entry o) n inflib,
    entry_in_inf_library_extends e n inflib
    -> exists k,
        k < n
        /\ inf_entry_extends (inflib k) e
        /\ entry_in_inf_library inh (inflib k) inflib
...
Lemma implies_entry_in_inf_library {o} :
  forall inh n (infLib : @inf_library o),
    (forall m, m < n -> ~ matching_inf_entries (infLib m) (infLib n))
    -> entry_in_inf_library inh (infLib n) infLib.
Proof.
  introv imp.
  left; exists (S n).
...
(*Lemma matching_entries_preserves_entry_in_inf_library_default {o} :
  forall (e1 e2 : @library_entry o) n infLib,
    matching_entries e1 e2
    -> entry_in_inf_library_default e1 infLib
...
Lemma correct_restriction_choice_sequence_name2restriction {o} :
  forall name, @correct_restriction o name (choice_sequence_name2restriction name).
Proof.
  introv.
...
Lemma safe_inf_library_entry_choice_sequence_name2inf_entry {o} :
  forall inh name, @safe_inf_library_entry o inh (choice_sequence_name2inf_entry name).
Proof.
  introv.
...
Lemma inf_entry_in_inf_library_default_choice_sequence_name2inf_entry {o} :
  forall inh name (inflib : @inf_library o),
    is_nat_or_seq_kind name
    -> ~ (exists n, same_entry_name (inf_entry2name (inflib n)) (entry_name_cs name))
    -> inf_entry_in_inf_library_default inh (choice_sequence_name2inf_entry name) inflib.
Proof.
  introv ins h.
  unfold inf_entry_in_inf_library_default; dands; eauto 3 with slow;[].
...
Definition choice_sequence_name2entry_upto {o}
           (n     : nat)
           (name  : choice_sequence_name) : @library_entry o :=
  lib_cs name (choice_sequence_name2choice_seq_vals_upto n name).

(*Lemma entry_in_inf_library_default_two_matching_entries {o} :
  forall (e1 e2 : @library_entry o) inflib,
    matching_entries e1 e2
    -> entry_in_inf_library_default e1 inflib
    -> entry_in_inf_library_default e2 inflib
    -> exists name e,
...
Proof.
  introv m h q.
  unfold entry_in_inf_library_default in *; exrepnd.
  destruct e1 as [name1 e1|], e2 as [name2 e2|]; simpl in *;tcsp;repnd.
...
(*Lemma entry_in_inf_library_default_implies_exists {o} :
...
(*Lemma entry_in_inf_library_default_implies_exists2 {o} :
  forall infLib (lib : @library o) e,
    inf_lib_extends infLib lib
    -> forallb (diff_entry_names e) lib = false
    -> entry_in_inf_library_default e infLib
...
Proof.
  ...
  { eapply matching_entries_preserves_entry_in_inf_library_default in i;
      [apply i in allf4;tcsp|]; eauto 3 with slow. }
  apply entry_in_inf_library_default_implies_exists in allf3; exrepnd; subst.
  simpl in *; GC.
    ...
    + (* [a] is in [lib] *)
      ...
      * (* [a] is in [lib2] *)
        ...
        { (* [a] is not in [lib1] *)
          ...
          {
            eapply matching_entries_preserves_entry_in_inf_library_default in w0;
              [| |eauto]; eauto 3 with slow;tcsp.
          }

          {
            eapply matching_entries_preserves_entry_in_inf_library_default in q2;
              [| |eauto]; eauto 3 with slow;tcsp.
          }

          {
            pose proof (entry_in_inf_library_default_implies_exists2 infLib lib2 a) as z.
            repeat (autodimp z hyp); exrepnd.
            apply entry_in_inf_library_default_implies_exists in q.
            exrepnd.
        ...
        { (* [a] is in [lib1] *)
          ...
          {
            pose proof (entry_in_inf_library_default_implies_exists e2 infLib) as z.
            repeat (autodimp z hyp); exrepnd; eauto 3 with slow.

            pose proof (entry_in_inf_library_default_implies_exists e1 infLib) as u.
  ...
  forall infLib (lib : @library o) e,
    ...
    -> (exists a k,
       ...
       \/ (exists a,
              entry_in_library a lib
              /\ matching_entries a e
              /\ entry_in_inf_library_default a infLib).
Proof.
  introv allt ext.
  destruct ext as [ext safe].
...
(*Lemma entry_in_inf_library_default_implies_safe {o} :
  forall (entry : @library_entry o) infLib,
    ...
    -> safe_library_entry entry.
Proof.
  introv h; unfold entry_in_inf_library_default in h; tcsp.
Qed.
...
(*Lemma entry_in_inf_library_default_implies_exists3 {o} :
  forall infLib (lib : @library o) e,
    inf_lib_extends_ext_entries infLib lib
    -> forallb (diff_entry_names e) lib = false
    -> entry_in_inf_library_default e infLib
...
Proof.
  ...
  unfold entry_in_inf_library_default; dands; eauto 3 with slow.

  introv xx.
  unfold entry_in_inf_library_default in ee; repnd.
  ...
  - pose proof (IHlib1 lib2) as q; repeat (autodimp q hyp); eauto 2 with slow; clear IHlib1.
    ...
    + (* [a] is not in [lib] *)
      ...
      * (* [a] is in [lib1] *)
        ...
        repeat (autodimp q hyp); repndors; exrepnd.
    ...
    + (* [a] is in [lib] *)
      ...
      * (* [a] is in [lib2] *)
        ...
        { (* [a] is not in [lib1] *)
          ...
          {
            eapply matching_entries_preserves_entry_in_inf_library_default in q3;
              [| |eauto]; eauto 3 with slow;tcsp.
          }

          {
            pose proof (entry_in_inf_library_default_implies_exists3 infLib lib2 a) as z.
            repeat (autodimp z hyp); exrepnd;[].
            apply entry_in_inf_library_default_implies_exists in q.
            exrepnd.
            rewrite q5 in z2; ginv; simpl in *.
            exists ((choice_sequence_name2entry_upto (Peano.max n n0) name restr) :: lib); dands; eauto 5 with slow.
          }
        }
...
Lemma BarLibExt_refl {o} :
  forall inh (lib : @library o), BarLibExt inh (const_bar lib) lib.
Proof.
...
Fixpoint name_in_library {o} (name : choice_sequence_name) (lib : @pre_library o) : Prop :=
  match lib with
  | [] => False
  | entry :: entries =>
...
Lemma entry_in_inf_library_shift_inf_lib_implies {o} :
  forall inh (entry : @inf_library_entry o) inflib,
    entry_in_inf_library inh entry (shift_inf_lib inflib)
    -> ~ same_entry_name (inf_entry2name (inflib 0)) (inf_entry2name entry)
    -> entry_in_inf_library inh entry inflib.
Proof.
  introv i d.
...
Lemma all_choice_sequence_names_in_inf_lib {o} :
  forall inh (inflib : @inf_library o) (name : choice_sequence_name),
    is_nat_or_seq_kind name
    ->
    exists entry,
      entry_in_inf_library inh entry inflib
...
Proof.
  ...
  {
    exrepnd.
    pose proof (nth_entry_in_inf_library inh n inflib) as q.
...
Inductive extend_choice_seq_vals_lawless_upto {o} (n : nat) :
  @ChoiceSeqVals o -> @ChoiceSeqVals o -> RestrictionPred -> Prop :=
| ext_choice_seq_vals_lawless_upto_type :
    forall vals vals' (M : RestrictionPred),
      length vals' = n - length vals
      -> (forall k v, select k vals' = Some v -> M (length vals + k) v)
      -> extend_choice_seq_vals_lawless_upto n (vals ++ vals') vals M.
Hint Constructors extend_choice_seq_vals_lawless_upto.

(*Definition old_extend_choice_seq_entry_lawless_upto {o} (e1 e2 : @ChoiceSeqEntry o) (n : nat) : Prop :=
  match e1, e2 with
  ...
  | lib_cs name1 vals1, lib_cs name2 vals2 =>
    if choice_sequence_name_deq name name1 then
      if choice_sequence_name_deq name name2 then
        extend_choice_seq_vals_lawless_upto n vals1 vals2 M
      else False
    else e1 = e2
  | _, _ => e1 = e2
  end.
...
(* [lib1] extend [lib2] *)
Fixpoint extend_pre_library_lawless_upto {o}
         (lib1 : @pre_library o)
         (lib2 : @pre_library o)
         (M    : @RestrictionPred o)
         (name : choice_sequence_name)
  ...
  | entry1 :: entries1, entry2 :: entries2 =>
    extend_library_entry_lawless_upto entry1 entry2 M name n
    /\ extend_pre_library_lawless_upto entries1 entries2 M name n
  | _, _ => False
  end.

(* [lib1] extend [lib2] *)
Inductive extend_library_lawless_upto {o} : @library o -> @library o -> choice_sequence_name -> nat -> Prop :=
| ext_lib_lawless_upto :
    forall lib1 lib2 R d M Md name n,
      find_restriction R name = Some (csc_type d M Md)
      -> extend_pre_library_lawless_upto lib1 lib2 M name n
      -> extend_library_lawless_upto (MkLibrary lib1 R) (MkLibrary lib2 R) name n.

Definition add_def_cs {o} (name : choice_sequence_name) (lib : @library o) : library :=
  add_cs name (choice_sequence_name2restriction name) lib.

Definition add_cs_if_not_in {o} name (lib : @library o) : library :=
  match find_cs lib name with
  | Some e => lib
  | None => add_def_cs name lib
  end.
...
Lemma choice_sequence_satisfies_restriction_nil {o} :
  forall inh r, @choice_sequence_satisfies_restriction o inh [] r.
Proof.
  introv.
  unfold choice_sequence_satisfies_restriction.
...
Lemma safe_choice_seq_restriction_upto_nil {o} :
  forall inh (restr : @ChoiceSeqRestriction o) lib,
    safe_choice_seq_restriction_upto inh [] restr lib.
Proof.
  introv; destruct restr; simpl; auto.
  introv sel; autorewrite with list in *; ginv.
Qed.
Hint Resolve safe_choice_seq_restriction_upto_nil : slow.
...
Proof.
  ...
  { apply IHlib in fcs; clear IHlib; destruct fcs.
    unfold in_lib in *; simpl in *; exrepnd; repndors; subst; simpl in *; subst; tcsp; eauto. }
Qed.
Hint Resolve find_cs_non_implies_not_in_lib : slow.

Lemma safe_choice_sequence_entry_upto_nil {o} :
  forall inh name (restr : @ChoiceSeqRestriction o) lib,
    correct_restriction name restr
    -> safe_choice_sequence_entry_upto inh name [] restr lib.
Proof.
  introv cor.
  unfold safe_choice_sequence_entry_upto; simpl; dands; eauto 3 with slow.
Qed.
Hint Resolve safe_choice_sequence_entry_upto_nil : slow.

Lemma find_cs_none_implies_lib_extends_choice_sequence_name2entry {o} :
  forall inh name (lib : @library o),
    InhAt_mono inh
    -> find_cs lib name = None
    -> lib_extends inh (add_def_cs name lib) lib.
Proof.
  introv mon f.
  unfold add_def_cs.
  apply lib_extends_cons_new; eauto 3 with slow.
Qed.
...
Lemma lib_extends_add_cs_if_not_in_name {o} :
  forall inh name (lib : @library o),
    InhAt_mono inh
    -> lib_extends inh (add_cs_if_not_in name lib) lib.
Proof.
  introv mon.
...
Lemma find_cs_some_implies_name_in_library {o} :
  forall name (lib : @pre_library o) c,
...
(*Lemma inf_entry_in_inf_library_default_implies_entry_in_inf_library_default {o} :
  forall (entry : @inf_library_entry o) e infLib,
    inf_entry_in_inf_library_default entry infLib
    -> inf_entry_extends entry e
    -> entry_in_inf_library_default e infLib.
Proof.
  introv h q.
...
Qed.
Hint Resolve inf_entry_in_inf_library_default_implies_entry_in_inf_library_default : slow.*)
...
Lemma correct_restriction_implies_same_restrictions2 {o} :
...
Lemma extend_choice_seq_vals_lawless_upto_implies_choice_sequence_vals_extend {o} :
  forall (vals1 vals2 : @ChoiceSeqVals o) n M,
    extend_choice_seq_vals_lawless_upto n vals1 vals2 M
    -> choice_sequence_vals_extend vals1 vals2.
Proof.
  introv h.
  inversion h; subst; eauto 3 with slow.
Qed.
Hint Resolve extend_choice_seq_vals_lawless_upto_implies_choice_sequence_vals_extend : slow.

Lemma extend_library_entry_lawless_upto_implies_entry_extends {o} :
  forall (entry1 entry2 : @library_entry o) M name n,
    extend_library_entry_lawless_upto entry1 entry2 M name n
...
Lemma extend_pre_library_lawless_upto_preserves_entry_in_library_extends {o} :
  forall entry (lib1 lib2 : @pre_library o) M name n,
    extend_pre_library_lawless_upto lib1 lib2 M name n
...
Qed.
Hint Resolve extend_pre_library_lawless_upto_preserves_entry_in_library_extends : slow.
...
Proof.
  introv ext i.
  inversion ext; subst; eauto 3 with slow.
Qed.
...
Lemma extend_library_entry_lawless_upto_preserves_matching_entries_lr {o} :
  forall (e1 e2 e : @library_entry o) M name n,
    extend_library_entry_lawless_upto e1 e2 M name n
    -> matching_entries e1 e
    -> matching_entries e2 e.
Proof.
...
Lemma extend_pre_library_lawless_upto_implies {o} :
  forall (lib1 lib2 : @pre_library o) M name n entry,
    extend_pre_library_lawless_upto lib1 lib2 M name n
    -> entry_in_library entry lib1
    ->
    exists e,
      entry_in_library e lib2
      /\ extend_library_entry_lawless_upto entry e M name n.
Proof.
...
(*Lemma implies_preserves_correct_restriction_name_csc_type {o} :
  forall name d1 d2 (M1 M2 : @RestrictionPred o) Md1 Md2,
    ...
    -> correct_restriction name (csc_type d1 M1 Md1)
    -> correct_restriction name (csc_type d2 M2 Md2).
Proof.
  introv h w cor.
  destruct name as [name kind], kind as [n|seq]; simpl in *; tcsp.

  - unfold correct_restriction in *; simpl in *; boolvar; tcsp.
    repnd; dands; introv.
    { rewrite <- h; auto. }
    { rewrite <- w; auto. }
...
Lemma extend_library_entry_lawless_upto_preserves_safe_library {o} :
  forall inh (entry1 entry2 : @library_entry o) R d M Md name n,
    find_restriction R name = Some (csc_type d M Md)
    -> extend_library_entry_lawless_upto entry1 entry2 M name n
    -> safe_library_entry inh R entry2
    -> safe_library_entry inh R entry1.
Proof.
  introv findr ext safe; destruct entry1, entry2; simpl in *; tcsp;
    boolvar; repeat subst; tcsp; ginv; tcsp.
  rewrite findr in *; simpl in *.

  inversion ext as [? ? ? eql ext0]; subst; simpl in *; dands; repnd; tcsp.
  unfold safe_choice_sequence_entry in *; repnd; dands; auto.

  introv i.

  destruct (lt_dec n0 (length vals0)) as [z|z].

  { rewrite select_app_left in i; auto. }

  { apply Nat.nlt_ge in z.
    rewrite select_app_r in i; try omega; auto.
    apply ext0 in i.
...
Lemma extend_library_lawless_upto_preserves_safe_library {o} :
  forall inh (lib1 lib2 : @library o) name n,
    extend_library_lawless_upto lib1 lib2 name n
    -> safe_library inh lib2
    -> safe_library inh lib1.
Proof.
  introv ext safe i.
  destruct lib1 as [lib1 R1], lib2 as [lib2 R2].
  inversion ext as [? ? ? ? ? ? ? ? findr ext0]; subst; simpl in *; clear ext.
  eapply extend_pre_library_lawless_upto_implies in ext0;[|eauto].
  exrepnd.
  apply safe in ext0; eauto 2 with slow.
Qed.
Hint Resolve extend_library_lawless_upto_preserves_safe_library : slow.

Lemma extend_pre_library_lawless_upto_implies_subset_library {o} :
  forall (lib1 lib2 : @pre_library o) M name n,
    extend_pre_library_lawless_upto lib1 lib2 M name n
...
Proof.
  ...
  - applydup ext1 in i; exrepnd.
    exists entry2; dands; tcsp.
Qed.
Hint Resolve extend_pre_library_lawless_upto_implies_subset_library : slow.
...
Proof.
  introv ext.
  inversion ext as [? ? ? ? ? ? ? ? findr ext0]; eauto 3 with slow.
Qed.
Hint Resolve extend_library_lawless_upto_implies_subset_library : slow.

Lemma in_extend_library_lawless_upto_implies {o} :
  forall (lib1 lib2 : @pre_library o) M name n entry,
    extend_pre_library_lawless_upto lib2 lib1 M name n
    -> List.In entry lib2
    -> exists entry',
        List.In entry' lib1
        /\ extend_library_entry_lawless_upto entry entry' M name n.
Proof.
  induction lib1; introv ext i; simpl in *; destruct lib2; simpl in *; tcsp.
  repnd.
  repndors; subst; tcsp.
...
Lemma implies_extend_library_lawless_upto_app {o} :
  forall name n (lib1 lib1' lib2 lib2' : @pre_library o) M,
    extend_pre_library_lawless_upto lib1' lib1 M name n
    -> extend_pre_library_lawless_upto lib2' lib2 M name n
    -> extend_pre_library_lawless_upto (lib1' ++ lib2') (lib1 ++ lib2) M name n.
Proof.
...
Lemma extend_library_lawless_upto_preserves_in_lib {o} :
  forall (lib1 lib2 : @library o) name n k,
    extend_library_lawless_upto lib1 lib2 name n
    -> in_lib k lib1
    -> in_lib k lib2.
Proof.
  introv ext i; unfold in_lib in *; exrepnd.
  destruct lib1 as [lib1 R1], lib2 as [lib2 R2]; simpl in *.
  inversion ext as [? ? ? ? ? ? ? ? findr ext0]; subst; clear ext.
  eapply in_extend_library_lawless_upto_implies in i1; eauto.
  exrepnd.
  eexists; dands; eauto.
  apply extend_library_entry_lawless_upto_implies_entry_extends in i2.
  destruct e, entry'; simpl in *; repnd; subst; tcsp; ginv.
  inversion i2; subst; tcsp.
Qed.

(*Fixpoint sane_library {o} (lib : @pre_library o) :=
  match lib with
  | [] => True
  | entry :: entries =>
    !in_lib (entry2name entry) entries
    /\ safe_library_entry_upto entry entries
    /\ sane_library entries
  end.*)

Fixpoint norepeats {T} (l : list T) : Prop :=
  match l with
  | [] => True
  | x :: xs => (!LIn x xs) /\ norepeats xs
  end.

Definition old_no_repeats_library {o} (lib : @library o) : Prop :=
  norepeats (map entry2name lib).

Fixpoint no_repeats_library {o} (lib : @pre_library o) : Prop :=
  match lib with
  | [] => True
  | entry :: entries => !in_lib (entry2name entry) entries /\ no_repeats_library entries
  end.

Lemma extend_pre_library_lawless_upto_not_in {o} :
  forall (lib1 lib2 : @pre_library o) M name n,
    extend_pre_library_lawless_upto lib1 lib2 M name n
    -> !LIn (entry_name_cs name) (map entry2name lib2)
    -> lib1 = lib2.
Proof.
  induction lib1; introv ext ni; destruct lib2; simpl in *; repnd; tcsp.
  apply not_over_or in ni; repnd.
  apply IHlib1 in ext; auto; subst.
  destruct a, l; simpl in *; boolvar; subst; ginv; tcsp.
Qed.

Lemma extend_library_lawless_upto_not_in {o} :
  forall (lib1 lib2 : @library o) name n,
    extend_library_lawless_upto lib1 lib2 name n
    -> !LIn (entry_name_cs name) (map entry2name lib2)
    -> lib1 = lib2.
Proof.
  ...
  destruct lib1 as [lib1 R1], lib2 as [lib2 R2]; simpl in *.
  inversion ext as [? ? ? ? ? ? ? ? findr ext0]; subst; clear ext.
  apply extend_pre_library_lawless_upto_not_in in ext0; subst; auto.
Qed.
...
Lemma not_in_lib_cs_implies_not_in_names {o} :
...
Proof.
  ...
  apply LIn_implies_In in i1.
  eexists; dands; eauto.
  destruct a; simpl in *; tcsp; ginv.
  simpl; auto.
Qed.
Hint Resolve not_in_lib_cs_implies_not_in_names : slow.

Lemma extend_pre_library_lawless_upto_no_repeats_implies_split {o} :
  forall (lib1 lib2 : @pre_library o) M name n,
    no_repeats_library lib2
    -> extend_pre_library_lawless_upto lib1 lib2 M name n
    -> (lib1 = lib2 /\ !in_lib (entry_name_cs name) lib2)
       \/
       exists liba libb vals1 vals2,
         lib1 = liba ++ lib_cs name vals1 :: libb
         /\ lib2 = liba ++ lib_cs name vals2 :: libb
         /\ !in_lib (entry_name_cs name) liba
         /\ extend_choice_seq_vals_lawless_upto n vals1 vals2 M.
Proof.
  induction lib1; introv norep ext; destruct lib2; simpl in *; repnd; tcsp.

  { left; dands; tcsp; introv xx; unfold in_lib in *; simpl in *; exrepnd; tcsp. }

  destruct a, l; simpl in *; boolvar; subst; simpl in *; tcsp; ginv.

  { apply extend_pre_library_lawless_upto_not_in in ext; auto; subst; eauto 3 with slow.
    right.
    exists ([] : @pre_library o) lib2 vals vals0; dands; tcsp.
    introv xx; unfold in_lib in xx; exrepnd; simpl in *; tcsp. }

  { apply IHlib1 in ext; auto;[].
    repndors; exrepnd; subst.

    { left; dands; tcsp.
      introv xx; unfold in_lib in xx; exrepnd; simpl in *;
        repndors; subst; simpl in *; subst; tcsp.
      destruct e; simpl in *; subst; tcsp.
      destruct ext.
      eexists; dands; eauto; simpl; auto. }

    right.
    exists (lib_cs name1 vals0 :: liba) libb vals1 vals2; simpl; dands; tcsp.
    intro xx; unfold in_lib in xx; exrepnd; simpl in *;
      repndors; subst; simpl in *; subst; tcsp.
    destruct e; simpl in *; subst; tcsp.
    destruct ext3.
    eexists; dands; eauto; simpl; auto. }

  { inversion ext0; subst.
    assert (correct = correct0) as eqcor by eauto 3 with pi; subst.
    apply IHlib1 in ext; auto;[].
    repndors; exrepnd; subst.
    ...
    right.
    exists (lib_abs opabs0 vars0 rhs0 correct0 :: liba) libb vals1 vals2; simpl; dands; tcsp.
    intro xx; unfold in_lib in xx; exrepnd; simpl in *;
      repndors; subst; simpl in *; subst; tcsp.
    destruct e; simpl in *; subst; tcsp.
    destruct ext4.
...
Lemma in_lib_cs_cons {o} :
  forall name name' e (lib : @pre_library o),
    !in_lib name (lib_cs name' e :: lib)
    <-> (name <> entry_name_cs name' /\ !in_lib name lib).
Proof.
  introv; unfold in_lib; simpl; split; intro h; repnd; dands; intro q; subst; tcsp.

  { destruct h; exists (lib_cs name' e); dands; tcsp. }

  { exrepnd; destruct h.
    exists e0; dands; tcsp. }

  { exrepnd; repndors; subst; simpl in *; destruct h; tcsp.
...
Lemma in_lib_cs_abs_cons {o} :
  forall name op vars rhs correct (lib : @pre_library o),
    !in_lib (entry_name_cs name) (lib_abs op vars rhs correct :: lib)
    <-> !in_lib (entry_name_cs name) lib.
Proof.
  introv; unfold in_lib; simpl; split; intro h; repnd; dands; intro q; subst; tcsp;
    exrepnd; repndors; subst; simpl in *; tcsp; destruct h; exists e; dands; tcsp.
Qed.

Lemma add_choice_if_not_in_left {o} :
  forall (liba libb : @pre_library o) name a vals,
    !in_lib (entry_name_cs name) liba
    -> add_choice name a (liba ++ lib_cs name vals :: libb)
       = (length vals, liba ++ lib_cs name (snoc vals a) :: libb).
Proof.
  induction liba; introv ni; simpl in *; tcsp.

  { boolvar; tcsp. }

  destruct a; simpl in *; tcsp; boolvar; subst; tcsp.

  { apply in_lib_cs_cons in ni; repnd; tcsp. }

  { apply in_lib_cs_cons in ni; repnd.
    rewrite IHliba; auto. }

  { apply in_lib_cs_abs_cons in ni.
    rewrite IHliba; auto. }
Qed.
...
Proof.
  introv norep ext.
  inversion ext as [? ? ? ? ? ? ? ? findr ext0]; subst; clear ext.
  apply extend_pre_library_lawless_upto_no_repeats_implies_split in ext0; auto;[].
  repndors; exrepnd; subst; auto;[].

  inversion ext0 as [? ? ? eql ext1]; clear ext0; subst; simpl in *; dands; repnd; tcsp.
  clear eql.

  induction vals' using rev_list_ind; simpl in *; autorewrite with list in *; eauto.
  ...
  rewrite snoc_append_l.
  eapply lib_extends_trans;[|eauto].
  apply (lib_extends_cs _ _ name a (length (vals2 ++ vals')) M d Md); auto.
...
Definition shadowed_library {o} (lib1 lib2 : @pre_library o) : Prop :=
  forall entry,
    List.In entry lib1
...
Fixpoint can_lib {o} (lib : @pre_library o) : pre_library * pre_library :=
  match lib with
  | [] => ([], [])
  | entry :: entries =>
...
Lemma can_lib_preserves_list_in {o} :
  forall entry (lib : @pre_library o) lib1 lib2,
...
Fixpoint canonicalize_library {o} (lib : @pre_library o) : pre_library :=
  match lib with
  | [] => []
  | entry :: entries =>
...
Fixpoint is_canonical_library {o} (lib : @pre_library o) : Prop :=
  match lib with
  | [] => True
  | entry :: entries =>
...
Lemma sublist_preserves_is_canonical_library {o} :
  forall lib2 (lib1 : @pre_library o),
    sublist lib1 lib2
...
Lemma can_lib_implies_is_canonical {o} :
  forall (lib : @pre_library o) lib1 lib2,
    can_lib lib = (lib1,lib2)
    -> is_canonical_library lib1.
Proof.
  introv can.
...
Lemma implies_entry_in_library_app_if_left {o} :
  forall entry (lib1 lib2 : @pre_library o),
    entry_in_library entry lib1
    -> entry_in_library entry (lib1 ++ lib2).
Proof.
...
Lemma shadowed_library_nil_l {o} :
  forall (lib : @pre_library o), shadowed_library [] lib.
Proof.
  introv i; simpl in *; tcsp.
Qed.
...
Lemma extend_library_entry_lawless_upto_preserves_matching_entries2 {o} :
  forall (entry entry1 entry2 : @library_entry o) M name n,
    extend_library_entry_lawless_upto entry1 entry2 M name n
...
Definition extend_pre_library_lawless_upto_preserves_shadowed {o} :
  forall M1 M2 name n (lib2 lib2' lib1 lib1' : @pre_library o),
    shadowed_library lib2 lib1
    -> extend_pre_library_lawless_upto lib1' lib1 M1 name n
    -> extend_pre_library_lawless_upto lib2' lib2 M2 name n
    -> shadowed_library lib2' lib1'.
Proof.
...
Proof.
  introv sh ext1 ext2.
  inversion ext1 as [? ? ? ? ? ? ? ? findr1 ext1']; subst; clear ext1.
  inversion ext2 as [? ? ? ? ? ? ? ? findr2 ext2']; subst; clear ext2.
  simpl in *.
  eapply extend_pre_library_lawless_upto_preserves_shadowed; eauto.
Qed.

Lemma extend_pre_library_lawless_upto_middle_right_implies {o} :
  forall (lib1 lib2 : @pre_library o) lib entry M name n,
    extend_pre_library_lawless_upto lib (lib1 ++ entry :: lib2) M name n
    ->
    exists l1 l2 e,
      lib = l1 ++ e :: l2
      /\ extend_pre_library_lawless_upto l1 lib1 M name n
      /\ extend_pre_library_lawless_upto l2 lib2 M name n
      /\ extend_library_entry_lawless_upto e entry M name n.
Proof.
  induction lib1; introv ext; simpl in *;
    destruct lib; simpl in *; tcsp; repnd.

  - exists ([] : @pre_library o) lib l; simpl; dands; auto.
...
Lemma implies_extend_library_lawless_upto_snoc {o} :
  forall (lib1 lib2 : @pre_library o) e1 e2 M name n,
    extend_library_entry_lawless_upto e1 e2 M name n
    -> extend_pre_library_lawless_upto lib1 lib2 M name n
    -> extend_pre_library_lawless_upto (snoc lib1 e1) (snoc lib2 e2) M name n.
Proof.
...
Lemma extend_library_entry_lawless_upto_preserves_diff_entry_names_false {o} :
  forall (e1 e2 : @library_entry o) M name n e,
    extend_library_entry_lawless_upto e1 e2 M name n
...
Definition has_memNat_restriction {o} (r : @restriction o) name : Prop :=
  if choice_sequence_name_deq name (res_name r) then
    memNat_restriction (res_res r)
  else True.

Fixpoint res_has_memNat_restriction {o} (R : @restrictions o) name : Prop :=
  match R with
  | [] => True
  | r :: rs => has_memNat_restriction r name /\ res_has_memNat_restriction rs name
  end.

Definition lib_has_memNat_restriction {o} (lib : @library o) name : Prop :=
  res_has_memNat_restriction (lib_res lib) name.
...
Lemma extend_pre_library_lawless_upto_preserves_forallb_diff_entry_names_false {o} :
  forall (lib1 lib2 : @pre_library o) M name n e,
    extend_pre_library_lawless_upto lib1 lib2 M name n
...
Proof.
  introv ext h.
  inversion ext as [? ? ? ? ? ? ? ? findr ext']; subst; clear ext.
  eapply extend_pre_library_lawless_upto_preserves_forallb_diff_entry_names_false; eauto.
Qed.
...
Lemma same_restrictions_preserves_inf_choice_sequence_satisfies_restriction {o} :
  forall inh (restr1 restr2 : @ChoiceSeqRestriction o) ivals,
    same_restrictions restr1 restr2
    -> inf_choice_sequence_satisfies_restriction inh ivals restr1
    -> inf_choice_sequence_satisfies_restriction inh ivals restr2.
Proof.
  introv same sat.
...
Lemma same_restrictions_csc_type_implies_regular {o} :
  forall inh irestr (ivals : nat -> option (@ChoiceSeqVal o)) d M Md,
    same_restrictions irestr (csc_type d M Md)
    -> inf_choice_sequence_satisfies_restriction inh ivals irestr
...
Proof.
  ...
  - rewrite inf_choice_seq_vals2choice_seq_vals_shift; auto;[].
    apply extend_choice_seq_entry_lawless_upto_type.
...
Lemma extend_library_entry_lawless_upto_choice_sequence_name2entry_upto_same {o} :
  forall n m name d (M : @RestrictionPred o) Md,
    m <= n
    -> is_nat_or_seq_kind name
    -> correct_restriction name (csc_type d M Md)
    -> extend_library_entry_lawless_upto
         (choice_sequence_name2entry_upto n name)
         (choice_sequence_name2entry_upto m name)
         M
         name
         n.
Proof.
  ...
  - inversion cor; subst; simpl in *.
    ...
    constructor; auto; autorewrite with slow; auto.
    introv h.
    rewrite select_fun2list in h; boolvar; ginv; eauto 3 with slow.

  - inversion cor; subst; simpl in *; GC.
    unfold choice_sequence_name2choice_seq_vals_upto; simpl.
    rewrite (fun2list_split n m); auto.
    constructor; auto; autorewrite with slow; auto.
    introv h.
...
Lemma extend_library_entry_lawless_upto_choice_sequence_name2entry_upto_same_max {o} :
  forall n m name d (M : @RestrictionPred o) Md,
    ...
    -> correct_restriction name (csc_type d M Md)
    -> extend_library_entry_lawless_upto
         (choice_sequence_name2entry_upto (Peano.max n m) name)
         (choice_sequence_name2entry_upto m name)
         M
         name
         n.
...
Lemma extend_library_entry_lawless_upto_choice_sequence_name2entry_upto_diff {o} :
  forall name1 name2 n1 n2 (M : @RestrictionPred o),
    name1 <> name2
    -> extend_library_entry_lawless_upto
         (choice_sequence_name2entry_upto n2 name2)
         (choice_sequence_name2entry_upto n2 name2)
         M
         name1
         n1.
...
Lemma exists_extend_library_entry_lawless_upto_default {o} :
  forall (d : nat -> @ChoiceSeqVal o) (M : RestrictionPred) (Md : forall n, M n (d n)) name n (entry : @library_entry o),
  exists entry',
    extend_library_entry_lawless_upto entry' entry M name n.
Proof.
  ...
  destruct (choice_sequence_name_deq name name0) as [dn|dn]; subst;
    [|exists (lib_cs name0 vals); simpl; boolvar; subst; tcsp];[].
...
Proof.
  ...
  - (* [a] is accessible *)
    ...
    {
      dup q as w.
      apply entry_in_inf_library_default_implies_exists in q; exrepnd.
...
Lemma extend_pre_library_lawless_upto_preserves_name_in_library {o} :
  forall (lib1 lib2 : @pre_library o) M name k,
    extend_pre_library_lawless_upto lib1 lib2 M name k
...
Qed.
Hint Resolve extend_pre_library_lawless_upto_preserves_name_in_library : slow.
...
Lemma extend_library_lawless_upto_preserves_name_in_library {o} :
  forall (lib1 lib2 : @library o) name k,
    ...
    -> name_in_library name lib1.
Proof.
  introv ext n; simpl in *.
  inversion ext as [? ? ? ? ? ? ? ? findr ext']; subst; eauto 3 with slow.
Qed.
Hint Resolve extend_library_lawless_upto_preserves_name_in_library : slow.

Lemma add_choice_preserves_names {o} :
  forall name v (lib lib' : @pre_library o) n,
    add_choice name v lib = (n, lib')
...
Proof.
  ...
  { remember (add_choice name v lib) as w; symmetry in Heqw; repnd.
    pose proof (IHlib w w0) as IHlib; repeat (autodimp IHlib hyp).
...
Lemma in_lib_if_same_names {o} :
  forall name (lib1 lib2 : @pre_library o),
...
Proof.
  ...
  - introv ext'; simpl.
    ...
    eapply lib_extends_trans;[|eauto].
    eapply extend_library_lawless_upto_implies_lib_extends; eauto 3 with slow.
