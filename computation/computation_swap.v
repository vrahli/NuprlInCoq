...
Lemma entry_depth_swap_cs_in_lib_entry {o} :
  forall sw (e : @library_entry o),
...
Lemma lib_depth_swap_cs_in_plib {o} :
  forall sw (lib : @plibrary o),
    lib_depth (swap_cs_in_plib sw lib) = lib_depth lib.
Proof.
  induction lib; simpl; auto; allrw; autorewrite with slow; auto.
Qed.
Hint Rewrite @lib_depth_swap_cs_in_plib : slow.
...
(*Lemma swap_cs_can_twice2 {o} :
  forall sw nfo (c : @CanonicalOp o),
    swap_cs_can
      (fst (swap_cs_nfo sw nfo),
       snd (swap_cs_nfo sw nfo))
      (swap_cs_can sw c)
    = swap_cs_can
        sw
...
Lemma swap_cs_sub_sw_sub {o} :
  forall sw sw' l,
    @swap_cs_sub o sw (sw_sub sw' l) = sw_sub (swap_cs_swap sw sw') l.
Proof.
...
Lemma swap_cs_term_push_swap_cs_sub_term {o} :
  forall sw sw' l (t : @NTerm o),
    swap_cs_term sw (push_swap_cs_sub_term sw' l t)
    = push_swap_cs_sub_term (swap_cs_swap sw sw') l (swap_cs_term sw t).
Proof.
...
Proof.
  ...
  { apply isnoncan_implies in isn; repndors; exrepnd; subst; simpl; tcsp. }
...
(*Lemma get_fresh_atom_swap2 {o} :
  forall sw (lib : @plibrary o) t,
    get_fresh_atom lib (swap_cs_term sw t)
    = get_fresh_atom lib t.
Proof.
  introv; unfold get_fresh_atom.
  unfold get_utokens_lib; autorewrite with slow; auto.
Qed.
Hint Rewrite @get_fresh_atom_swap2 : slow.*)
...
Lemma swap_cs_swap_twice :
  forall sw sw',
    swap_cs_swap sw (swap_cs_swap sw sw') = sw'.
Proof.
  introv; destruct sw, sw'; simpl; boolvar2.
Qed.
...
Proof.
  soterm_ind t as [v ts ind|op bs ind] Case; introv; simpl; auto.
  { f_equal.
    allrw map_map; unfold compose; simpl; apply eq_maps; introv i; tcsp. }
  rewrite swap_cs_op_swap_cs_op; auto.
  f_equal.
  allrw map_map; unfold compose; simpl; apply eq_maps; introv i.
  destruct x; simpl; f_equal.
...
(*      { SSCase "NSwapCs2".
        ...
        apply eq_maps; introv i; destruct x, c; simpl; auto;
