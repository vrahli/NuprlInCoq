...
Lemma implies_cl_sub_swap_cs_sub {o} :
  forall sw (sub : @Sub o),
    cl_sub sub
    -> cl_sub (swap_cs_sub sw sub).
Proof.
  induction sub; introv cl; simpl in *; eauto 3 with slow.
  repnd; simpl in *.
  allrw @cl_sub_cons; repnd; dands; eauto 3 with slow.
Qed.
Hint Resolve implies_cl_sub_swap_cs_sub : slow.

Lemma dom_sub_swap_cs_sub {o} :
  forall sw (sub : @Sub o),
    dom_sub (swap_cs_sub sw sub) = dom_sub sub.
Proof.
  induction sub; repnd; simpl; auto; congruence.
Qed.
Hint Rewrite @dom_sub_swap_cs_sub : slow.

Lemma swap_cs_sub_twice {o} :
  forall sw (sub : @Sub o),
    swap_cs_sub sw (swap_cs_sub sw sub) = sub.
Proof.
  induction sub; repnd; simpl; autorewrite with slow; auto; try congruence.
Qed.
...
SearchAbout (remove_nvars _ _ = []).
          ...
          { SSSCase "NCompOp".
            ...
            - SSSSCase "NSwapCs2".
              ...
              allrw @nt_wf_NCompOp; exrepnd; allunfold @nobnd; ginv.
              allsimpl; allrw @sub_filter_nil_r; allrw remove_nvars_nil_l; allrw app_nil_r.
              eapply (ind (oterm (NSwapCs2 nsw3) bs2) (oterm (NSwapCs2 nsw3) bs2)) in Heqcomp1;eauto 3 with slow;exrepnd;[].

              allsimpl; rw @compute_step_ncompop_nswap2; boolvar; tcsp.
              allrw @sub_filter_nil_r.
              rw Heqcomp1.
              dcwf h; allsimpl;[].
          ...
          { SSSCase "NArithOp".
            ...
            - SSSSCase "NSwapCs2".
              allsimpl.
              allrw @compute_step_narithop_nswap2; boolvar; tcsp; ginv.
              repeat (dcwf h); [].
              remember (compute_step lib (oterm (NSwapCs2 nsw3) bs2)) as comp1;
                symmetry in Heqcomp1; destruct comp1; ginv.
              simpl in sv; allrw remove_nvars_nil_l; allrw subvars_app_l; repnd.
      ...
      { csunf comp; csunf; allsimpl.
        ...
        - rw @compute_step_fresh_if_isnoncan_like0; eauto with slow; allsimpl; allrw app_nil_r.
          ...
          pose proof (compute_step_subst_utoken t x lib [(n, mk_utoken a')]) as ch.
          allrw @nt_wf_fresh.
          ...
          pose proof (h s lib (sub_filter sub [n])) as k; clear h.
    ...
    + SCase "NSwapCs2".
      simpl in *.
      apply compute_step_NSwapCs2_success in comp; exrepnd; subst; simpl in *; fold_terms; autorewrite with slow in *.
      ...
      { csunf; simpl; eexists; dands; eauto.
        ...
        introv i.
        unfold push_swap_cs_bterms in *.
        rewrite <- map_combine in i; apply in_map_iff in i; exrepnd; ginv; simpl in *.
        rewrite <- map_combine in i1; apply in_map_iff in i1; exrepnd; ginv; simpl in *.
        apply in_combine_same in i1; repnd; subst.
        destruct a1; simpl.
        apply alpha_eq_bterm_congr; autorewrite with slow; fold_terms.
        rewrite lsubst_aux_cl_sub_push_swap_cs_sub_term; autorewrite with slow; eauto 3 with slow.
        apply disjoint_sym; apply disjoint_dom_sub_filt. }

      { csunf; simpl; eexists; dands; eauto. }

      allrw @nt_wf_swap_cs2_iff; exrepnd; ginv.
      rewrite compute_step_swap_cs2_isnoncan_like_eq; eauto 3 with slow;[].
      rewrite <- lsubst_aux_swap_cs_term.
      assert (cl_sub (swap_cs_sub nsw sub)) as cl' by eauto 3 with slow;[].
      eapply ind in comp2; try (left;reflexivity); try exact cl'; autorewrite with slow; eauto 3 with slow.
      exrepnd.
      allrw; eexists; dands; eauto; fold_terms; eauto 3 with slow.
      apply implies_alpha_eq_mk_swap_cs2.
      eapply alpha_eq_trans;[apply implies_alpha_eq_swap_cs_term;eauto|].
...
Proof.
  ...
  repnd; allrw @prog_sub_cons;allrw @cl_sub_cons; repnd; dands; eauto 3 with slow.
Qed.
Hint Resolve prog_sub_implies_cl_sub : slow.

Lemma prog_sub_implies_sub_range_sat_wf {o} :
  forall (sub : @Sub o),
    prog_sub sub
    -> sub_range_sat sub nt_wf.
Proof.
  unfold sub_range_sat; induction sub; introv h; simpl; tcsp.
  introv q; repndors; subst; simpl in *; allrw @prog_sub_cons; repnd; eauto 3 with slow.
  allrw @prog_sub_cons; repnd; eapply IHsub; eauto.
Qed.
Hint Resolve prog_sub_implies_sub_range_sat_wf : slow.
...
Proof.
  ...
  - rw @reduces_in_atmost_k_steps_S in r; exrepnd.
    pose proof (compute_step_lsubst_aux t u0 lib sub) as h.
    ...
    pose proof (reduces_to_alpha lib (lsubst_aux u0 sub) v v0) as x;
      ...
      exrepnd;
      eauto 3 with slow;
      try (apply lsubst_aux_wf_iff; eauto 3 with slow);[].
