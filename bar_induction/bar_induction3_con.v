...
<<
   H |- squash(X 0 (norm c 0))
     ...
     H, n:nat, s: nat_n -> nat |- (R n s) in Type(i)                                            // R is a well-formed predicate on finite sequences
     H, s: nat -> nat, con: (forall m: nat. R m s) |- squash(exists n:nat. B n s)               // B is a bar
     H, n:nat, s: nat_n -> nat, con: (forall m: nat_n. R m s), m: B n s |- X n s                // Base case: the conclusion is true at the bar
     H, n:nat, s: nat_n -> nat, x: (forall m: nat. R n s -> X (n + 1) (ext(s,n,m))) |- X n s    // induction case
>>
  ...
  mk_rule
    ...
    [ (* well-formedness of the bar B *)
      ...
      mk_bseq (snoc (snoc H (mk_hyp s mk_nat2nat))
                    (mk_hyp con (mk_all mk_tnat m (mk_apply2 R (mk_var m) (mk_var s)))))
      ...
      mk_bseq (snoc (snoc (snoc (snoc H (mk_hyp n mk_tnat))
                                (mk_hyp s (mk_natk2nat (mk_var n))))
                          (mk_hyp con (mk_all (mk_natk (mk_var n)) m (mk_apply2 R (mk_var m) (mk_var s)))))
