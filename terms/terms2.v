...
Definition mk_seqlib {o} (name : seqname) : @NTerm o := oterm (Can (Nseqlib name)) [].
...
Lemma fold_seqlib {o} :
  forall (n : seqname), @oterm o (Can (Nseqlib n)) [] = mk_seqlib n.
Proof. sp. Qed.
...
Proof.
 ...
 assert (forall n t, (size t) = n -> P t) as Hass;
   [|introv;
      apply Hass with (n := size t); reflexivity].
...
(* - clear Hvar Hbt.
 ...
 - apply Hbt.
   ...
   eapply le_lt_trans;[exact Hs|]; clear Hs.
   subst.
   apply (size_subterm3 op) in Hin; simpl in *; auto.
Defined.
...
Fixpoint wft {o} (t : @NTerm o) : bool :=
  match t with
  | vterm _ => true
(*  | sterm f => obseq (fun n => oband (bool2obool (no_vars_like_b (f n))) (wft (f n)))*)
  | oterm op bts =>
    andb (beq_list eq_nat_dec (map (num_bvars) bts) (OpBindings op))
         (ball (map wftb bts))
  end
with wftb {p} (bt : BTerm) : bool :=
       match bt with
       | bterm vars t => wft t
       end.

Definition wf_term {p} (t : @NTerm p) := wft t = true.

Lemma fold_wf_term {o} : forall t : @NTerm o, wft t = true <=> wf_term t.
Proof. sp. Qed.

Definition wf_bterm {p} (b : @BTerm p) := wftb b = true.
  ...
  - Case "oterm".
    ...
    + SCase "<-".
      ...
      { introv i.
        ...
        applydup ind in i.
        apply i0; clear i0.
        rw ball_map_true in q.
        apply q in i; simpl in i; auto. }
...
Lemma isprogram_mk_seqlib {p} : forall (n : seqname), @isprogram p (mk_seqlib n).
Proof.
  repeat constructor. intros; allsimpl; sp.
Qed.

Lemma isprog_mk_seqlib {p} : forall (n : seqname), @isprog p (mk_seqlib n).
Proof.
  repeat constructor.
Qed.

Lemma isvalue_mk_seqlib {p} : forall (n : seqname), @isvalue p (mk_seqlib n).
Proof.
  repeat constructor. intros; allsimpl; sp.
Qed.
...
Proof.
  ...
  - unfold wf_term in h; simpl in h.
    allrw andb_eq_true; repnd; dands; auto.
Qed.
...
Proof.
  ...
  - unfold wf_term in h; simpl in h; allrw andb_eq_true; repnd; dands; auto.
Qed.
...
Lemma isprogram_seqlib_implies {p} :
  forall (bterms : list (@BTerm p)) n,
    isprogram (oterm (Can (Nseqlib n)) bterms)
    -> bterms = [].
Proof.
  introv isp; inversion isp as [cl w].
  inversion w as [| o lnt k e]; subst; allsimpl.
...
Definition mkc_seqlib {o} (n : seqname) : CTerm :=
  exist (@isprog o) (mk_seqlib n) (isprog_mk_seqlib n).

Lemma mkc_seqlib_eq {o} :
  forall (f g : seqname),
    @mkc_seqlib o f = mkc_seqlib g
    -> f = g.
Proof.
  unfold mkc_seqlib; introv h.
  inversion h; sp.
Qed.
...
Proof.
  sp; split; intro k.
  - unfold wf_term in *; simpl in *; allrw andb_eq_true; repnd; dands; auto.
  - repnd.
