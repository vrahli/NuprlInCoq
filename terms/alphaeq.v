...
Proof.
  ...
  inverts Hlt0 as Hb1 Hb2 Hb3 Hb4 Hb5.
    ...
    assert (ord_le (osize (lsubst nnt1 (var_ren lv1 lv4))) (osize nnt1)) as op.
    {  change_to_lsubst_aux4.
       rewrite lsubst_aux_allvars_preserves_osize; [ eauto 3 with slow |].
       apply allvars_combine; fail. }
    pose proof (Hind nnt1 (lsubst nnt1 (var_ren lv1 lv4)) lv1 XX op
                  (lsubst nnt2 (var_ren lv2 lv4)) Hb5 (lv4++lvn++lv)) as w.
    try (rw fold_var_ren in w).
    try (rw fold_var_ren in w).
    try (rw fold_var_ren).
    try (rw fold_var_ren).
    apply alpha3_lsubst_allvars_congr in w; try congruence.
    + revert w. change_to_lsubst_aux4. intro w.
      rewrite lsubst_aux_nest_vars_same in w; auto;
