...
Lemma extensional_ext_trycatch {p} : extensional_op_ext (@NCan p NTryCatch).
Proof.
  ...
  - apply no_change_after_val_like with (k2:=k) in Hcv3; try splr.
    ...
    apply (Hi _ _ (mk_atom_eq a1 a1 v1 mk_bot)) in hh;
      ...
      eauto 2 with slow;
      try (complete (apply approx_ext_star_mk_atom_eq; auto;
                     apply approx_ext_star_refl; eauto 3 with slow)).
  ...
  - apply isexc_implies in Hcv0; auto; exrepnd; subst.
...

    {
      ...
      - unfold approx_ext_starbts, lblift_sub; allsimpl; dands; auto.
