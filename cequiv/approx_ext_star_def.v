...
(** %\noindent% To prove that [approx] is a cogruence, Howe defined a
    relation
    [approx_ext_star] that contains [approx_ext_open] and is a congruence by
    definition. The main challenge then is to prove that [approx_ext_star]
    implies [approx_ext_open], and hence they are equivalent.
    Howe reduced this
    ...
    We begin by defining his [approx_ext_star] relation. It is a relation
...
Inductive approx_ext_star {p} :
  @library p -> @NTerm p -> @NTerm p -> [univ] :=
| apsv: forall lib v t2,
          (approx_ext_open lib (vterm v) t2)
          -> (approx_ext_star lib (vterm v) t2)
| apss: forall lib t (f1 f2 : ntseq),
          wf_term (sterm f1)
          -> wf_term (sterm f2)
          -> (forall n, approx_ext_star lib (f1 n) (f2 n))
          -> approx_ext_open lib (sterm f2) t
          -> approx_ext_star lib (sterm f1) t
| apso: forall lib
               (op : Opid)
               (t2: NTerm)
               (lbt1 lbt1' : list BTerm),
          length lbt1 = length lbt1'
          -> lblift_sub lib op (approx_ext_star lib) lbt1 lbt1'
          -> approx_ext_open lib (oterm op lbt1') t2
          -> approx_ext_star lib (oterm op lbt1) t2.
Hint Constructors approx_ext_star : slow.


Definition approx_ext_star_bterm {o} (lib : @library o) op :=
  blift_sub lib op (approx_ext_star lib).

Definition approx_ext_starbts {o} (lib : @library o) op :=
  lblift_sub lib op (approx_ext_star lib).
