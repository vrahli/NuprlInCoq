...
Lemma extensional_ext_decide {p} : extensional_op_ext (@NCan p NDecide).
Proof.
  ...
  - apply iscan_implies in Hcv0; repndors; exrepnd; subst;
    ...
    unfold approx_ext_starbts, lblift_sub in hh1; simpl in hh1; repnd; cpx.
    repeat(approxrelbtd); show_hyps.
    ...
    with (lnt1:= [d]) (lnt2:= [dr])
      ...
      [|prove_bin_rel_nterm; prove_approx_ext_star; auto; prove_isprogram].
    ...
    + applydup @isprogram_inl_iff in Hcv8; repnd.
      ...
      apply approx_ext_star_open_trans with (b := subst b0 v0 dr); auto.
    ...
    + applydup @isprogram_inr_iff in Hcv8; repnd.
      ...
      apply approx_ext_star_open_trans with (b := subst b3 v3 dr); auto.
      apply approx_ext_implies_approx_ext_open.
      apply reduces_to_implies_approx_ext_eauto; prove_isprogram.
