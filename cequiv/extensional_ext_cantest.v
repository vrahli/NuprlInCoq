...
Lemma extensional_ext_cantest {p} : forall a, extensional_op_ext (@NCan p (NCanTest a)).
Proof.
  ...
  - apply iscan_implies in Hcv0; repndors; exrepnd; subst;
    ...
    { apply (Hi _ _ (if canonical_form_test_for a c1 then b0 else c0)) in hh;
      ...
      apply @approx_ext_star_open_trans with (b := if canonical_form_test_for a c1 then b0 else c0); auto.
      apply approx_ext_implies_approx_ext_open.
      apply @approx_ext_trans with (b := oterm (NCan (NCanTest a)) [nobnd (oterm (Can c1) lbt'),nobnd b0,nobnd c0]).
    ...
    { apply howe_lemma2_seq in h1; exrepnd; auto.
      ...
      eapply approx_ext_star_open_trans;[exact hh|clear hh].
      apply approx_ext_implies_approx_ext_open.
      apply reduces_to_implies_approx_ext_eauto; prove_isprogram.
