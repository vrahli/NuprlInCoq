...
Require Export approx_ext_star_props1.
...
Hint Resolve approx_ext_star_relates_only_wf : slow.


Lemma alpha_implies_approx_ext_star {p} :
  forall lib nt1 nt2,
  @nt_wf p nt1
  -> alpha_eq nt1 nt2
...
Proof.
  ...
  apply alpha_implies_approx_ext_open; trivial.
Qed.

Lemma approx_ext_star_trans {p} : forall lib t1 t2 t3,
  @approx_ext_star p lib t1 t2
...
Abort. (** see comments above for approx_ext_star_open_trans*)

Lemma le_approx_ext_ntwf {p} :
  forall lib, le_bin_rel (approx_ext_star lib) (indep_bin_rel nt_wf (@nt_wf p)).
Proof.
  eauto with slow.
Qed.

Lemma approx_ext_rel_wf_sub {p} :
  forall lib lvi lnta lntb,
  bin_rel_nterm (@approx_ext_star p lib) lnta lntb
  -> wf_sub (combine lvi lnta) # wf_sub (combine lvi lntb).
Proof.
  introv Hb. apply (le_binrel_list_un _ _ _ _ (le_approx_ext_ntwf lib)) in Hb.
  exrepnd. unfold wf_sub, sub_range_sat.   dands; eauto with slow.
Qed.

Lemma approx_ext_star_btermd_fr {p} :
...
Lemma change_nr_ut_sub_in_lsubst_aux_approx_ext_star {o} :
  forall lib (t1 t2 : @NTerm o) sub1 sub2 l,
    wf_term t1
    -> wf_term t2
    -> approx_ext_star lib (lsubst_aux t1 sub1) (lsubst_aux t2 sub1)
    -> subset (get_utokens t1) l
    -> subset (get_utokens t2) l
    -> nrut_sub l sub1
    -> nrut_sub l sub2
    -> dom_sub sub1 = dom_sub sub2
    -> approx_ext_star lib (lsubst_aux t1 sub2) (lsubst_aux t2 sub2).
Proof.
...
Focus 2.
{
  ...
  constructor.
       apply approx_ext_open_vterm_iff_reduces_to in ao;
       [|apply lsubst_aux_preserves_wf_term2; eauto with slow];
       apply approx_ext_open_vterm_iff_reduces_to;
       [apply lsubst_aux_preserves_wf_term2; eauto with slow|];
...
*)
    ...
    + exrepnd; rw e0.
      ...
      allrw map_length.
      apply approx_ext_open_simpler_equiv in apo.
...
SearchAbout dom_sub ax_sub.
  ...
  - Case "vterm".
    ...
    apply approx_ext_open_simpler_equiv.
...
Qed.
      ...
      apply approx_ext_utoken_implies_reduces_to in h.
...
(*Lemma approx_ext_open_change_utoks {o} :
  forall lib (t1 t2 : @NTerm o) ren,
    ...
    -> approx_ext_open lib (ren_utokens ren t1) (ren_utokens ren t2).
Proof.
  introv norep1 norep2 dl1 dl2 disj1 disj2 apo.
  ...
  pose proof (ex_ren_utokens_sub
                sub
                ren
                (get_utokens_lib lib t1 ++ get_utokens_lib lib t2)) as exren.
  ...
  pose proof (approx_ext_change_utoks lib (lsubst t1 sub') (lsubst t2 sub') (ren ++ ren')) as k.
  ...
  {
    introv i j.
    applydup exren8 in i.
    applydup dl1 in i.
    applydup exren4 in j; apply in_app_iff in j0; repndors; tcsp.
  }
  ...
  { introv a b; applydup disj2 in a.
    ...
    allrw not_over_or; repnd.
    apply not_over_and in a0;
      [|destruct (in_deq _ (get_patom_deq o) t (get_utokens t2));
        try (complete (left; tcsp));
        destruct (in_deq _ (get_patom_deq o) t (get_utokens_library lib));
        try (complete (left; tcsp));
        right; tcsp];[].
