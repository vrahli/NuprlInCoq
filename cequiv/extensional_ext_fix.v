...
Lemma extensional_ext_fix {p} : extensional_op_ext (@NCan p NFix).
Proof.
  ...
  - unfold lblift_sub in Has; repnd; allsimpl; GC.
    ...
    { simpl in XX1; inversion XX1; subst; GC.
      ...
      assert (approx_ext_star
        ...
        as XXX.
      { repeat (prove_approx_ext_star; auto; prove_isprogram). }
      ...
      match goal with
          [ H : approx_ext_star ?lib ?cc ?t2 |- approx_ext_star ?lib ?cc ?t1] =>
          apply (approx_ext_star_open_trans lib) with (b:=t2) end; auto.

      apply approx_ext_implies_approx_ext_open.
      apply reduces_to_implies_approx_ext_eauto; try (rw <- @isprogram_fix_iff; auto).
    ...
    { csunf XX1; allsimpl; ginv.
      ...
      apply (Hi _ _ (mk_apply (mk_ntseq f') (oterm (NCan NFix) [bterm [] (sterm f')]))) in ca0;
        try prove_isprogram;
        [|repeat (prove_approx_ext_star; eauto 2 with slow; prove_isprogram);
           eapply apss;try (exact h1); eauto 3 with slow].

      eapply approx_ext_star_open_trans;[exact ca0|].
      apply approx_ext_implies_approx_ext_open.
      apply reduces_to_implies_approx_ext_eauto; try (rw <- @isprogram_fix_iff; auto).
  ...
  - apply isexc_implies in Hcv0; auto; exrepnd; subst.
    ...
    apply approx_ext_star_open_trans with (b := mk_exception a' e'); try (rw fold_fix).
