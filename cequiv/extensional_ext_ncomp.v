...
Lemma extensional_ext_ncomp {p} : forall a, extensional_op_ext (@NCan p (NCompOp a)).
Proof.
  ...
  - unfold extensional_op_ext_ind in Hi.
    ...
    allrw <- @pk2term_eq.
    exrepnd.
    unfold approx_ext_starbts, lblift_sub in h1; simpl in h1; repnd; cpx.
    unfold approx_ext_starbts, lblift_sub in h2; simpl in h2; repnd; cpx.
    ...
    apply @approx_ext_star_open_trans with (b := if d1 then c0 else d0); auto.
    apply approx_ext_implies_approx_ext_open.
    apply @approx_ext_trans with (b := oterm (NCan (NCompOp a))
                                        [nobnd (pk2term pk1),
                                         nobnd (pk2term pk2),
                                         nobnd c0,
                                         nobnd d0]).
