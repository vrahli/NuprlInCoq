...
Lemma nuprl_extensional_ext_abs {p} :
  forall x : opabs, @extensional_op_ext p (Abs x).
Proof.
  introv Hpra Hprt Hprt' Hcv Has Hi.
  allfold (approx_ext_starbts lib).
  apply computes_to_val_like_in_max_k_steps_S in Hcv; exrepnd.
  csunf Hcv1; allsimpl.
  apply compute_step_lib_success in Hcv1; exrepnd; subst.
  dup Hcv2 as fe1.
  pose proof (approx_ext_starbts_numvars lib (Abs x) lbt lbt' Has) as eqnum.
  ...
  - apply @approx_ext_star_open_trans with (b := mk_instance vars lbt' rhs); auto.
  ...
  - unfold correct_abs in correct;repnd.
    eapply mk_instance_approx_ext_star_congr; eauto; try (complete (intro xxx; ginv)).
