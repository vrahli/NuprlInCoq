...
Require Export approx_ext_star_def.
...
Lemma nr_ut_sub_approx_ext_star_aux {o} :
  forall lib (t1 t2 : @NTerm o) sub,
    nr_ut_sub lib (mk_pair t1 t2) sub
    -> approx_ext_star lib t1 t2
    -> approx_ext_star lib (lsubst_aux t1 sub) (lsubst_aux t2 sub).
Proof.
  ...
  - Case "vterm".
    ...
    pose proof (approx_ext_open_lsubst_congr lib (vterm v) t2 sub) as h.
  ...
  - Case "sterm".
    ...
    allrw <- @approx_ext_open_simpler_equiv.
...
Lemma approx_ext_star_refl {p} : forall lib t, @nt_wf p t -> approx_ext_star lib t t.
Proof.
  ...
  - Case "oterm".
    ...
    + left.
      ...
      inverts Hbt1; tcsp.
Qed.
Hint Resolve approx_ext_star_refl : slow.

(* begin hide *)

Lemma approx_ext_starbt_refl {p} :
  forall lib op bt, @bt_wf p bt -> approx_ext_star_bterm lib op bt bt.
Proof.
  introv hbt.
  ...
  introv i; allrw in_app_iff; sp.
Qed.
Hint Resolve approx_ext_starbt_refl : slow.

Lemma approx_ext_open_implies_approx_ext_star {p} :
...
Proof.
  ...
  - Case "sterm".
    introv Hap.
    applydup @approx_ext_open_relates_only_wf in Hap; repnd.

    apply (apss _ _ _ f); eauto 2 with slow.
    introv.
    apply ind.
    apply approx_ext_open_relates_only_wf in Hap; repnd.
    apply approx_ext_open_refl; eauto 3 with slow.
  ...
  - Case "oterm".
    ...
    intros. apply approx_ext_starbt_refl.
    unfold approx_ext_open in Hap.
    repnud Hap.
    invertsn Hap0.
    apply Hap0. apply selectbt_in; auto.
Qed.
Hint Resolve approx_ext_open_implies_approx_ext_star : slow.
...
(*
Lemma approx_ext_open_vterm {o} :
  forall lib x (t : @NTerm o),
    approx_ext_open lib (vterm x) t
    -> reduces_to lib t (vterm x).
Proof.
  nterm_ind1s t as [v|op bs ind] Case; introv ap.

  - Case "vterm".
    unfold approx_ext_open, olift in ap; repnd.
  ...
  - Case "oterm".
    ...
    + inversion h as [c]; clear h.
      unfold close_compute_ext in c; simpl in c; repnd.
      clear c2 c3.
...
(* This is what's called simpl_olift in approx_ext.v *)
...
(*
Lemma approx_ext_open_vterm_iff_reduces_to {o} :
  forall lib x (t : @NTerm o),
    wf_term t
    -> (approx_ext_open lib (vterm x) t
        <=> reduces_to lib t (vterm x)).
Proof.
  ...
  - apply approx_ext_open_vterm; auto.
  - unfold approx_ext_open.
    ...
    + introv cl isp1 isp2.
      ...
      eapply approx_ext_alpha_rw_l_aux;[eauto|].

      eapply approx_ext_comput_functionality_left;[eauto|].
      ...
      * t_change t'.
        ...
        apply alpha_implies_approx_ext2; auto.
Qed.
*)

(*
Lemma compute_step_approx_ext_star {o} :
  forall lib (t1 t2 t3 : @NTerm o),
    approx_ext_star lib t1 t3
    -> compute_step lib t1 = csuccess t2
    -> approx_ext_star lib t2 t3.
Proof.
    ...
    + SCase "NCan".
      ...
      * SSCase "Can".
        ...
        { SSSCase "NApply".
          ...
          allfold (approx_ext_star_bterm lib).

          apply approx_ext_star_bterm_nobnd in h1; exrepnd; subst.
          apply approx_ext_star_bterm_nobnd in h2; exrepnd; subst.
          fold_terms.
          ...
          apply approx_ext_star_btermd_1var in h3; exrepnd; subst.
          fold_terms.

          assert (approx_ext_open lib (mk_apply (mk_lam vr b3) b0) t3) as ap.
          {
            eapply approx_ext_open_trans;[|exact ap2].
            applydup @approx_ext_open_relates_only_wf in ap2; repnd.
            applydup @approx_ext_open_relates_only_wf in ap3; repnd.
...
Proof.
  ...
  - Case "vterm".
    ...
    apply approx_ext_open_vterm in aop.
    constructor.
    applydup @approx_ext_star_relates_only_wf in ap2; repnd.

    apply approx_ext_open_vterm_iff_reduces_to;[apply wf_term_eq;auto|].
...
Theorem approx_ext_star_relates_only_wf {p} : forall lib (t1 t2 : @NTerm p),
  approx_ext_star lib t1 t2 -> nt_wf t1 # nt_wf t2.
Proof.
  nterm_ind1s t1 as [v1|f ind|o lbt1 Hind] Case; introv Hap.

  - Case "vterm".
    invertsn Hap. apply approx_ext_open_relates_only_wf in Hap; sp.

  - Case "sterm".
    inversion Hap as [|? ? ? ? wf1 wf2 imp aop|]; clear Hap; subst.
    allapply @approx_ext_open_relates_only_wf; repnd; dands; eauto 3 with slow.

  - Case "oterm".
    inverts Hap as Hlen Hrel Hapo.

    apply approx_ext_open_relates_only_wf in Hapo. repnd; split; trivial.
...
Theorem approx_ext_starbt_relates_only_wf {p} :
  forall lib op (t1 t2 : @BTerm p),
    approx_ext_star_bterm op lib t1 t2
    -> bt_wf t1 # bt_wf t2.
Proof.
  introv H. destruct t1 as [lv1 nt1]. destruct t2 as [lv2 nt2].
  repeat(rewrite bt_wf_iff).
  allunfold @approx_ext_star_bterm.
  unfold blift_sub in H.
  exrepnd; repndors; exrepnd; subst.

  - apply approx_ext_star_relates_only_wf in H1; repnd.
    invertsna H0 H2bal.
    invertsna H2 H1bal.
...
Lemma approx_ext_star_congruence {p} :
...
Proof.
  ...
  apply approx_ext_open_refl. apply ntot_wf_iff.
  split; auto. introv Hle. rw <- Hbts1 in Hle.
  apply Hbts in Hle.
  apply approx_ext_starbt_relates_only_wf in Hle. repnd; auto.
Qed.

(* begin hide *)

Theorem approx_ext_star_congruence2 {o} :
  forall lib op (lbt1 lbt2 : list (@BTerm o)),
    approx_ext_starbts lib op lbt1 lbt2
    -> nt_wf (oterm op lbt2)
    -> approx_ext_star lib (oterm op lbt1) (oterm op lbt2).
Proof.
   introv Haps Hnt.
   inverts Hnt.
   apply approx_ext_star_congruence; sp.
Qed.

Lemma approx_ext_star_alpha_fun_l {p} :
  forall lib nt1 nt2 nt1',
  @approx_ext_star p lib nt1 nt2
  -> alpha_eq nt1 nt1'
...
Lemma approx_ext_star_alpha_fun_l_iff {p} :
  forall lib nt1 nt1', @alpha_eq p nt1 nt1'
    ->  (forall nt2,(approx_ext_star lib nt1 nt2 <=> approx_ext_star lib nt1' nt2)).
Proof.
  introv Hal. applydup @alpha_eq_sym in Hal.
  split; intro H; eapply approx_ext_star_alpha_fun_l; eauto.
Qed.

(* end hide *)

(* this is not transitivity.
    Clearly, transitivity implies this lemma. (approx_ext_open => approx_ext_star)
    ...
    But I couldnt *)
Lemma approx_ext_star_open_trans {p} :
  forall lib a b c,
    @approx_ext_star p lib a b
    -> approx_ext_open lib b c
    -> approx_ext_star lib a c.
Proof.
  introv Has Hap.
...
Hint Resolve alpha_implies_approx_ext_open : slow.

Lemma approx_ext_star_alpha_fun_r {p} :
...
Proof.
  ...
  - Case "oterm".
    ...
    apply alpha_implies_approx_ext_open in Hal; eauto with slow.

    eapply approx_ext_star_open_trans; eauto with slow.
Qed.

Hint Resolve approx_ext_star_alpha_fun_r approx_ext_star_alpha_fun_l : slow.

(* more useful for rewrite tactics*)
Lemma approx_ext_star_alpha_fun_r_iff {p} :
  forall lib nt2 nt2', @alpha_eq p nt2 nt2'
         -> (forall nt1, (approx_ext_star lib nt1 nt2 <=> approx_ext_star lib nt1 nt2')).
Proof.
  introv Hal. applydup @alpha_eq_sym in Hal.
  split; intro H; eapply approx_ext_star_alpha_fun_r; eauto.
Qed.

Ltac alpharw_as H := let X99:= fresh "Xalrw" in
let lhs := get_alpha_lhs H in
match goal with 
| [ |- approx_ext_star ?lib (lsubst lhs ?sub1) _] =>
    apply (approx_ext_star_alpha_fun_l_iff lib _ _ (lsubst_alpha_congr2 _ _ sub1 H))
| [ |- approx_ext_star ?lib _ (lsubst lhs ?sub1)] =>
    apply (approx_ext_star_alpha_fun_r_iff lib _ _ (lsubst_alpha_congr2 _ _ sub1 H))
| [ |- approx_ext_star ?lib lhs ?rhs ] =>
    apply (approx_ext_star_alpha_fun_l_iff lib _ _ H)
| [ |- approx_ext_star ?lib ?llhs lhs ] =>
    apply (approx_ext_star_alpha_fun_r_iff lib _ _ H)
end.

Ltac alpharwh_as H Hyp := let X99:= fresh "Xalrw" in
let lhs := get_alpha_lhs H in
match goal with
| [ Hyp : (approx_ext_star ?lib (lsubst lhs ?sub1)) |- _ ] =>
    apply (approx_ext_star_alpha_fun_l_iff lib _ _ (lsubst_alpha_congr2 _ _ sub1 H)) in Hyp
| [ Hyp : approx_ext_star ?lib _ (lsubst lhs ?sub1) |- _ ] =>
    apply (approx_ext_star_alpha_fun_r_iff lib _ _ (lsubst_alpha_congr2 _ _ sub1 H)) in Hyp
| [ Hyp : approx_ext_star ?lib lhs ?rhs |- _ ] =>
    apply (approx_ext_star_alpha_fun_l_iff lib _ _ H) in Hyp
| [ Hyp : approx_ext_star ?lib ?lllhs lhs |- _ ] =>
    apply (approx_ext_star_alpha_fun_r_iff lib _ _ H) in Hyp
end.
...
Proof.
  ...
  exists lbtcv.
  unfold approx_ext_open in Has1.
  rwhg Hfr1'0 Has1.
  dands;spc.
  unfold approx_ext_starbts.
...
Proof.
  ...
  eauto with slow.
Qed.
Hint Resolve approx_ext_star_bterm_alpha_fun_r : slow.

Lemma approx_ext_star_bterm_alpha_fun_l {o} :
  forall lib op (b1 b2 b3 : @BTerm o),
    alpha_eq_bterm b1 b2
    -> approx_ext_star_bterm lib op b2 b3
    -> approx_ext_star_bterm lib op b1 b3.
Proof.
  introv aeq ap.
  unfold approx_ext_star_bterm, blift_sub in ap; exrepnd.
  ...
  eauto with slow.
Qed.
Hint Resolve approx_ext_star_bterm_alpha_fun_l : slow.

Lemma approx_ext_starbts_cons {o} :
  forall lib op (b1 b2 : @BTerm o) bs1 bs2,
    approx_ext_starbts lib op (b1 :: bs1) (b2 :: bs2)
    <=> (approx_ext_star_bterm lib op b1 b2 # approx_ext_starbts lib op bs1 bs2).
Proof.
  introv.
  unfold approx_ext_starbts, lblift_sub; simpl; split; intro k; repnd; cpx; dands; auto.
...
Lemma approx_ext_star_berms_alpha_fun_l {o} :
  forall lib op (bs1 bs2 bs3 : list (@BTerm o)),
    alpha_eq_bterms bs1 bs2
    -> approx_ext_starbts lib op bs2 bs3
    -> approx_ext_starbts lib op bs1 bs3.
Proof.
  ...
  - destruct bs2.
    ...
    + apply alpha_eq_bterms_cons in aeq; repnd.
      ...
      * unfold approx_ext_starbts, lblift_sub in ap; allsimpl; sp.

      * allrw @approx_ext_starbts_cons; repnd; dands; eauto with slow.
Qed.
Hint Resolve approx_ext_star_berms_alpha_fun_l : slow.

Lemma approx_ext_star_berms_alpha_fun_r {o} :
  forall lib op (bs1 bs2 bs3 : list (@BTerm o)),
...
Proof.
  ...
  - destruct bs2.
    ...
    + allrw @approx_ext_starbts_cons; repnd.
      destruct bs3.
      ...
      * apply alpha_eq_bterms_cons in aeq; repnd.
        allrw @approx_ext_starbts_cons; repnd; dands; eauto with slow.
Qed.
Hint Resolve approx_ext_star_berms_alpha_fun_r : slow.
...
(* lemma 0.6 in annotated paper/
  ...
  It will enable us to prove lemmas for destructing approx_ext_star(_bterm)
  hypotheses
  so that the bound variables are picked nicely.
 Even now, approx_ext_ot_change can be used to simplify the proof.
 *)

Theorem approx_ext_star_lsubst_vars {p} :
  forall lib a b lvi lvo,
    length lvi = length lvo ->
    let sub := @var_ren p lvi lvo in
    approx_ext_star lib a b
    -> approx_ext_star lib (lsubst a sub) (lsubst b sub).
Proof.
  ...
  - Case "sterm".
    ...
    econstructor; eauto.
    apply (approx_ext_open_lsubst _ _ _ lvi lvo) in aop;
      autorewrite with slow in *; auto.

  - Case "oterm".
    apply (approx_ext_ot_change _ lvo) in Hap; exrepnd.
    ...
    eapply approx_ext_open_alpha_rw_r_aux in Hap0;[|exact k1].
    clear b k1.
    rename t' into b.
    ...
    eapply approx_ext_star_berms_alpha_fun_l in Hap2;
    ...
    apply (apso _ _ _ _ (map (fun t : BTerm => lsubst_bterm_aux t (var_ren lvi lvo)) lbtcv));
      allrw map_length; auto;
      [|apply (approx_ext_open_lsubst _ _ _ lvi lvo) in Hap0;
    ...
    unfold blift_sub.
    unfold approx_ext_starbts in Hap2.
    ...
    + left; dands; auto.
      ...
      assert (approx_ext_star lib nt1n nt2n) as apr by (eauto 3 with slow).
    ...
    + right.
      ...
      assert (approx_ext_star
                lib
                (lsubst (lsubst nt1n (var_ren (dom_sub sub) lvn)) sub')
                (lsubst (lsubst nt2n (var_ren (dom_sub sub) lvn)) sub'))
             as apr.
      { eapply approx_ext_star_alpha_fun_r;[|apply alpha_eq_sym in nest2; exact nest2].
        eapply approx_ext_star_alpha_fun_l;[|apply alpha_eq_sym in nest1; exact nest1].
        eauto 3 with slow. }
      rw <- bl3 in apr.
...
(* The key complication in prev. proof
    was that the approx_ext_star hypothesis "picks"
...
Lemma approx_ext_star_btermd {p} :
...
Lemma approx_ext_star_samevar {p} :
  forall lib op a b lv,
    op <> NCan NFresh
    -> approx_ext_star_bterm lib op (bterm lv a) (@bterm p lv b)
    -> approx_ext_star lib a b.
Proof.
  introv d Has.
  apply (approx_ext_star_btermd _ _ _ _ lv) in Has; auto.
  exrepnd.
  pose proof (change_bvars_alpha_wspec lv a) as Hfa.
  exrepnd. rename ntcv into a'.
  assert (approx_ext_star lib a' b) ;[| eauto with slow].
...
Lemma approx_ext_star_bterm_nobnd {p} : forall lib op a bt,
  approx_ext_star_bterm lib op (bterm [] a) bt
  -> {b : @NTerm p
      $ bt = (bterm [] b)
      # approx_ext_star lib a b}.
Proof.
  ...
  unfold approx_ext_star_bterm, blift_sub in Has; exrepnd.
