...
Lemma implies_isprogram_find_ref_def {o} :
  forall lib name (d : @NTerm o),
    isprogram d
    -> isprogram (find_ref_def lib name d).
Proof.
  introv isp.
  unfold find_ref_def.
  remember (find_ref lib name) as fcs; symmetry in Heqfcs; destruct fcs; auto.
Qed.
Hint Resolve implies_isprogram_find_ref_def : slow.

Lemma implies_approx_star_find_ref_def {o} :
...
Qed.
Hint Resolve implies_approx_star_find_ref_def : slow.

Lemma extensional_read_ref {p} : extensional_op (@NCan p NReadRef).
Proof.
  ...
  apply isprogram_read_ref_implies_ex in Hcv6; unfold nobnd in *; exrepnd; subst; cpx.
  ...
  - apply iscan_implies in Hcv0; repndors; exrepnd; subst.
    ...
    { csunf XX1; allsimpl.
      apply compute_step_read_ref_success in XX1; exrepnd; repndors;
      ...
      applydup @isprogram_read_ref_implies in Hprt'; repnd.

      pose proof (Hi (find_ref_def lib name d)
                     c
                     (find_ref_def lib name dr)) as q.
  ...
  - apply isexc_implies in Hcv0; auto; exrepnd; subst.
    ...
    apply reduces_to_trans with (b := mk_read_ref (mk_exception a' e') ur).
