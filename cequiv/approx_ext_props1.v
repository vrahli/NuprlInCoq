...
Lemma approx_ext_bad_implies_approx_ext {o} :
  forall lib (t1 t2 : @NTerm o),
    approx_ext_bad lib t1 t2 -> approx_ext lib t1 t2.
Proof.
  intro lib.
  pose proof
       (approx_ext_acc
          lib
          (fun a b => approx_ext_bad lib a b)
          (@bot2 o)) as HH.
  allsimpl.
  match goal with
  ...
  inversion ap as [cl]; clear ap.
  constructor.
  allunfold @close_compute_ext; repnd; dands; auto.
...
Lemma approx_ext_implies_approx_ext_bad {o} :
  forall lib (t1 t2 : @NTerm o),
    approx_ext lib t1 t2 -> approx_ext_bad lib t1 t2.
Proof.
  intro lib.
  cofix IND.
  introv apr.
...
Lemma approx_ext_open_simpler_equiv_r {o} :
  forall lib (a c : @NTerm o) r,
    respects_alpha_r (approx_ext_aux lib r \2/ r)
    -> respects_alpha_l (approx_ext_aux lib r \2/ r)
    -> (simpl_olift (approx_ext_aux lib r \2/ r) a c <=> olift (approx_ext_aux lib r \2/ r) a c).
Proof.
  introv rr rl.
  split.
...
Lemma approx_ext_change_utoks {o} :
  forall lib (t1 t2 : @NTerm o) rens,
    ...
    -> approx_ext lib (rens_utokens rens t1) (rens_utokens rens t2).
Proof.
  intro lib.
...
Definition approx_ext_or_bts {o} lib (r : bin_rel (@NTerm o)) :=
  lblift (olift (approx_ext_aux lib r \2/ r)).

Lemma approx_ext_or_bts_alpha_eq_bterms_l {o} :
  forall lib (bs1 bs2 bs3 : list (@BTerm o)) r,
    alpha_eq_bterms bs1 bs2
    -> approx_ext_or_bts lib r bs1 bs3
    -> approx_ext_or_bts lib r bs2 bs3.
Proof.
  introv aeq apr.
  allunfold @approx_ext_or_bts.
...
Lemma approx_ext_or_bts_alpha_eq_bterms_r {o} :
  forall lib (bs1 bs2 bs3 : list (@BTerm o)) r,
    alpha_eq_bterms bs2 bs3
    -> approx_ext_or_bts lib r bs1 bs3
    -> approx_ext_or_bts lib r bs1 bs2.
Proof.
  introv aeq apr.
  allunfold @approx_ext_or_bts.
  allunfold @lblift; repnd.
...
Lemma respects_alpha_r_approx_ext_aux {o} :
  forall lib (r : bin_rel (@NTerm o)),
    respects_alpha_r r
    -> respects_alpha_r (approx_ext_aux lib r).
Proof.
  ...
  allunfold @close_compute_ext; repnd; dands; tcsp.
...
Lemma respects_alpha_l_approx_ext_aux {o} :
  forall lib (r : bin_rel (@NTerm o)),
    respects_alpha_l r
    -> respects_alpha_l (approx_ext_aux lib r).
Proof.
  introv resp; introv aeq apr.
...
Proof.
  intros.
  assert (SIM: approx_ext_aux lib (r0 \2/ l) x0 x1) by eauto 6 with slow.
...
(*
Lemma approx_ext_change_utoks_lsubst_aux {o} :
  forall lib (t1 t2 : @NTerm o) sub1 sub2 l,
    ...
    -> approx_ext lib (lsubst_aux t1 sub1) (lsubst_aux t2 sub1)
    -> approx_ext lib (lsubst_aux t1 sub2) (lsubst_aux t2 sub2).
Proof.
  intro lib.
  ...
  pose proof (respects_alpha_r_approx_ext_aux lib r rar) as rar_aa.
  pose proof (respects_alpha_l_approx_ext_aux lib r ral) as ral_aa.
  ...
  - clear cl3 cl.
    ...
    assert (approx_ext_or_bts lib bot2 bs' tr_subterms) as apr1 by sp.

    pose proof (approx_ext_or_bts_alpha_eq_bterms_l
                  lib bs' (lsubst_bterms_aux bs sub1) tr_subterms bot2) as apr2.
    repeat (autodimp apr2 hyp); eauto 2 with slow.

    pose proof (approx_ext_or_bts_alpha_eq_bterms_r
    ...
    fold (approx_ext_or_bts lib r tl_subterms bs'0).
    eapply approx_ext_or_bts_alpha_eq_bterms_l;[apply alpha_eq_bterms_sym;exact aebs2|].
    eapply approx_ext_or_bts_alpha_eq_bterms_r;[apply alpha_eq_bterms_sym;exact aebs1|].

    clear h6 h0 q0 q6.

    unfold approx_ext_or_bts in apr3; unfold approx_ext_or_bts.
...
(* XXXXXXXXXXXX *)
    ...
    apply approx_ext_open_simpler_equiv; eauto 3 with slow.
...
(* rename the tokens of sub to fresh tokens *)
    ...
    pose proof (respects_alpha_r_approx_ext_aux_bot2 lib) as respr.
    ...
    pose proof (respects_alpha_l_approx_ext_aux_bot2 lib) as respl.
...
(*Lemma approx_ext_change_utoks {o} :
  forall lib (t1 t2 : @NTerm o) ren,
    ...
    -> approx_ext lib t1 t2
    -> approx_ext lib (ren_utokens ren t1) (ren_utokens ren t2).
Proof.
  ...
  (*inversion apr as [? ? ? cl]; subst; clear apr.*)
  inversion apr as [|? ? ? cl]; clear apr; subst.
  allunfold @close_compute_ext; repnd; dands; tcsp; eauto with slow; introv comp.
...
(*
Lemma ren_utokens_lsubst_aux_approx_ext {o} :
  forall lib (t1 t2 : @NTerm o) ren sub,
    ...
    -> disjoint (range_utok_ren ren) (diff (get_patom_deq o) (dom_utok_ren ren) (get_utokens t1))
    -> disjoint (range_utok_ren ren) (diff (get_patom_deq o) (dom_utok_ren ren) (get_utokens t2))
    -> (forall sub, prog_sub sub -> approx_ext lib (lsubst_aux t1 sub) (lsubst_aux t2 sub))
    -> approx_ext lib (lsubst_aux (ren_utokens ren t1) sub) (lsubst_aux (ren_utokens ren t2) sub).
Proof.
  intro lib.
  ...
  - pose proof (imp sub ps) as h.
    apply approx_ext_relates_only_progs in h; repnd.
    rw <- @cl_lsubst_lsubst_aux in h0; eauto 2 with slow.
...
Proof.
  intros.
  assert (SIM: approx_ext_aux lib (r0 \2/ l) x0 x1) by auto.
  clear PR; revert x0 x1 SIM; cofix CIH.
  intros; destruct SIM; econstructor; eauto.
  invertsna c Hcl. repnd.
  unfold close_comput.
