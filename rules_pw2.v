...
(* Same as rules_pw.v but without the second sequent and where the parameter
 * in the main sequent is a variable
 *
 * !To remove *)
...
(*
   H, q : P , w : pW(P;ap.A;bp,ba.B;cp,ca,cb.C;q) |- Q ext Ax
    ...
    [ mk_nwfsequent (snoc (snoc (snoc (snoc H (mk_hyp q P))
                          ...
                          (mk_hyp v (mk_function
                                       ...
                                       (lsubst Q [(w,mk_apply (mk_var f) (mk_var b)),
                                                  (q,lsubst C [(cp,mk_var q),(ca,mk_var a),(cb,mk_var b)])]))))
...
Lemma rule_pw_induction_true :
  forall (H : nwfhypotheses)
    ...
    rule_true (rule_pw_induction H Q P A B C i ap bp ba cp ca cb w b a f v q).
Proof.
  ...
  exists (covered_axiom (nh_vars_hyps (snoc (snoc H (mk_hyp q P)) (mk_hyp w (mk_pw P ap A bp ba B cp ca cb C (mk_var q)))))).
...
*)
  ...
  generalize (param_w_ind3
                ...
                (fun p1 p2 t1 t2 =>
                   forall (c1 : cover_vars Q (snoc s1a (w, t1)))
                     ...
                     tequality
                       (mkc_apply (lsubstc Q wfct (snoc s1a (w, t1)) c1) p1)
                       (mkc_apply (lsubstc Q wfct (snoc s2a (w, t2)) c0) p2)
                       # member mkc_axiom
                       (mkc_apply (lsubstc Q wfct (snoc s1a (w, t1)) c1) p1)));
    intros h.
  ...
  assert (cequivc
            (mkc_apply (lsubstc Q wfct (snoc s1a (w, t0)) cov1) p1)
            (mkc_apply (lsubstc Q wfct (snoc s1a (w, t4)) c1) p1))
         as cequiv1.
  (* begin proof of assert *)
  ...
  assert (cequivc
            (mkc_apply (lsubstc Q wfct (snoc s2a (w, t3)) cov2) p2)
            (mkc_apply (lsubstc Q wfct (snoc s2a (w, t5)) c0) p2))
         as cequivc2.
  (* begin proof of assert *)
  apply implies_cequivc_apply; try (complete sp).
  unfold cequivc; simpl.
  ...
  apply tequality_respects_cequivc_left
        with (T1 := mkc_apply (lsubstc Q wfct (snoc s1a (w, t0)) cov1) p1);
    try (complete sp).
  apply tequality_respects_cequivc_right
        with (T2 := mkc_apply (lsubstc Q wfct (snoc s2a (w, t3)) cov2) p2);
    try (complete sp).

  apply cequivc_preserving_equality
        with (A := mkc_apply (lsubstc Q wfct (snoc s1a (w, t0)) cov1) p1);
    try (complete sp).
  ...
  repeat (dest_imp k hyp); try (complete (allapply similarity_dom; repnd; allrw; sp)).
  (* end proof of assert *)
  ...
  assert (lsubstc (lsubst C [(cp, mk_var q), (ca, mk_var a), (cb, get_cterm a0)])
                  wp0 (snoc (snoc s1a (q, t7)) (a, t5)) cvp0
          = lsubstc3 cp t7 ca t5 cb a0
                     (lsubstc_vars C wC (csub_filter s1a [cp, ca, cb]) [cp, ca, cb] cvC))
  ...
  assert (lsubstc (lsubst C [(cp, mk_var q), (ca, mk_var a), (cb, get_cterm a')])
                  wp0 (snoc (snoc s2a0 (q, t8)) (a, t6)) cvp0
          = lsubstc3 cp t8 ca t6 cb a'
  ...
  intro eq; repeat (dest_imp eq hyp); try (complete (repeat (rw not_over_or in bc7); sp)).
  revert wfc cvc; rw eq; introv; clear eq.
  ...
  generalize (lsubstc_snoc_snoc
                ...
                (lsubstc3 cp t7 ca t5 cb a0
                          (lsubstc_vars C wC (csub_filter s1a [cp, ca, cb])
                                                        [cp, ca, cb] cvC))
                (mkc_apply t0 a0)
                w1 c12); intro e; dest_imp e hyp; exrepnd; proof_irr.
  allrw.
  ...
  generalize (lsubstc_snoc_snoc
                ...
                (lsubstc3 cp t8 ca t6 cb a'
                          (lsubstc_vars C wC (csub_filter s2a0 [cp, ca, cb])
                                        [cp, ca, cb] cvC0))
                (mkc_apply t4 a')
                w1 c14); intro e; dest_imp e hyp; exrepnd; proof_irr.
  allrw.
