...
Lemma per_product_bar_implies_per_bar {o} :
  forall ts uk lib (T T' : @CTerm o) eq,
    per_product_bar ts uk lib T T' eq
    -> per_bar (per_product_bar ts) uk lib T T' eq.
Proof.
  introv per.
...
Lemma per_product_eq_bar_change_pers2 {o} :
  forall ts uk (lib lib0 : @library o) T T' A A' v v' B B'
    ...
    -> in_ext_ext lib (fun lib' x => type_sys_props4 ts uk lib' A A' (eqa lib' x))
    -> in_ext_ext lib (fun lib' x => forall a a' (e : eqa lib' x a a'), type_sys_props4 ts uk lib' (substc a v B) (substc a' v' B') (eqb lib' x a a' e))
    -> per_product_bar ts uk lib0 T T' eqa'
    -> per_product_bar ts uk lib0 T T' eqb'
    -> eqa' t1 t2
    -> eqb' t1 t2.
Proof.
...
Lemma local_per_bar_per_product_bar {o} :
  forall (ts : cts(o)) uk lib T A v B A' v' B' (eqa : lib-per(lib,o)) (eqb : lib-per-fam(lib,eqa,o)),
    ...
    -> T ===>(lib) (mkc_product A v B)
    -> local_ts_T (per_bar (per_product_bar ts)) uk lib T.
Proof.
...

  {
    ...
    eapply (per_product_eq_bar_change_pers2 ts uk lib lib'2 T T' A A' v v' B B');
    ...
    -> in_ext_ext
         ...
         (fun lib' x =>
            forall a a' (e : eqa lib' x a a'),
              type_sys_props4 ts uk lib' (substc a v' B') (substc a' v B) (eqb lib' x a a' e))
    -> T ===>(lib) (mkc_product A v B)
    -> local_ts_T2 (per_bar (per_product_bar ts)) uk lib T.
Proof.
  introv tsa tsb comp eqiff alla.
  unfold per_bar in *.
...
(*
Definition per_product_bar_or {o} ts uk lib (T T' : @CTerm o) eq :=
  per_product_bar ts uk lib T T' eq
