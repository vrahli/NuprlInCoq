...
Lemma per_qnat_monotone_func2 {o} :
  forall (ts : cts(o)), type_monotone_func2 (per_qnat ts).
Proof.
  introv per.
...
Proof.
  ...
  apply (lib_extends_preserves_in_open_bar_ext _ _ _ y) in h; auto.
  eapply in_open_bar_ext_pres; try exact h; eauto; clear h; introv h; simpl in *.
  introv.
  assert (eqa lib'0 (lib_extends_trans (lib_extends_trans e y) x) a0 a') as f by (eapply (lib_per_cond _ eqa); eauto).
...
Lemma sub_per_weq_bar2 {o} :
  forall (lib lib' lib'' : @library o)
         (x : lib_extends lib' lib)
         (y : lib_extends lib'' lib')
         (w : lib_extends lib'' lib) eqa eqb,
    sub_per (weq_bar lib' (raise_lib_per eqa x) (raise_lib_per_fam eqb x))
            (weq_bar lib'' (raise_lib_per eqa w) (raise_lib_per_fam eqb w)).
Proof.
  introv y h.
  unfold weq_bar in *; exrepnd.
  apply (lib_extends_preserves_in_open_bar_ext _ _ _ y) in h; auto.
  eapply in_open_bar_ext_pres; try exact h; eauto; clear h; introv h; simpl in *.
  eapply weq_eq_term_equals; try exact h; try apply lib_per_cond.
  introv; try apply lib_per_fam_cond.
Qed.
Hint Resolve sub_per_weq_bar2 : slow.

Lemma sub_lib_per_weq_bar_lib_per {o} :
  forall {lib lib'} (x : @lib_extends o lib' lib) eqa eqb,
    sub_lib_per (weq_bar_lib_per lib eqa eqb) x.
Proof.
  introv h z; simpl in *.
  eapply sub_per_weq_bar2;[|eauto];auto.
Qed.
Hint Resolve sub_lib_per_weq_bar_lib_per : slow.

Lemma per_w_bar_monotone_func2 {o} :
  forall (ts : cts(o)), type_monotone_func2 (per_w_bar ts).
Proof.
  introv per.
...
Lemma sub_per_per_bar_eq2 {o} :
  forall {lib lib' lib'' : @library o}
         ...
         (y : lib_extends lib'' lib')
         (w : lib_extends lib'' lib) eqa,
    sub_per (per_bar_eq lib' (raise_lib_per eqa x))
            (per_bar_eq lib'' (raise_lib_per eqa w)).
Proof.
  introv y h.
  unfold per_bar_eq in *; exrepnd; simpl in *.
  apply (lib_extends_preserves_in_open_bar_ext _ _ _ y) in h; simpl in *; auto.
...
Lemma sub_lib_per_per_bar_eq_bar_lib_per {o} :
  forall {lib lib'} (x : @lib_extends o lib' lib) eqa,
    sub_lib_per (per_bar_eq_bar_lib_per lib eqa) x.
Proof.
  introv h z; simpl in *.
...
Proof.
  ...
  eapply in_open_bar_ext_pres; try exact h; eauto; clear h; introv h; simpl in *.
  exrepnd.
  assert (eqa lib'0 (lib_extends_trans e w) a b) as f by (eapply (lib_per_cond _ eqa); eauto).
  exists f.
...
Proof.
  ...
  eapply in_open_bar_ext_pres; try exact h; eauto; clear h; introv h; simpl in *.
  exrepnd.
  assert (eqa lib'0 (lib_extends_trans e w) a0 a') as f by (eapply (lib_per_cond _ eqa); eauto).
...
Proof.
  ...
  - Case "CL_bar".
    pose proof (per_bar_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_int".
    pose proof (per_int_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_nat".
    pose proof (per_nat_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_qnat".
    pose proof (per_qnat_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_csname".
    pose proof (per_csname_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_atom".
    pose proof (per_atom_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_uatom".
    pose proof (per_uatom_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_base".
    pose proof (per_base_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_approx".
    pose proof (per_approx_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_cequiv".
    pose proof (per_cequiv_monotone_func2 ts uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_eq".
    pose proof (per_eq_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_qtime".
    pose proof (per_qtime_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_qlt".
    pose proof (per_qlt_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_func".
    pose proof (per_func_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.
  ...
  - Case "CL_image".
    pose proof (per_image_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_ffdefs".
    pose proof (per_ffdefs_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_set".
    pose proof (per_set_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.

  - Case "CL_product".
    pose proof (per_product_monotone_func2 (close ts) uk lib T T' eq) as q.
    repeat (autodimp q hyp).
    exrepnd; exists eq'; introv; pose proof (q0 _ x) as q0;
      repnd; dands; eauto 3 with slow.
Qed.
...
Lemma sub_lib_per_univi_eq_lib_per {o} :
  forall uk {lib' lib} (x : @lib_extends o lib' lib) j,
    @type_monotone o (univi j)
    -> sub_lib_per (univi_eq_lib_per uk lib j) x.
Proof.
