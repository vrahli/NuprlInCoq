...
Lemma local_equality_of_refname_bar {o} :
  forall {lib : SL} (bar : @BarLib o lib) n t1 t2,
    all_in_bar_ext bar (fun lib' (x : lib_extends lib' lib) => equality_of_refname_bar lib' n t1 t2)
...
Proof.
  ...
  eapply alla0; eauto.
Qed.
Hint Resolve local_equality_of_refname_bar : slow.

Lemma per_bar_eq_equality_of_refname_bar_implies {o} :
  forall {lib : SL} (bar : @BarLib o lib) n t1 t2,
    per_bar_eq bar (equality_of_refname_bar_lib_per lib n) t1 t2
    -> equality_of_refname_bar lib n t1 t2.
Proof.
  introv alla.
  ...
  eapply alla0; eauto.
Qed.
Hint Resolve per_bar_eq_equality_of_refname_bar_implies : slow.

Lemma all_in_bar_ext_equal_equality_of_refname_bar_implies_per_bar_eq_implies_equality_of_refname_bar {o} :
  forall (lib : SL) (bar : @BarLib o lib) (eqa : lib-per(lib,o)) n,
    all_in_bar_ext bar (fun lib' x => (eqa lib' x) <=2=> (equality_of_refname_bar lib' n))
    -> (per_bar_eq bar eqa) <=2=> (equality_of_refname_bar lib n).
Proof.
  introv alla; introv; split; introv h.

  - pose proof (all_in_bar_ext_eq_term_equals_preserves_per_bar_eq
                  _ bar eqa (equality_of_refname_bar_lib_per lib n) t1 t2 alla h) as q.
    eauto 3 with slow.
  ...
  - introv br ext; introv.
    ...
    eapply sub_per_equality_of_refname_bar;[|eauto]; eauto 3 with slow.
Qed.
Hint Resolve all_in_bar_ext_equal_equality_of_refname_bar_implies_per_bar_eq_implies_equality_of_refname_bar : slow.

Definition equality_of_refname_bar_to_lib_per {o}
           (lib : SL)
           (T : @CTerm o) : lib-per(lib,o).
Proof.
  exists (fun (lib' : SL) (x : lib_extends lib' lib) t t' =>
            {n : nat , T ===>(lib') (mkc_refname n) # equality_of_refname_bar lib' n t t' }).
  introv x y; introv; simpl; tcsp.
Defined.

Lemma local_per_refname_bar {o} :
  forall (ts : cts(o)), local_ts (per_bar (per_refname ts)).
Proof.
  introv eqiff alla.

  apply all_in_bar_ext_exists_bar_implies in alla; exrepnd.
  exists (bar_of_bar_fam fbar).
  exists (equality_of_refname_bar_to_lib_per lib T).
  dands.
...

  {
    ...
    - rw @per_bar_eq_iff in h; unfold per_bar_eq_bi in h; exrepnd.
      ...
      unfold per_refname in allb1; exrepnd.
    ...
    - rw @per_bar_eq_iff.
      ...
      unfold per_refname in allb0; exrepnd.
      eapply (lib_per_cond _ eqa0); apply allb2.
      spcast; computes_to_eqval; apply_cequivc_val; auto.
  }
Qed.

Lemma per_refname_implies_per_bar_per_refname {o} :
  forall ts lib (T T' : @CTerm o) eq,
    per_refname ts lib T T' eq
    -> per_bar (per_refname ts) lib T T' eq.
Proof.
  introv per.
  unfold per_refname in per; exrepnd.
  ...
  - eapply eq_term_equals_trans;[eauto|].
    ...
    + pose proof (h lib (lib_extends_refl lib) lib (lib_extends_refl lib) (lib_extends_refl lib)) as h; simpl in *; auto.
      ...
      eapply h1; eauto.
Qed.
Hint Resolve per_refname_implies_per_bar_per_refname : slow.


(* ====== dest lemmas ====== *)

Lemma dest_close_per_refname_l {p} :
...
(*Lemma dest_close_per_refname_bar_l {p} :
  forall (ts : cts(p)) lib T T' eq (bar : BarLib lib),
    type_system ts
    -> defines_only_universes ts
    -> type_monotone ts
    -> all_in_bar bar (fun lib => T ===>(lib) mkc_refname)
    -> close ts lib T T' eq
...
Lemma dest_close_per_refname_bar_r {p} :
  forall (ts : cts(p)) lib T T' eq (bar : BarLib lib),
    type_system ts
    -> defines_only_universes ts
    -> type_monotone ts
...
Lemma dest_close_per_refname_ceq_bar_l {p} :
  forall (ts : cts(p)) lib T T' eq (bar : BarLib lib),
    type_system ts
    -> defines_only_universes ts
    -> type_monotone ts
...
Lemma dest_close_per_refname_ceq_bar_r {p} :
  forall (ts : cts(p)) lib T T' eq (bar : BarLib lib),
    type_system ts
    -> defines_only_universes ts
    -> type_monotone ts
