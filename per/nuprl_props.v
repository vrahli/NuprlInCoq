...
Lemma nuprli_refl {p} :
  forall uk lib i t1 t2 eq,
    @nuprli p i uk lib t1 t2 eq -> nuprli i uk lib t1 t1 eq.
Proof.
  intros.
...
Lemma equality_eq_refl {p} :
  forall uk lib A B a b eq,
    @nuprl p uk lib A B eq
    -> eq a b
    -> eq a a.
Proof.
  sp; nts.
...
Lemma equality_sym {p} :
  forall uk lib t1 t2 T,
    @equality p uk lib t1 t2 T -> equality uk lib t2 t1 T.
Proof.
  unfold equality; sp.
  exists eq; sp.
...
Lemma equality_trans {p} :
  forall uk lib t1 t2 t3 T,
    @equality p uk lib t1 t2 T -> equality uk lib t2 t3 T -> equality uk lib t1 t3 T.
Proof.
...
Lemma tequality_sym {p} :
  forall uk lib t1 t2,
    @tequality p uk lib t1 t2 -> tequality uk lib t2 t1.
Proof.
  unfold tequality; sp.
  exists eq.
  apply nuprl_sym; sp.
...
Lemma tequality_refl {p} :
  forall uk lib t1 t2,
    @tequality p uk lib t1 t2 -> tequality uk lib t1 t1.
Proof.
...
Lemma tequality_trans {p} :
  forall uk lib t1 t2 t3,
    @tequality p uk lib t1 t2 -> tequality uk lib t2 t3 -> tequality uk lib t1 t3.
Proof.
  unfold tequality; sp.
  exists eq0.
...
Lemma member_eq {p} :
  forall uk lib t T,
    @equality p uk lib t t T = member uk lib t T.
Proof.
  unfold member; auto.
Qed.
...
Lemma fold_type {p} :
  forall uk lib T,
    @tequality p uk lib T T = type uk lib T.
Proof.
  unfold type; auto.
Qed.
...
Lemma fold_equorsq {p} :
  forall uk lib t1 t2 T,
    (@equality p uk lib t1 t2 T {+} ccequivc_ext lib t1 t2) = equorsq uk lib t1 t2 T.
Proof. sp. Qed.

Lemma fold_equorsq2 {p} :
  forall uk lib t1 t2 t3 t4 T,
    (@equorsq p uk lib t1 t2 T # equorsq uk lib t3 t4 T) = equorsq2 uk lib t1 t2 t3 t4 T.
Proof. sp. Qed.

Lemma equality_refl {p} :
  forall uk lib t1 t2 T,
    @equality p uk lib t1 t2 T -> member uk lib t1 T.
Proof.
...
Lemma tequality_iff_nuprl {p} :
  forall uk lib a b,
    {eq : per(p) , nuprl uk lib a b eq} <=> tequality uk lib a b.
Proof.
  sp.
Qed.

Lemma tequality_if_nuprl {p} :
  forall uk lib a b eq,
    @nuprl p uk lib a b eq -> tequality uk lib a b.
Proof.
  sp.
...
Lemma equality_eq {p} :
  forall uk lib A a b eq,
    @nuprl p uk lib A A eq
    -> (eq a b <=> equality uk lib a b A).
Proof.
  sp; split; intro k.
...
Proof.
  ...
  rw <- (equality_eq1 uk lib A B a c eq n).
  rw <- (equality_eq1 uk lib B A b d eq n0).
  nts.
  split; sp.
...
Lemma eq_equality2 {p} :
  forall uk lib a b A B (eq : per(p)),
    eq a b
    -> nuprl uk lib A B eq
    -> equality uk lib a b A.
Proof.
...
Lemma eq_equality3 {p} :
  forall uk lib a b A B (eq : per(p)) i,
    eq a b
...
Lemma cequivc_preserving_equality {p} :
  forall uk lib a b A B,
    @equality p uk lib a b A
    -> ccequivc_ext lib A B
    -> equality uk lib a b B.
Proof.
...
Proof.
  ...
  apply (nts_tyv uk) with (eq := eq) in al; sp.
  exists eq; sp.
  generalize (type_system_type_mem2 nuprl uk lib A B eq); sp.
Qed.
...
Proof.
  ...
  nts.
  assert (nuprl uk lib A A eq # nuprl uk lib B B eq) as n; repd.
...
Lemma tequalityi_refl {p} :
  forall uk lib l T1 T2, @tequalityi p lib uk l T1 T2 -> tequalityi lib uk l T1 T1.
Proof.
...
Lemma eqtypes_refl {p} :
  forall u lib l T1 T2, @eqtypes p lib u l T1 T2 -> eqtypes lib u l T1 T1.
Proof.
  destruct l; simpl; sp.
...
Lemma dest_nuprl_uni {o} :
  forall uk (lib : @library o) i eq,
    nuprl uk lib (mkc_uni uk i) (mkc_uni uk i) eq
    -> univ uk lib (mkc_uni uk i) (mkc_uni uk i) eq.
Proof.
  introv cl.
...
Lemma univ_implies_univi_bar {o} :
  forall uk lib i (eq : per(o)),
    univ uk lib (mkc_uni uk i) (mkc_uni uk i) eq
    -> exists j, i < j # univi_bar j uk lib (mkc_uni uk i) (mkc_uni uk i) eq.
Proof.
  introv u.
  ...
  apply ccequivc_ext_uni_uni_implies in u3; repnd; subst.
  exists j; dands; eauto 3 with slow.
Qed.

Lemma univ_implies_univi_bar2 {o} :
  forall (uk : ukind) lib i (eq : per(o)),
    univ uk lib (mkc_uni uk i) (mkc_uni uk i) eq
    ->
    exists (eqa : lib-per(lib,o)),
      (eq <=2=> (per_bar_eq lib eqa))
        # in_open_bar_ext lib (fun lib' x => (eqa lib' x) <=2=> (univi_eq (univi_bar i) uk lib')).
Proof.
  introv u.
  ...
  allrw @univi_exists_iff; exrepnd; GC.
  apply ccomputes_to_valc_ext_implies_ccequivc_ext in u3.
  apply ccequivc_ext_uni_uni_implies in u3; repnd; subst; auto.
Qed.

Lemma univ_implies_univi_bar3 {o} :
  forall uk lib i (eq : per(o)),
    univ uk lib (mkc_uni uk i) (mkc_uni uk i) eq
    -> eq <=2=> (per_bar_eq lib (univi_eq_lib_per uk lib i)).
Proof.
  introv u.
...
Lemma univi_eq_preserves_nuprl {o} :
  forall uk i lib (A B : @CTerm o) eq,
    univi_eq (univi_bar i) uk lib A B
    -> nuprl uk lib A A eq
    -> nuprl uk lib B B eq.
Proof.
  introv u n.
  ...
  assert (nuprl uk lib A A eqa) as h.
...
Lemma tequalityi_preserving_equality {o} :
  forall uk lib i a b (A B : @CTerm o),
    equality uk lib a b A
    -> tequalityi lib uk i A B
    -> equality uk lib a b B.
Proof.
...
Lemma eqtypes_preserving_equality {p} :
  forall uk lib l a b A B,
    @equality p uk lib a b A
    -> eqtypes uk lib l A B
    -> equality uk lib a b B.
Proof.
...
Lemma tequalityi_sym {p} :
  forall uk lib i T1 T2, @tequalityi p lib uk i T1 T2 -> tequalityi lib uk i T2 T1.
Proof.
...
Lemma eqtypes_sym {p} :
  forall uk lib l T1 T2, @eqtypes p uk lib l T1 T2 -> eqtypes uk lib l T2 T1.
Proof.
...
Lemma tequalityi_trans {p} :
  forall uk lib i t1 t2 t3,
    @tequalityi p lib uk i t1 t2
    -> tequalityi lib uk i t2 t3
    -> tequalityi lib uk i t1 t3.
Proof.
...
Lemma eqtypes_trans {p} :
  forall uk lib l t1 t2 t3,
    @eqtypes p uk lib l t1 t2
    -> eqtypes uk lib l t2 t3
    -> eqtypes uk lib l t1 t3.
Proof.
...
Lemma equality3_implies_cequorsq2 {p} :
  forall uk lib a b c d T,
    @equality p uk lib a c T
    -> equality uk lib b d T
    -> equality uk lib a b T
    -> equorsq2 uk lib a b c d T.
Proof.
  sp.
...
Lemma inhabited_implies_tequality {p} :
  forall uk lib t1 t2 T,
    @equality p uk lib t1 t2 T -> type uk lib T.
Proof.
...
Lemma cequivc_equality {p} : forall uk lib, respects3 (ccequivc_ext lib) (@equality p uk lib).
Proof.
...
Lemma cequorsq2_prop {p} :
  forall uk lib A a1 a2 b1 b2,
    @equorsq2 p uk lib a1 b1 a2 b2 A
    -> equality uk lib a1 a2 A
    -> equality uk lib a1 b2 A.
Proof.
  introv ceq e1.
...
Lemma cequorsq_equality_trans1 {p} :
  forall uk lib t1 t2 t3 T,
    @equorsq p uk lib t1 t2 T
    -> equality uk lib t2 t3 T
    -> equality uk lib t1 t3 T.
Proof.
  introv c e.
...
Lemma cequorsq_equality_trans2 {p} :
  forall uk lib t1 t2 t3 T,
    @equality p uk lib t1 t2 T
    -> equorsq uk lib t2 t3 T
    -> equality uk lib t1 t3 T.
Proof.
  introv e c.
  unfold equorsq in c; sp.
...
Lemma cequorsq_sym {p} :
  forall uk lib a b T, @equorsq p uk lib a b T -> equorsq uk lib b a T.
Proof.
...
Lemma respects_alphaeqc_equality {p} :
  forall uk lib, respects3 alphaeqc (@equality p uk lib).
Proof.
  introv; unfolds_base; dands; introv Hr Heq;
  eauto 3 with nequality.
Qed.
...
Lemma respects_alphaeqc_tequality {p} :
  forall uk lib, respects2 alphaeqc (@tequality p uk lib).
Proof.
  introv; unfolds_base; dands; introv Hr Heq;
  eauto 3 with nequality.
Qed.
...
Lemma respects_cequivc_tequality {p} :
  forall uk lib, respects2 (ccequivc_ext lib) (@tequality p uk lib).
Proof.
  introv; unfolds_base; dands; introv Hr Heq;
  eauto 3 with nequality.
Qed.
...
Lemma respects_cequivc_equality {p} :
  forall uk lib, respects3 (ccequivc_ext lib) (@equality p uk lib).
Proof.
  introv; unfolds_base; dands; introv Hr Heq;
  eauto 3 with nequality.
Qed.
...
Lemma nuprli_sym {p} :
  forall uk lib i t1 t2 eq,
    @nuprli p i uk lib t1 t2 eq -> nuprli i uk lib t2 t1 eq.
Proof.
  intros.
  generalize (@nuprli_type_system p i); intro nts.
  dest_ts nts.
...
Lemma nuprl_ext {p} :
  forall uk lib t1 t2 eq1 eq2,
    @nuprl p uk lib t1 t2 eq1
    -> eq_term_equals eq1 eq2
    -> nuprl uk lib t1 t2 eq2.
Proof.
  introv n1 eqs; nts.
...
Lemma nuprli_ext {p} :
  forall uk lib i t1 t2 eq1 eq2,
    @nuprli p i uk lib t1 t2 eq1
    -> eq1 <=2=> eq2
    -> nuprli i uk lib t1 t2 eq2.
Proof.
  introv n1 eqs.
  generalize (@nuprli_type_system p i); intro nts.
  dest_ts nts.
...
Lemma eqorceq_iff_equorsq {o} :
  forall uk lib (A B a b : @CTerm o) eq,
    nuprl uk lib A B eq
    -> (eqorceq lib eq a b <=> equorsq uk lib a b A).
Proof.
  introv n.
  unfold eqorceq, equorsq.
  split; intro k; dorn k; auto; left; apply nuprl_refl in n; auto.
  - exists eq; dands; auto.
  - pose proof (equality_eq uk lib A a b eq n) as h; apply h; auto.
Qed.

Lemma equorsq_tequality {o} :
  forall uk lib (A B a b : @CTerm o),
    equorsq uk lib a b A
    -> tequality uk lib A B
    -> equorsq uk lib a b B.
Proof.
  introv e t.
  allunfold @equorsq.
  dorn e; auto.
...
Lemma respects_cequivc_equorsq {p} :
  forall uk lib, respects3 (ccequivc_ext lib) (@equorsq p uk lib).
Proof.
...
Lemma respects_alphaeqc_equorsq {p} :
  forall uk lib, respects3 alphaeqc (@equorsq p uk lib).
Proof.
  unfold equorsq; introv; unfolds_base; dands; introv Hr Heq; allsimpl; dorn Heq;
  try (complete (left; eauto 3 with nequality));
  right; spcast; eauto 3 with nequality.
...
Lemma equorsq_sym {o} :
  forall uk lib (A a b : @CTerm o),
    equorsq uk lib a b A -> equorsq uk lib b a A.
Proof.
  introv e.
...
Lemma eqorceq_commutes_nuprl {p} :
  forall uk lib (a b c d : @CTerm p) eq A B,
    nuprl uk lib A B eq
    -> eqorceq lib eq a b
    -> eqorceq lib eq c d
    -> eq a c
...
Lemma eqorceq_sym_trans {p} :
  forall uk lib eq (a b A B : @CTerm p),
    nuprl uk lib A B eq
    -> eqorceq lib eq a b
    -> eqorceq lib eq b a.
Proof.
...
Lemma nuprl_refl2 {p} :
  forall uk lib (t1 t2 : @CTerm p) eq,
    nuprl uk lib t1 t2 eq -> nuprl uk lib t2 t2 eq.
Proof.
  introv n.
...
Lemma nuprl_uniquely_eq_ext {p} :
  forall uk lib (t1 t2 : @CTerm p) eq1 eq2,
    nuprl uk lib t1 t2 eq1
    -> eq_term_equals eq1 eq2
    -> nuprl uk lib t1 t2 eq2.
Proof.
  introv n e; nts.
...
Lemma equality_or_cequivc_eqorceq {p} :
  forall uk lib (A a b : @CTerm p) eq,
    nuprl uk lib A A eq
    -> (eqorceq lib eq a b <=> (equality uk lib a b A {+} ccequivc_ext lib a b)).
Proof.
...
Lemma eqorceq_implies_equality_or_cequivc {p} :
  forall uk lib (A a b : @CTerm p) eq,
    nuprl uk lib A A eq
    -> eqorceq lib eq a b
    -> (equality uk lib a b A {+} ccequivc_ext lib a b).
Proof.
  introv n e.
  generalize (equality_or_cequivc_eqorceq uk lib A a b eq); sp.
  allrw <-; sp.
Qed.
...
Lemma dest_nuprl_approx {o} :
  forall uk (lib : @library o) (a b c d : @CTerm o) eq,
    nuprl uk lib (mkc_approx a b) (mkc_approx c d) eq
    -> per_bar (per_approx nuprl) uk lib (mkc_approx a b) (mkc_approx c d) eq.
Proof.
  introv cl.
...
Lemma false_not_inhabited {p} :
  forall uk lib (t : @CTerm p), !member uk lib t mkc_false.
Proof.
  introv m.
...
Lemma equality3_implies_equorsq2 {p} :
  forall uk lib (a b c d T : @CTerm p),
    equality uk lib a c T
    -> equality uk lib b d T
    -> equality uk lib a b T
    -> equorsq2 uk lib a b c d T.
Proof.
...
Definition inhabited_type {p} uk lib (T : @CTerm p) := {t : CTerm , member uk lib t T}.

Definition sym_type {p} uk lib (R : @CTerm p) :=
  forall x y,
    inhabited_type uk lib (mkc_apply2 R x y)
    -> inhabited_type uk lib (mkc_apply2 R y x).

Definition trans_type {p} uk lib (R : @CTerm p) :=
  forall x y z,
    inhabited_type uk lib (mkc_apply2 R x y)
    -> inhabited_type uk lib (mkc_apply2 R y z)
    -> inhabited_type uk lib (mkc_apply2 R x z).

Definition is_per_type {p} uk lib (R : @CTerm p) :=
  sym_type uk lib R # trans_type uk lib R.

Lemma is_per_type_iff {p} :
  forall uk lib R1 R2,
    (forall x y : @CTerm p,
       inhabited_type uk lib (mkc_apply2 R1 x y)
       <=>
       inhabited_type uk lib (mkc_apply2 R2 x y))
    -> is_per_type uk lib R1
    -> is_per_type uk lib R2.
Proof.
...
Lemma inhabited_type_if_equality {p} :
  forall uk lib (a b R : @CTerm p), equality uk lib a b R -> inhabited_type uk lib R.
Proof.
  introv eq.
...
Proof.
  ...
  generalize (nuprl_uniquely_valued uk lib T eq0 eq); intro i.
...
Lemma inhabited_type_iff_inhabited {p} :
  forall uk lib (T U : @CTerm p) eq,
    nuprl uk lib T U eq
    -> (inhabited eq <=> inhabited_type uk lib T).
Proof.
...
Lemma inhabited_if_inhabited_type_i {p} :
  forall uk lib (T U : @CTerm p) eq i,
    inhabited_type uk lib T
    -> nuprli i uk lib T U eq
    -> inhabited eq.
Proof.
  introv inh neq.
...
Lemma is_per_type_iff_is_per {p} :
  forall uk lib R eq,
    (forall x y : @CTerm p, nuprl uk lib (mkc_apply2 R x y) (mkc_apply2 R x y) (eq x y))
    -> (is_per eq <=> is_per_type uk lib R).
Proof.
  introv n.
  split; intro isper.
  ...
  - destruct isper as [sym trans].
    ...
    + intro inh.
      generalize (inhabited_type_iff_inhabited uk lib (mkc_apply2 R x y) (mkc_apply2 R x y) (eq x y)).
      intro iff1; dest_imp iff1 hyp.
      generalize (inhabited_type_iff_inhabited uk lib (mkc_apply2 R y x) (mkc_apply2 R y x) (eq y x)).
    ...
    + intros inh1 inh2.
      generalize (inhabited_type_iff_inhabited uk lib (mkc_apply2 R x y) (mkc_apply2 R x y) (eq x y)).
      intro iff1; dest_imp iff1 hyp.
      generalize (inhabited_type_iff_inhabited uk lib (mkc_apply2 R y z) (mkc_apply2 R y z) (eq y z)).
      intro iff2; dest_imp iff2 hyp.
      generalize (inhabited_type_iff_inhabited uk lib (mkc_apply2 R x z) (mkc_apply2 R x z) (eq x z)).
...
Lemma is_per_type_iff_is_per1 {p} :
  forall uk lib R1 R2 eq,
    (forall x y : @CTerm p, nuprl uk lib (mkc_apply2 R1 x y) (mkc_apply2 R2 x y) (eq x y))
    -> (is_per eq <=> is_per_type uk lib R1).
Proof.
  introv n.
...
Lemma inhabited_type_iff {p} :
  forall uk lib (T1 T2 : @CTerm p) eq1 eq2,
    nuprl uk lib T1 T1 eq1
    -> nuprl uk lib T2 T2 eq2
    -> ((inhabited eq1 <=> inhabited eq2)
        <=> (inhabited_type uk lib T1 <=> inhabited_type uk lib T2)).
Proof.
  introv n1 n2.
  generalize (inhabited_type_iff_inhabited uk lib T1 T1 eq1); intro i1.
  dest_imp i1 hyp.
  generalize (inhabited_type_iff_inhabited uk lib T2 T2 eq2); intro i2.
  dest_imp i2 hyp.
  allrw; sp.
Qed.
...
Lemma inhabited_type_cequivc {p} :
  forall uk lib (a b : @CTerm p),
    ccequivc_ext lib a b
    -> inhabited_type uk lib a
    -> inhabited_type uk lib b.
Proof.
  introv ceq inh.
...
Lemma inhabited_type_respects_cequivc {p} :
  forall uk lib, respects1 (@ccequivc_ext p lib) (inhabited_type uk lib).
Proof.
  introv; introv.
...
Proof.
  ...
  exists t.
  generalize (nuprl_uniquely_valued uk lib a eq eq0); intro i; repeat (dest_imp i hyp).
...
Lemma inhabited_type_respects_alphaeqc {o} :
  forall uk lib, respects1 alphaeqc (@inhabited_type o uk lib).
Proof.
  introv aeq inh.
...
Lemma type_respects_cequivc {o} :
  forall uk lib, respects1 (ccequivc_ext lib) (@type o uk lib).
Proof.
  introv ceq typ.
  eapply type_respects_cequivc_left in ceq; eauto.
  apply tequality_refl in ceq; auto.
Qed.
Hint Resolve type_respects_cequivc : respects.

Lemma type_respects_alphaeqc {o} :
  forall uk lib, respects1 alphaeqc (@type o uk lib).
Proof.
  introv aeq inh.
  apply (alphaeqc_implies_ccequivc_ext lib) in aeq.
  eapply type_respects_cequivc_left in aeq; eauto.
...
Lemma member_respects_alphaeqc_l {o} :
  forall uk lib (t1 t2 T : @CTerm o),
    alphaeqc t1 t2 -> member uk lib t1 T -> member uk lib t2 T.
Proof.
  introv aeq mem.
  allunfold @member.
...
Lemma member_respects_alphaeqc_r {o} :
  forall uk lib (t T1 T2 : @CTerm o),
    alphaeqc T1 T2 -> member uk lib t T1 -> member uk lib t T2.
Proof.
  introv aeq mem.
  allunfold @member.
...
Lemma respects_alphaeqc_member {o} :
  forall uk (lib : @library o), respects2 alphaeqc (member uk lib).
Proof.
...
Lemma respects_alphaeqc_equorsq3 {o} :
  forall uk lib (t1 t2 T1 T2 : @CTerm o),
    alphaeqc T1 T2
    -> equorsq uk lib t1 t2 T1
    -> equorsq uk lib t1 t2 T2.
Proof.
  introv aeq e.
  eauto 3 with slow.
  pose proof (respects_alphaeqc_equorsq uk lib) as h.
