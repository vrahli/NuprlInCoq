...
Lemma teq_and_eq_if_equality {o} :
  forall uk lib (A : @NTerm o) a b s1 s2 H wA wa wb ca1 ca2 cb1 cb2 cA1 cA2,
    ...
    -> (forall s1 s2 ca1 cb2 cA1,
          hyps_functionality_ext uk lib s1 H
          -> similarity uk lib s1 s2 H
          -> equality uk lib (lsubstc a wa s1 ca1) (lsubstc b wb s2 cb2) (lsubstc A wA s1 cA1))
    -> (tequality uk lib
        ...
        # equality uk lib (lsubstc a wa s1 ca1) (lsubstc b wb s1 cb1) (lsubstc A wA s1 cA1)).
Proof.
  introv hf sim teq equs.

  assert (hyps_functionality uk lib s1 H) as hf1 by (apply hf; eauto 2 with slow).
  assert (hyps_functionality_ext uk lib s2 H) as hf2 by eauto 3 with slow.
...
Lemma teq_and_member_if_member {o} :
  forall uk lib (A : @NTerm o) a s1 s2 H wA wa ca1 ca2 cA1 cA2,
    hyps_functionality_ext uk lib s1 H
    -> similarity uk lib s1 s2 H
    -> tequality uk lib (lsubstc A wA s1 cA1) (lsubstc A wA s2 cA2)
...
Ltac teq_and_eq T a b s1 s2 H :=
  ...
  match goal with
    | [ lib : library
      ...
      , cT1 : cover_vars T s1
      , cT2 : cover_vars T s2
      , hf  : hyps_functionality_ext _ _ s1 H
      , sim : similarity _ _ s1 s2 H |- _ ] =>
      pose proof (teq_and_eq_if_equality
                    uk lib T a b s1 s2 H
