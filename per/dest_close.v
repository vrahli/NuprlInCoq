...
Proof.
  ...
  allunfold_per.
  uncast.
  unfold per_extensional in p0.
  computes_to_valc_diff.
XXXXXXXXXXXXXXXXX
  assert ({i : nat , ccomputes_to_valc lib T (mkc_uni i)})
        as c1.
  unfold defines_only_universes in dou;
              generalize (type_system_type_mem ts T T' eq);
              intro h.
  repeat (autodimp h h'); try (complete (allunfold @type_system; sp)).
