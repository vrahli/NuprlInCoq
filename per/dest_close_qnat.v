...
Lemma per_bar_eq_equality_of_qnat_bar_implies {o} :
  forall inh (lib : @library o) t1 t2,
    per_bar_eq inh lib (equality_of_qnat_bar_lib_per inh lib) t1 t2
...
Lemma all_in_bar_ext_equal_equality_of_qnat_bar_implies_per_bar_eq_implies_equality_of_qnat_bar {o} :
  forall inh (lib : @library o) (eqa : lib-per(inh,lib,o)),
    in_open_bar_ext inh lib (fun lib' x => (eqa lib' x) <=2=> (equality_of_qnat_bar inh lib'))
    -> (per_bar_eq inh lib eqa) <=2=> (equality_of_qnat_bar inh lib).
Proof.
  introv alla; introv; split; introv h.

  - pose proof (in_open_bar_ext_eq_term_equals_preserves_per_bar_eq
                  _ _ eqa (equality_of_qnat_bar_lib_per inh lib) t1 t2 alla h) as q.
    eauto 3 with slow.
...
Lemma local_per_qnat_bar {o} :
  forall inh (ts : cts(o)), local_ts inh (per_qnat_bar inh ts).
Proof.
  introv eqiff alla.
...
Lemma per_qnat_implies_per_qnat_bar {o} :
  forall inh ts lib (T T' : @CTerm o) eq,
    per_qnat inh ts lib T T' eq
    -> per_qnat_bar inh ts lib T T' eq.
Proof.
  introv per.
