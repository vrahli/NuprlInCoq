...
Lemma dest_nuprl_per_func_l {o} :
  forall (ts : cts(o)) uk lib T A v B T' eq,
    ...
    -> per_bar (per_func_ext (close ts)) uk lib T T' eq.
Proof.
  introv equ comp cl.
...
Lemma dest_nuprl_function {o} :
  forall uk (lib : @library o) (A : @CTerm o) v B C w D eq,
    nuprl uk lib (mkc_function A v B) (mkc_function C w D) eq
    -> per_bar (per_func_ext nuprl) uk lib (mkc_function A v B) (mkc_function C w D) eq.
Proof.
  introv cl.
  unfold nuprl in cl.
...
Lemma type_family_ext_nuprl_function_uniquely_valued_eqas {o} :
  forall uk lib A v B C w D (eqa1 eqa2 : lib-per(lib,o)) (eqb1 : lib-per-fam(lib,eqa1,o)) (eqb2 : lib-per-fam(lib,eqa2,o)),
    type_family_ext mkc_function nuprl uk lib (mkc_function A v B) (mkc_function C w D) eqa1 eqb1
    -> type_family_ext mkc_function nuprl uk lib (mkc_function A v B) (mkc_function C w D) eqa2 eqb2
...
Proof.
  ...
  - Case "->".
    ...
    assert (nuprl uk lib A1 A2 (per_bar_eq lib eqa)) as ea.
...

    {
      ...
      apply (lib_extends_preserves_in_open_bar_ext _ _ _ x) in teq4.
      apply (lib_extends_preserves_in_open_bar_ext _ _ _ x) in teq3.
      eapply in_open_bar_ext_comb; try exact teq4; clear teq4.
      eapply in_open_bar_ext_pres; try exact teq3; clear teq3; introv teq4 teq3; simpl in *.
      fold (@nuprl o).
...
Lemma if_member_function {o} :
  forall uk lib (f : @CTerm o) A v B,
    member uk lib f (mkc_function A v B)
    ->
    forall lib' (x : lib_extends lib' lib) x y,
      equality uk lib' x y A
      -> equality uk lib' (mkc_apply f x) (mkc_apply f y) (substc x v B).
Proof.
  introv m x e.
...

  {
    ...
    apply (lib_extends_preserves_in_open_bar_ext _ _ _ x) in m5.
    eapply in_open_bar_ext_comb; try exact m4; clear m4.
    eapply in_open_bar_ext_pres; try exact m5; clear m5; introv m5 m4; simpl in *.
    fold (@nuprl o).

    assert (eqa lib'0 (lib_extends_trans e0 x) x0 y) as xx.
    {
      pose proof (equality_eq1 uk lib'0 A A x0 y (eqa lib'0 (lib_extends_trans e0 x))) as w.
      apply w; auto; eauto 3 with slow.
    }

    pose proof (m5 _ _ xx) as m5; simpl in *.
    apply nuprl_refl in m5.
    eapply type_extensionality_nuprl;[eauto|].
    introv; split; intro w; eauto;[].
    exrepnd.
    unfold raise_ext_per, raise_ext_per_fam in *.
...

  {
    ...
    apply in_open_bar_ext_twice in m5.
    apply in_open_bar_ext_dup.
    eapply in_open_bar_ext_comb; try exact m0; clear m0.
    eapply in_open_bar_ext_comb; try exact m4; clear m4.
    eapply in_open_bar_ext_pres; try exact m5; clear m5; introv m5 m4 m0; simpl in *.
    ...
    {
      pose proof (equality_eq1 uk lib'1 A A x0 y (eqa lib'1 (lib_extends_trans (lib_extends_trans e1 e0) x))) as w.
...
Lemma implies_member_function {o} :
  forall uk lib (f : @CTerm o) g A v B,
    type uk lib A
    ...
    -> (forall lib' (x : lib_extends lib' lib) a a',
          equality uk lib' a a' A
          -> equality uk lib' (mkc_apply f a) (mkc_apply g a') (substc a v B))
    -> equality uk lib f g (mkc_function A v B).
Proof.
  introv ty esw teq eqap.
  unfold member, equality.
  unfold type, tequality in ty; exrepnd.
  rename eq into eqa.

  pose proof (nuprl_monotone_func uk lib A A eqa ty0) as tya; exrepnd.
  rename eq' into eqa'.
...
Proof.
...

  {
    ...
    {
      pose proof (equality_eq1 uk lib'1 A A a a' (eqa lib'1 (lib_extends_trans (lib_extends_trans e1 e0) x))) as w.
      apply w; auto; eauto 4 with slow.
    }
...
Lemma equality_in_function2 {p} :
  forall uk lib (f g : @CTerm p) A v B,
    equality uk lib f g (mkc_function A v B)
    <=>
    (type uk lib (mkc_function A v B)
     # equality_swap_invariant_cond uk lib A v B v B
     # (forall lib' (x : lib_extends lib' lib) a a',
          equality uk lib' a a' A
          -> equality uk lib' (mkc_apply f a) (mkc_apply g a') (substc a v B))).
Proof.
  ...
  { unfold type. apply tequality_function; tcsp. }

  { apply tequality_function in k0; sp. }
...
Lemma inhabited_function {p} :
  forall uk lib (A : @CTerm p) v B,
    inhabited_type uk lib (mkc_function A v B)
    <=>
    (type uk lib A
     # equality_swap_invariant_cond uk lib A v B v B
     # (forall lib' (x : lib_extends lib' lib) a a', equality uk lib' a a' A -> tequality uk lib' (substc a v B) (substc a' v B))
     # {f : CTerm
        , forall lib' (x : lib_extends lib' lib) a a',
            equality uk lib' a a' A
            -> equality uk lib' (mkc_apply f a) (mkc_apply f a') (substc a v B)}).
Proof.
...
Lemma tequality_function_fun {p} :
  forall uk lib (A : @CTerm p) v B,
    (type uk lib (mkc_function A v (mk_cv [v] B)) {+} type uk lib (mkc_fun A B))
    -> tequality uk lib (mkc_function A v (mk_cv [v] B))
                 (mkc_fun A B).
Proof.
  introv o; repdors.
...
Lemma tequality_mkc_fun_dom {p} :
  forall uk lib (A1 A2 B : @CTerm p),
    tequality uk lib A1 A2
    -> type uk lib (mkc_fun A1 B)
    -> tequality uk lib (mkc_fun A1 B) (mkc_fun A2 B).
Proof.
  introv teqa teqf.
  allrw <- @fold_mkc_fun.
  apply tequality_function in teqf; apply tequality_function; repnd; dands; tcsp.
Qed.


Lemma tequality_fun {p} :
  forall uk lib (A1 A2 B1 B2 : @CTerm p),
    tequality uk lib (mkc_fun A1 B1) (mkc_fun A2 B2)
    <=>
    (tequality uk lib A1 A2
      # (forall lib' (x : lib_extends lib' lib), inhabited_type uk lib' A1 -> tequality uk lib' B1 B2)).
Proof.
  intros.
  allrw <- @fold_mkc_fun.
  rw (@tequality_function p uk).
  split; intro teq; repnd; dands; auto; eauto 2 with slow; introv x e.

  - unfold inhabited_type in e; exrepnd.
    generalize (teq1 lib' x t t); intro k; autodimp k hyp.
...
Lemma equality_in_fun {p} :
  forall uk lib (f g A B : @CTerm p),
    equality uk lib f g (mkc_fun A B)
    <=>
    (type uk lib A
     # (forall lib' (x : lib_extends lib' lib), inhabited_type uk lib' A -> type uk lib' B)
     # (forall lib' (x : lib_extends lib' lib) a a',
          equality uk lib' a a' A
          -> equality uk lib' (mkc_apply f a) (mkc_apply g a') B)).
Proof.
  introv.
  rw <- @fold_mkc_fun.
  rw @equality_in_function.
  split; intro k; repnd; dands; auto; eauto 2 with slow.

  {
    introv x i.
    unfold inhabited_type in i; exrepnd.
    generalize (k2 lib' x t t); intro j; autodimp j hyp.
...
Proof.
  ...
  allrw <- @fold_mkc_fun.
  rw (@tequality_function p uk).
  split; intro k; repnd; dands; eauto 2 with slow; introv x.

  {
    introv i.
    unfold inhabited_type in i; exrepnd.
    generalize (k1 lib' x t t i0); intro teq.
...
Proof.
...

  {
    ...
    eapply nuprli_value_respecting_left in pera5;[|apply ccequivc_ext_sym;eapply lib_extends_preserves_ccequivc_ext;eauto].
...
Lemma dest_nuprli_function {o} :
  forall i uk (lib : @library o) (A : @CTerm o) v B C w D eq,
    nuprli i uk lib (mkc_function A v B) (mkc_function C w D) eq
    -> per_bar (per_func_ext (nuprli i)) uk lib (mkc_function A v B) (mkc_function C w D) eq.
Proof.
  introv cl.
  unfold nuprli in cl.
...
Lemma type_family_ext_nuprli_function_uniquely_valued_eqas {o} :
  forall i uk lib A v B C w D (eqa1 eqa2 : lib-per(lib,o)) (eqb1 : lib-per-fam(lib,eqa1,o)) (eqb2 : lib-per-fam(lib,eqa2,o)),
    type_family_ext mkc_function (nuprli i) uk lib (mkc_function A v B) (mkc_function C w D) eqa1 eqb1
    -> type_family_ext mkc_function (nuprli i) uk lib (mkc_function A v B) (mkc_function C w D) eqa2 eqb2
    -> in_ext_ext lib (fun lib' x => (eqa1 lib' x) <=2=> (eqa2 lib' x)).
Proof.
  introv tfa tfb.
...
Proof.
  ...
  - Case "->".
...

    {
      ...
      unfold univi_eq in *; exrepnd.
      apply dest_nuprli_function2 in h0; exrepnd; eauto 3 with slow.
    }
...
Lemma equality_function_uk0 {o} :
  forall lib (A1 A2 : @CTerm o) v1 v2 B1 B2 i,
    ...
    (equality uk0 lib A1 A2 (mkc_uni uk0 i)
     # forall lib' (x : lib_extends lib' lib) a a',
         equality uk0 lib' a a' A1
         -> equality uk0 lib' (substc a v1 B1) (substc a' v2 B2) (mkc_uni uk0 i)).
Proof.
  introv; rw (@equality_function o uk0); split; intro h; tcsp.
Qed.
