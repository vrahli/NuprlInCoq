...
Lemma pweq_implies2 {o} :
  forall lib (eqp1 eqp2 : per(o)) eqa1 eqa2 eqb1 eqb2
    ...
    -> forall t1 t2,
         ...
         -> pweq lib eqp2 eqa2 eqb2 cp2 ca2 cb2 C2 p2 t1 t2.
Proof.
  introv tes tet tesa teta tesb tetb eqresa eqresb.
  introv eqip eqia eqib eCs eps pweq1.
  revert_dependents p2.
  induction pweq1 as [ p t1 t2 ep a1 f1 a2 f2 ea c1 c2 I R ].

  introv eps.

  assert (eqp1 p2 p2) as ep1 by (apply tet with (t2 := p); sp).
  assert (eqp2 p2 p2) as ep2 by (rw <- eqip; sp).

  assert (eqa1 p2 p2 ep1 a1 a2)
    as ea1
      by (generalize (eqia p2 p2 ep1 ep2 a1 a2); intro e; allrw <-; sp;
          generalize (eqresa p p2 p p2 ep ep1 eps eps); intro eqt;
          rw <- eqt; sp).
  ...
  assert (eqa1 p p2 eps a1 a1)
    as e1
      by (generalize (eqresa p p p p2 ep eps ep eps); introv k;
          rw <- k; clear k;
          generalize (tesa p p ep); intro k1;
          generalize (teta p p ep); intro k2;
          apply k2 with (t2 := a2); sp).

  assert (eqa1 p p2 eps a2 a2)
    as e2
      by (generalize (eqresa p p p p2 ep eps ep eps); introv k;
          rw <- k; clear k;
          generalize (tesa p p ep); intro k1;
  ...
  generalize (eqresb p p2 p p2 ep ep1 a1 a1 a2 a2 ea ea1 eps eps e1 e2).
  introv eqt.
  rw <- eqt in eb1.
  ...
  assert (eqa1 p p2 eps a1 a1)
    as ea3
      by (generalize (eqresa p p p p2 ep eps ep eps); introv k;
          rw <- k; clear k;
          generalize (tesa p p ep); introv k1;
          generalize (teta p p ep); introv k2;
          apply k2 with (t2 := a2); sp).

  assert (eqa1 p p ep a1 a1)
    as e3
      by (generalize (tesa p p ep); introv k1;
          generalize (teta p p ep); introv k2;
          apply k2 with (t2 := a2); sp).

  assert (eqa1 p p2 eps a1 a2)
    as e4 by (generalize (eqresa p p p p2 ep eps ep eps); introv k; rw <- k; sp).

  assert (eqa1 p p2 eps a2 a1)
    as e5 by (generalize (tesa p p2 eps); sp).

  assert (eqb1 p p2 eps a1 a1 ea3 b1 b1)
    as eb3
      by (generalize (eqresb p p p p2 ep eps a1 a1 a2 a1 ea ea3 ep eps e3 e5);
          intro k; rw <- k;
          generalize (tesb p p ep a1 a2 ea); intro k1;
          generalize (tetb p p ep a1 a2 ea); intro k2;
          apply k2 with (t2 := b2); sp).

  apply eCs with (ep := eps) (ea := ea3); auto.
Qed.

Lemma eq_term_equals_pweq {o} :
  forall lib (eqp1 eqp2 : per(o)) eqa1 eqa2 eqb1 eqb2 cp ca cb C p,
    eq_term_equals eqp1 eqp2
    -> eq_term_equals_fam eqp1 eqa1 eqp2 eqa2
    -> eq_term_equals_fam_fam eqp1 eqa1 eqb1 eqp2 eqa2 eqb2
    -> eq_term_equals (pweq lib eqp1 eqa1 eqb1 cp ca cb C p)
                      (pweq lib eqp2 eqa2 eqb2 cp ca cb C p).
Proof.
  introv eqip eqia eqib.
  unfold eq_term_equals; introv; split; intro k.
  apply @pweq_implies with (eqp1 := eqp1) (eqa1 := eqa1) (eqb1 := eqb1); sp.

  apply @pweq_implies with (eqp1 := eqp2) (eqa1 := eqa2) (eqb1 := eqb2); sp;
  try (apply eq_term_equals_fam_fam_sym; sp);
  try (apply eq_term_equals_fam_sym; sp);
  try (apply eq_term_equals_sym; sp).
Qed.

Lemma eq_term_equals_pweq2 {o} :
  forall lib (eqp1 eqp2 : per(o)) eqa1 eqa2 eqb1 eqb2
         cp1 ca1 cb1 C1 p1
         cp2 ca2 cb2 C2 p2,
    term_equality_symmetric eqp1
    ...
    -> eqp1 p1 p2
    -> eq_term_equals (pweq lib eqp1 eqa1 eqb1 cp1 ca1 cb1 C1 p1)
                      (pweq lib eqp2 eqa2 eqb2 cp2 ca2 cb2 C2 p2).
Proof.
  ...
  apply @pweq_implies2 with (eqp1 := eqp1) (eqa1 := eqa1) (eqb1 := eqb1) (cp1 := cp1) (ca1 := ca1) (cb1 := cb1) (C1 := C1) (p1 := p1); auto.

  apply @pweq_implies2 with (eqp1 := eqp2) (eqa1 := eqa2) (eqb1 := eqb2) (cp1 := cp2) (ca1 := ca2) (cb1 := cb2) (C1 := C2) (p1 := p2);
    sp;
    try (apply eq_term_equals_fam_fam_sym; sp);
    try (apply eq_term_equals_fam_sym; sp);
    try (apply eq_term_equals_sym; sp).
...
Lemma pweq_sym {o} :
  forall lib (eqp : per(o)) eqa eqb t1 t2 P1 P2 v1 v2 A1 A2
    ...
    -> pweq lib eqp eqa eqb cp ca cb C p t2 t1.
Proof.
  introv teqsp teqtp tsp ftsp fftsp eqCs Hweq.
  induction Hweq as
      [ps tts1 tts2 eqps a1 f1 a2 f2 eqpa Hib Hic Hid Hie].
  dup ftsp as tspa.
  repnud ftsp.
  ...
  eapply @pweq_cons with  (a2:=a1) (a1:=a2) (f1 := f2)  (f2 := f1)
    (ep:=eqps) (ea := eqpa); eauto.
  introv Heqb.
  assert (eq_term_equals
            (eqb ps ps eqps a2 a1 eqpa)
            (eqb ps ps eqps a1 a2 eqpas))
...
(*  clear ftspuv ftsptys ftsptyt ftsptyst ftsptyvr
  ...
  apply pweq_implies2 with (eqp1 := eqp)
                             (eqa1 := eqa)
                             (eqb1 := eqb)
                             (cp1 := cp)
                             (ca1 := ca)
...
Proof.
  ...
  assert (eqa p1 p1 ep a1 a1)
    as ea1
      by (generalize (tspa p1 p1 ep); intro tsp;
          onedtsp uv tys tyt tyst tyvr tes tet tevr tygs tygt dum;
          apply tet with (t2 := a2); sp).

  assert (eqa p1 p1 ep a1 a)
    as ea'
      by (generalize (tspa p1 p1 ep); intro tsp;
          onedtsp uv tys tyt tyst tyvr tes tet tevr tygs tygt dum;
          apply tet with (t2 := a2); sp;
  ...
  apply @pweq_cons with (ep := ep) (a1 := a1) (f1 := f1) (a2 := a) (f2 := f) (ea := ea');
    try (complete (spcast; sp)).
  introv eb.
  apply I with (b2 := b2); sp.

  applydup @tsp_implies_eq_fam_fam_refl_left in tspb as rl.
  generalize (rl p1 p1 ep ep a1 a ea' ea1); intro e1.
  generalize (rl p1 p1 ep ep a1 a2 ea ea1); intro e2.
  apply e2.
  apply e1 in eb; auto.
...
Lemma pweq_implies_param {o} :
  forall lib (ts : cts(o)) eqp eqa eqb
    ...
    -> equal_Cparams eqp eqa eqb cp ca cb C cp' ca' cb' C'
    -> pweq lib eqp eqa eqb cp ca cb C p1 t1 t2
    -> eqp p1 p2
    -> pweq lib eqp eqa eqb cp ca cb C p2 t1 t2.
Proof.
  introv tspp tspa tspb eqCs pw eps.
  revert dependent p2.
  induction pw as [ p1 t1 t2 ep a1 f1 a2 f2 ea c1 c2 I R ].
  introv eps.
  ...
  apply @pweq_cons with (ep := ep2) (a1 := a1) (f1 := f1) (a2 := a2) (f2 := f2) (ea := ea2);
    try (complete (spcast; sp)).
...
Lemma pweq_trans {o} :
  forall lib (ts : cts(o)) P P' eqp ap A ap' A' eqa bp ba B bp' ba' B' eqb
    ...
    -> equal_Cparams eqp eqa eqb cp ca cb C cp' ca' cb' C'
    -> pweq lib eqp eqa eqb cp ca cb C p t1 t2
    -> pweq lib eqp eqa eqb cp ca cb C p t2 t3
    -> pweq lib eqp eqa eqb cp ca cb C p t1 t3.
Proof.
  introv tspp tspa tspb eqCs pw1 pw2.
  revert dependent t3.
  induction pw1 as [ p1 t1 t2 ep a1 f1 a2 f2 ea c1 c2 R I ].
  introv pw2.
  inversion pw2 as [ ep' a2' f2' a3 f3 ea' c2' c3 R2 ].
  ccomputes_to_eqval.

  assert (eqa p1 p1 ep a1 a3)
         as ea2
         by (apply @type_sys_props_fam_implies_sym_trans_respeq with (P := P) (P' := P') in tspa; auto; repnd;
             generalize (tspa1 p1 p1 ep); intro trans;
             apply trans with (t2 := a2); auto;
             generalize (tspa p1 p1 p1 p1 ep ep' ep ep); intro e; apply e; auto).
  ...
  apply @pweq_cons with (ep := ep) (a1 := a1) (f1 := f1) (a2 := a3) (f2 := f3) (ea := ea2);
    try (complete (spcast; sp)).

  introv eb.

  apply I with (b2 := b1); try (complete sp).

  applydup @tsp_implies_eq_fam_fam_refl_left in tspb as k.
  generalize (k p1 p1 ep ep a1 a3 ea2 ea1); intro e1.
  apply e1 in eb.
  generalize (k p1 p1 ep ep a1 a2 ea ea1); intro e2.
  apply e2.
  apply @type_sys_props_fam_fam_implies_sym_trans_respeq with (P := P) (P' := P') (ap := ap) (A := A) (ap' := ap') (A' := A') in tspb; repnd; sp.
  apply tspb1 with (t2 := b2); sp.
  apply tspb0; sp.

  assert (eqa p1 p1 ep a3 a3)
         as ea''
         by (apply @type_sys_props_fam_implies_sym_trans_respeq with (P := P) (P' := P') in tspa; auto; repnd;
             apply tspa1 with (t2 := a1); sp; apply tspa0; sp).

  assert (eqb p1 p1 ep' a2 a3 ea' b1 b2)
         as eb'
         by (applydup @tsp_implies_eq_fam_fam_refl_right in tspb as k;
             generalize (k p1 p1 ep' ep a2 a3 ea' ea''); intro e; apply e; clear e;
             generalize (k p1 p1 ep ep a1 a3 ea2 ea''); intro e; apply e in eb; clear e; auto).

  generalize (R2 b1 b2 eb'); intro pw.

  generalize (pweq_implies_param
                lib ts eqp eqa eqb
                P P' ap A ap' A' bp ba B bp' ba' B'
                cp ca cb C cp' ca' cb' C'
  ...
  applydup @type_sys_props_implies_term_eq_sym in tspp as sym.
  apply sym.
  generalize (j p1 p1 ep a1 a2 ea b1 b1); intro l; apply l.
